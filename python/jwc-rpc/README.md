...
Typed Python bindings for the `jwc --mode rpc` protocol used by the coding agent.
...
- typed startup options for common `jwc --mode rpc` flags such as thinking level,
...
```python
from jwc_rpc import MessageUpdateEvent, RpcClient
...
## Attaching to a Running Socket Server

When jwc is already listening on a Unix socket (`jwc --mode rpc --listen <path>`), discover the session via the cross-process registry and attach without spawning a child process. Process-backed `RpcClient()` remains the default for starting a new RPC server.

```python
from jwc_rpc import RpcClient, list_sessions

for owner in list_sessions():
    if owner.transport == "socket" and owner.endpoint:
        with RpcClient.connect_unix(owner.endpoint) as client:
            print(client.get_state().session_id)
```

Registry records use `transport: "socket"` and `endpoint` for the socket path. Older `stdio` records without `endpoint` stay valid.
...
from jwc_rpc import RpcClient, host_tool
...
```python
from jwc_rpc import RpcClient, host_uri
...
```python
from jwc_rpc import assistant_text, assistant_text_with_thinking
