"""Discovery of running jwc RPC sessions (issue 10).

Each live ``jwc --mode rpc`` server writes a record under
...
Agent directory resolution mirrors TypeScript ``getAgentDir()``: the first set
value among ``JWC_CODING_AGENT_DIR``, ``GJC_CODING_AGENT_DIR``, and
``PI_CODING_AGENT_DIR`` (legacy aliases); otherwise
``$HOME/<config-dir>/agent`` where ``config-dir`` is ``JWC_CONFIG_DIR``,
``GJC_CONFIG_DIR``, ``PI_CONFIG_DIR``, or ``.jwc``. Pass ``sessions_dir``
explicitly for custom layouts.
"""
...
def _resolve_agent_dir() -> Path:
    for key in ("JWC_CODING_AGENT_DIR", "GJC_CODING_AGENT_DIR", "PI_CODING_AGENT_DIR"):
        raw = os.environ.get(key)
        if raw and raw.strip():
            return Path(raw).expanduser().resolve()
    config_dir = (
        os.environ.get("JWC_CONFIG_DIR")
        or os.environ.get("GJC_CONFIG_DIR")
        or os.environ.get("PI_CONFIG_DIR")
        or ".jwc"
    )
    return (Path.home() / config_dir / "agent").resolve()


def _resolve_sessions_dir(sessions_dir: str | Path | None) -> Path:
    if sessions_dir is not None:
        return Path(sessions_dir)
    return _resolve_agent_dir() / "rpc-sessions"
...
def list_sessions(sessions_dir: str | Path | None = None) -> tuple[SessionHandle, ...]:
    """List live jwc RPC sessions, reaping records whose process is gone (issue 10)."""
