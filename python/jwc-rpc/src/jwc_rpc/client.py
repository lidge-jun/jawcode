...
from .registry import SessionHandle, list_sessions as registry_list_sessions
...
class RpcClient:
    def __init__(
        ...
        *,
        ...
        self._transport_mode: Literal["process", "socket"] = "process"
        self._unix_socket_path: Path | None = None
        self._process: subprocess.Popen[str] | None = None
        self._socket: socket.socket | None = None
        self._socket_reader: io.TextIOWrapper | None = None
    ...
    def __exit__(self, _exc_type: object, _exc: object, _tb: object) -> None:
        self.stop()
    @classmethod
    def list_sessions(cls, sessions_dir: str | Path | None = None) -> tuple[SessionHandle, ...]:
        """Discover live jwc RPC sessions via the cross-process registry (issue 10)."""
        return registry_list_sessions(sessions_dir)
    @classmethod
    def connect_unix(
        cls,
        ...
        *,
        ...
        """Attach to an already-running jwc RPC server listening on a Unix socket."""
        client = cls(
            startup_timeout=startup_timeout,
            request_timeout=request_timeout,
            max_event_history=max_event_history,
        )
        client._transport_mode = "socket"
        client._unix_socket_path = Path(socket_path)
        return client.start()
    ...
    def start(self) -> RpcClient:
        ...
        if self._transport_mode == "socket":
            self._start_socket_transport()
        else:
            self._start_process_transport()
        ...
        if not self._ready_received:
            ...
            if error is not None:
                if self._transport_mode == "socket":
                    raise RpcError(f"RPC socket closed before ready: {error}. Stderr: {stderr}") from error
    ...
    def _start_process_transport(self) -> None:
        ...
        self._stdout_thread = threading.Thread(target=self._read_stdout_loop, name="jwc-rpc-stdout", daemon=True)
        self._stderr_thread = threading.Thread(target=self._read_stderr_loop, name="jwc-rpc-stderr", daemon=True)
        self._stdout_thread.start()
        self._stderr_thread.start()

    def _start_socket_transport(self) -> None:
        if self._unix_socket_path is None:
            raise RpcError("Unix socket path is not configured")
        sock = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
        try:
            sock.connect(str(self._unix_socket_path))
        except OSError as exc:
            sock.close()
            raise RpcError(f"Failed to connect to RPC socket at {self._unix_socket_path}: {exc}") from exc
        sock_file = io.TextIOWrapper(sock.makefile("rwb", buffering=0), encoding="utf-8", newline="\n", write_through=True)
        self._socket = sock
        self._socket_reader = sock_file
        self._stdout_thread = threading.Thread(target=self._read_socket_loop, name="jwc-rpc-socket", daemon=True)
        self._stdout_thread.start()

    def stop(self) -> None:
        if self._transport_mode == "socket":
            self._stop_socket_transport()
            return
    ...
    def _stop_socket_transport(self) -> None:
        if self._socket is None and self._socket_reader is None:
            return
        ...
        try:
            ...
            self._mark_closed(RpcError("RPC socket closed"))
            self._pending_host_tool_calls.clear()
            self._pending_host_uri_requests.clear()
            self._socket = None
    ...
    def run_workflow_gate_policy(
        ...
        def handle(gate: WorkflowGate) -> None:
            threading.Thread(
                ...
                name=f"jwc-rpc-workflow-gate-{gate.gate_id}",
                daemon=True,
            ).start()
    ...
    def _request(self, command_type: str, **payload: JsonValue) -> JsonObject:
        ...
        try:
            self._write_transport_line(envelope)
    ...
    def _send_notification(self, payload: JsonObject) -> None:
        self._require_transport()
        self._write_transport_line(payload)
    ...
    def _handle_host_tool_call(self, payload: JsonObject) -> None:
        ...
        threading.Thread(target=run_tool, name=f"jwc-rpc-host-tool:{tool_name}", daemon=True).start()
    ...
    def _handle_host_uri_request(self, payload: JsonObject) -> None:
        ...
        threading.Thread(target=run, name=f"jwc-rpc-host-uri:{scheme}:{operation}", daemon=True).start()
    ...
    def _transport_write_error(self, exc: BaseException) -> BaseException:
        if self._transport_mode == "socket":
            return RpcError(f"Failed to write RPC command: {exc}")
        return RpcProcessExitError(f"Failed to write RPC command: {exc}")

    def _write_transport_line(self, payload: JsonObject) -> None:
        line = json.dumps(payload) + "\n"
        with self._write_lock:
            try:
                if self._transport_mode == "socket":
                    if self._socket_reader is None:
                        raise RpcError("RPC socket writer is unavailable")
                    self._socket_reader.write(line)
                    self._socket_reader.flush()
                    return
                process = self._process
                ...
                raise self._transport_write_error(exc) from exc

    def _handle_transport_line(self, stripped: str, *, line_number: int) -> None:
        try:
    ...
    def _read_stdout_loop(self) -> None:
        ...
        try:
            for line in process.stdout:
                ...
                if not stripped:
                    continue
                self._handle_transport_line(stripped, line_number=line_number)
        except Exception as exc:
