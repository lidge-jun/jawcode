...
- `question` → one comment, suffixed with a 👎-to-keep-open prompt; if the
  issue author doesn't react 👎 within `ROBJWC_QUESTION_AUTOCLOSE_HOURS`
...
- **robojwc** — FastAPI + sqlite event queue + `WorkerPool` running `gjc` in
...
- **gh-proxy** — sibling on an `internal: true` network. Holds `GITHUB_TOKEN`,
  verifies HMAC-signed requests from robojwc, executes REST + `git push`.
...
persistent `session_dir`, model randomly drawn from `ROBJWC_MODEL` (CSV).
...
comment out `ROBJWC_GH_PROXY_URL` / `ROBJWC_GH_PROXY_HMAC_KEY` and set
...
robojwc does not ship a tunnel. Cloudflare, smee, ngrok are all fine. The
...
The container entrypoint is `python -m robojwc serve`. Other commands run
inside the running container:

```bash
docker compose exec robojwc robojwc triage  owner/repo#123   # synthesize an issues.opened and wait
docker compose exec robojwc robojwc replay  <delivery_id>    # re-enqueue a stored event and wait
docker compose exec robojwc robojwc status                   # dump issues table
docker compose exec robojwc robojwc cleanup owner/repo#123   # force workspace removal, state=abandoned
```

`bun run robojwc:…` shortcuts in the root `package.json` cover the common
lifecycle commands (`robojwc:dev`, `robojwc:build`, `robojwc:up`, `robojwc:down`,
`robojwc:logs`, `robojwc:restart`, `robojwc:reset`).

## Tests

```bash
pytest -x tests/                              # unit suite, no network
ROBJWC_INTEGRATION=1 pytest -x tests/test_worker_smoke.py
```
...
- gh-proxy has no host port. The `robojwc_internal` network is
...
- Agent subprocess env is scrubbed of `GITHUB_TOKEN` /
  `ROBJWC_GH_PROXY_HMAC_KEY` / friends via `worker._SCRUBBED_ENV_KEYS`.
...
- Pre-push gates (`gh_push_branch`): branch matches the workspace
  ...
  `origin/<default>..HEAD` carries `ROBJWC_GIT_AUTHOR_NAME` +
  `ROBJWC_GIT_AUTHOR_EMAIL`.
...
- **Crash recovery.** On startup, `db.reset_stuck_running()` flips
  ...
  `ROBJWC_SHUTDOWN_DRAIN_TIMEOUT_SECONDS` (25s) +
  `ROBJWC_SHUTDOWN_KILL_TIMEOUT_SECONDS` (5s); compose
  `stop_grace_period: 30s` covers both.
- **Logs.** Structured JSON on stdout, rotated to
  `/data/logs/robojwc.log.jsonl`.
...
| `Failed to load pi_natives` | Wrong arch / missing native. `bun run pi:image` then `bun run robojwc:build`. |
| `No API key found for <provider>` | `~/.gjc/agent/models.container.yml` mount missing or provider id mismatch with `ROBJWC_MODEL`. |
...
tests/               pytest unit suite + one ROBJWC_INTEGRATION=1 smoke test
