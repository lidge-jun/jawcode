#!/usr/bin/env bash
# robojwc container entrypoint. No per-boot pip installs — everything is baked
# into the image; we only sanity-check the runtime mount and create state dirs.
#
# Used by both the orchestrator (CMD: `python -m robojwc serve`) and the
# sibling gh-proxy (compose command: `python -m robojwc.proxy serve`). The
...
# as $@ here (after tini --), so $1=python, $2=-m, $3=robojwc.proxy is the
# canonical shape; we also accept a single concatenated arg for safety.
is_proxy_role=0
if [ "${1:-}" = "python" ] && [ "${2:-}" = "-m" ] && [[ "${3:-}" == robojwc.proxy* ]]; then
    is_proxy_role=1
elif [[ "${1:-}" == *"robojwc.proxy"* ]]; then
    is_proxy_role=1
fi

/usr/sbin/groupadd -f -g 2000 gjc
max_slots="${ROBJWC_MAX_CONCURRENCY:-8}"
...
if [ ! -d "$PI_ROOT/packages/coding-agent" ]; then
    echo "robojwc: PI_ROOT=$PI_ROOT does not look like a pi checkout (no packages/coding-agent/)" >&2
    exit 1
fi
...
rm -rf /srv/agent-home/.agent /srv/agent-home/.jwc/agent
mkdir -p /srv/agent-home/.agent /srv/agent-home/.jwc/agent
if [ -e /srv/agent-home-stage/.agent ]; then
    cp -a /srv/agent-home-stage/.agent/. /srv/agent-home/.agent/
fi
if [ -e /srv/agent-home-stage/.jwc/agent ]; then
    cp -a /srv/agent-home-stage/.jwc/agent/. /srv/agent-home/.jwc/agent/
fi
...
touch /data/robojwc.sqlite
chown root:root /data/robojwc.sqlite
chmod 0600 /data/robojwc.sqlite
for db_file in /data/robojwc.sqlite-wal /data/robojwc.sqlite-shm; do
    if [ -e "$db_file" ]; then
