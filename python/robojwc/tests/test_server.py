...
from robojwc.config import Settings, reset_settings_cache
from robojwc.dashboard import tail_jsonl
from robojwc.db import Database, close_database, get_database, issue_key
from robojwc.github_client import GitHubClient
from robojwc.manual_triage import InvalidIssueRef, ManualTriageTimeout, await_terminal_state, parse_issue_ref
from robojwc.sandbox import LocalGitTransport
from robojwc.server import create_app
...
def test_index_substitutes_replay_token(env, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("ROBJWC_REPLAY_TOKEN", "secret-token-7")
...
def test_api_status_reports_runtime_counts_and_inflight(settings: Settings) -> None:
    ...
    assert runtime["bot_login"] == "robojwc-bot"
...
def test_api_logs_tails_jsonl_file(settings: Settings) -> None:
    log_path = settings.log_dir / "robojwc.log.jsonl"
    log_path.parent.mkdir(parents=True, exist_ok=True)
    payloads = [
        {"ts": "2026-05-14T21:28:28Z", "level": "INFO", "logger": "robojwc.queue", "msg": "dispatch loop online"},
        {
            "ts": "2026-05-14T21:28:54Z",
            "level": "INFO",
            "logger": "robojwc.server",
            "msg": "skip",
            "event": "issues",
            "reason": "issues.labeled ignored",
        },
        {"ts": "2026-05-14T21:30:00Z", "level": "WARNING", "logger": "robojwc.queue", "msg": "tool_end", "ok": False},
    ]
...
def test_parse_issue_ref_accepts_github_issue_urls() -> None:
    cases = (
        "https://github.com/can1357/jawcode/issues/1348",
        "http://github.com/can1357/jawcode/issues/1348",
        "github.com/can1357/jawcode/issues/1348",
        "https://www.github.com/can1357/jawcode/issues/1348",
        "https://github.com/can1357/jawcode/issues/1348/",
        "https://github.com/can1357/jawcode/issues/1348?foo=bar",
        "https://github.com/can1357/jawcode/issues/1348#issuecomment-99",
        "  https://github.com/can1357/jawcode/issues/1348  ",
    )
    for case in cases:
        assert parse_issue_ref(case) == ("can1357/jawcode", 1348), case
...
def _enable_replay(monkeypatch: pytest.MonkeyPatch) -> str:
    ...
    monkeypatch.setenv("ROBJWC_REPLAY_TOKEN", token)
    reset_settings_cache()
    return token
...
def test_trigger_triage_rejects_repo_not_in_allowlist(env, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "ROBJWC_REPO_ALLOWLIST" in resp.json()["detail"]
...
def rate_limited_settings(monkeypatch: pytest.MonkeyPatch, env: dict[str, str]) -> Settings:
    monkeypatch.setenv("ROBJWC_RATE_LIMIT_DEFAULT", "2")
    monkeypatch.setenv("ROBJWC_RATE_LIMIT_CONTRIBUTOR", "4")
    monkeypatch.setenv("ROBJWC_RATE_LIMIT_WINDOW_SECONDS", "3600")
    monkeypatch.setenv("ROBJWC_RATE_LIMIT_UNLIMITED", "can1357")
    cfg = Settings()  # type: ignore[call-arg]
    cfg.ensure_paths()
    return cfg
...
def _allowlist(monkeypatch: pytest.MonkeyPatch, repos: str) -> None:
    monkeypatch.setenv("ROBJWC_REPO_ALLOWLIST", repos)
    reset_settings_cache()
...
def test_webhook_directive_on_unknown_issue_is_queued_with_metadata(env) -> None:
    ...
    with TestClient(app) as client:
        resp = _post_issue_comment(
            ...
            user="can1357",
            number=77,
            body="@robojwc-bot please refactor X",
            association="OWNER",
        )
        assert resp.status_code == 202
    ...
    assert row is not None
    assert row.state == "queued"
    directive = row.payload.get("_robojwc_directive")
...
def test_webhook_maintainer_bypasses_rate_limit(
    ...
    """Login in ROBJWC_MAINTAINER_LOGINS is always unlimited, even with NONE association."""
    monkeypatch.setenv("ROBJWC_MAINTAINER_LOGINS", "can1357")
    reset_settings_cache()
    cfg = Settings()  # type: ignore[call-arg]
    cfg.ensure_paths()
    ...
    with TestClient(app) as client:
        ...
        for i in range(4):
            resp = _post_issue_comment(
                ...
                user="can1357",
                number=300 + i,
                body=("@robojwc-bot do X" if i == 3 else "comment"),
                association="NONE",
            )
            assert resp.status_code == 202
            states.append(resp.json()["state"])
...
def stub_run_task(monkeypatch: pytest.MonkeyPatch) -> list[dict]:
    ...
    from robojwc import tasks as tasks_module
...
async def test_handle_pr_conversation_unmapped_bot_pr_uses_pr_branch(
    ...
    from robojwc import tasks
    from robojwc.github_client import CommentInfo, IssueInfo, PullRequestInfo, RepoInfo
...
async def test_handle_comment_directive_bootstraps_untriaged_issue(
    ...
    from robojwc import tasks
    from robojwc.github_client import GitHubClient, IssueInfo, RepoInfo
    ...
    payload = {
        ...
        "repository": {"full_name": "octo/widget"},
        "_robojwc_directive": {"body": "please refactor X", "author": "can1357"},
    }
...
async def test_handle_comment_directive_reopens_finalized_issue(
    ...
    payload = {
        ...
        "repository": {"full_name": "octo/widget"},
        "_robojwc_directive": {"body": "redo the fix", "author": "can1357"},
    }
    await tasks.handle_comment(
        settings=settings,
        db=db,
...
async def test_directive_handler_attaches_thread_from_github(
    ...
    from robojwc import tasks
    from robojwc.github_client import (
        CommentInfo,
        GitHubClient,
        IssueInfo,
        RepoInfo,
    ...
    payload = {
        ...
        "repository": {"full_name": "octo/widget"},
        "_robojwc_directive": {"body": "do X", "author": "can1357"},
    }
...
async def test_triage_issue_skips_when_a_closing_pr_already_exists(
    ...
    from robojwc import tasks
    from robojwc.github_client import IssueInfo, RepoInfo
...
async def test_triage_issue_fails_open_when_timeline_fetch_errors(
    ...
    from robojwc import tasks
    from robojwc.github_client import GitHubError, IssueInfo, RepoInfo
