...
Runs only when ROBJWC_INTEGRATION=1 and `gjc` is available on PATH (or via
ROBJWC_GJC_COMMAND). Spins up:
...
INTEGRATION = os.environ.get("ROBJWC_INTEGRATION") == "1"

pytestmark = pytest.mark.skipif(
    not INTEGRATION,
    reason="ROBJWC_INTEGRATION=1 required to run the gjc-backed smoke test",
)
...
def test_triage_end_to_end(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from robojwc.sandbox import LocalGitTransport, SandboxManager
    from robojwc.tasks import triage_issue

    bare = _seed_failing_repo(tmp_path)

    monkeypatch.setenv("ROBJWC_GH_PROXY_URL", "http://gh-proxy.invalid:8081")
    monkeypatch.setenv("ROBJWC_GH_PROXY_HMAC_KEY", "test-hmac-key-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
    monkeypatch.setenv("GITHUB_TOKEN", "")
    monkeypatch.setenv("GITHUB_WEBHOOK_SECRET", "secret")
    monkeypatch.setenv("ROBJWC_BOT_LOGIN", "robojwc-bot")
    monkeypatch.setenv("ROBJWC_REPO_ALLOWLIST", "octo/widget")
    monkeypatch.setenv("ROBJWC_WORKSPACE_ROOT", str(tmp_path / "workspaces"))
    monkeypatch.setenv("ROBJWC_SQLITE_PATH", str(tmp_path / "robojwc.sqlite"))
    monkeypatch.setenv("ROBJWC_LOG_DIR", str(tmp_path / "logs"))
    monkeypatch.setenv("ROBJWC_TASK_TIMEOUT_SECONDS", "300")
