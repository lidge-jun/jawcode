...
from robojwc.github_events import (
    extract_mention,
    is_maintainer,
    rate_limit_cap,
    route,
...
def test_extract_mention_returns_body_minus_mention() -> None:
    assert extract_mention("hey @robojwc-bot please look", "robojwc-bot") == "hey please look"
    assert extract_mention("@robojwc-bot do X", "robojwc-bot") == "do X"


def test_extract_mention_returns_none_without_mention() -> None:
    assert extract_mention("hello there", "robojwc-bot") is None
    assert extract_mention(None, "robojwc-bot") is None
    assert extract_mention("", "robojwc-bot") is None


def test_extract_mention_is_case_insensitive() -> None:
    assert extract_mention("yo @ROBJWC-BOT", "robojwc-bot") == "yo"


def test_extract_mention_respects_hyphen_word_boundary() -> None:
    # @robojwc-bot-helper must NOT match @robojwc-bot.
    assert extract_mention("@robojwc-bot-helper hi", "robojwc-bot") is None


def test_extract_mention_handles_multiple_occurrences() -> None:
    assert extract_mention("@robojwc-bot one, then @robojwc-bot two", "robojwc-bot") == "one, then two"
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                "user": {"login": "can1357"},
                "author_association": "OWNER",
                "body": "@robojwc-bot change the indentation in foo.py",
            },
    ...
    decision = route(
        "pull_request_review_comment",
        {
            ...
            "comment": {
                "user": {"login": "can1357"},
                "author_association": "OWNER",
                "body": "@robojwc-bot use a generator here",
            },
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                "user": {"login": "can1357"},
                "author_association": "OWNER",
                "body": "@robojwc-bot /model gpt /thinking low\nrefactor X",
            },
            "issue": {"number": 9},
            "repository": {"full_name": "octo/widget"},
        },
