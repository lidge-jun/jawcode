...
from robojwc.sandbox import LocalGitTransport, SandboxManager, Workspace

pytestmark = pytest.mark.skipif(
    os.environ.get("ROBJWC_PERMISSION_E2E") != "1",
    reason="set ROBJWC_PERMISSION_E2E=1 to run slot-permission e2e tests",
)

_SLOT_ONE = 2001
_SLOT_TWO = 2002
_SHARED_GJC_GID = 2000
_AUTHOR_NAME = "robojwc-bot"
_AUTHOR_EMAIL = "robojwc-bot@example.invalid"
...
def slot_tmp_path() -> Iterator[Path]:
    root = Path(tempfile.mkdtemp(prefix="robojwc-permission-e2e-", dir="/tmp"))
    root.chmod(0o755)
    try:
        yield root
    finally:
