...
from robojwc.dashboard import reset_index_cache, static_dir
from robojwc.db import Database, close_database

# Minimum HTML the dashboard handler needs to render: `<title>` plus a script
# block carrying the `__ROBJWC_CONFIG__` sentinel. The real Vite-built bundle
# adds JS/CSS asset links; tests only care about the rendering contract.
_PLACEHOLDER_INDEX_HTML = (
    "<!doctype html>\n"
    '<html lang="en">\n'
    '  <head><meta charset="utf-8"><title>robojwc</title></head>\n'
    "  <body>\n"
    '    <div id="app"></div>\n'
    '    <script id="robojwc-config" type="application/json">__ROBJWC_CONFIG__</script>\n'
    "  </body>\n"
    "</html>\n"
)
...
def _baseline_env(tmp_path: Path) -> dict[str, str]:
    return {
        ...
        "ROBJWC_GH_PROXY_URL": "http://gh-proxy.invalid:8081",
        "ROBJWC_GH_PROXY_HMAC_KEY": "test-hmac-key-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
        "GITHUB_WEBHOOK_SECRET": "test-webhook-secret",
        "ROBJWC_BOT_LOGIN": "robojwc-bot",
        "ROBJWC_GIT_AUTHOR_NAME": "robojwc-bot",
        "ROBJWC_GIT_AUTHOR_EMAIL": "robojwc-bot@example.invalid",
        "ROBJWC_REPO_ALLOWLIST": "octo/widget",
        "ROBJWC_MODEL": "anthropic/claude-sonnet-4-5",
        "ROBJWC_THINKING": "high",
        "ROBJWC_WORKSPACE_ROOT": str(tmp_path / "workspaces"),
        "ROBJWC_SQLITE_PATH": str(tmp_path / "robojwc.sqlite"),
        "ROBJWC_LOG_DIR": str(tmp_path / "logs"),
        ...
        # cache flip `ROBJWC_NATIVES_CACHE_ENABLED=true` explicitly.
        "ROBJWC_NATIVES_CACHE_ROOT": str(tmp_path / "natives-cache"),
        "ROBJWC_NATIVES_CACHE_ENABLED": "false",
    }
...
def env(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> dict[str, str]:
    ...
    monkeypatch.delenv("ROBJWC_PROVIDER", raising=False)
    monkeypatch.setenv("ROBJWC_REPLAY_TOKEN", "")
...
def proxy_env(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> dict[str, str]:
    ...
    baseline.pop("ROBJWC_GH_PROXY_URL", None)
    baseline.pop("ROBJWC_GH_PROXY_HMAC_KEY", None)
    ...
    monkeypatch.setenv("ROBJWC_GH_PROXY_URL", "")
    monkeypatch.setenv("ROBJWC_GH_PROXY_HMAC_KEY", "")
    monkeypatch.delenv("ROBJWC_PROVIDER", raising=False)
    monkeypatch.setenv("ROBJWC_REPLAY_TOKEN", "")
    reset_settings_cache()
    yield baseline
