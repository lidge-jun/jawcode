...
// `__ROBJWC_CONFIG__` sentinel in `static/index.html` with a JSON blob so the
...
function readConfig(): AppConfig {
  const node = document.getElementById("robojwc-config");
  const text = node?.textContent?.trim();
  if (!text || text === "__ROBJWC_CONFIG__") {
