...
4. `queue.WorkerPool._dispatch_loop` atomically claims `state='queued'` rows under `BEGIN IMMEDIATE`, guarded by an in-process `_inflight` set keyed by `(owner, repo, number)` to serialize per-issue work. Cap: `ROBJWC_MAX_CONCURRENCY` (default 8).
...
6. `tasks.*` dispatchers build `TaskInputs` and call `worker.run_task()` which spawns `gjc --mode rpc` with `cwd=worktree`, persistent `session_dir`, and a randomly-picked model from `ROBJWC_MODEL` (CSV pool). When `<session_dir>/*.jsonl` already exists the worker passes `--continue`, so both follow-up events and crash-restarted events resume the same session.
...
Task runner is `bun` against the **monorepo root** `package.json`. robojwc itself no longer ships a `package.json`; every recipe lives at the root under the `robojwc:*` namespace. Local venv (no docker): `bun run robojwc:install` runs `pip install -e 'python/robojwc[dev]'`. From there:

```
bun run test:py                   # pytest -x python/jwc-rpc/tests python/robojwc/tests
bun run robojwc:test:integration   # ROBJWC_INTEGRATION=1, requires gjc on PATH
bun run robojwc:serve              # python -m robojwc serve on the host
```
...
bun run robojwc:rebuild            # docker compose build --no-cache
bun run robojwc:reset              # `down -v` + drop the pi image
```

Frontend (Vite + SolidJS, in `web/` — still a bun workspace):

```
bun run robojwc:web:dev            # vite dev server with proxy to :8080
bun run robojwc:web:build          # produce src/static/ bundle
bun --cwd=python/robojwc/web run typecheck   # tsc --noEmit
```

In-container CLI (`robojwc` console script → `robojwc.cli:main`): no root aliases — invoke directly:

```
docker compose --project-directory python/robojwc exec robojwc robojwc triage owner/repo#N
docker compose --project-directory python/robojwc exec robojwc robojwc replay <delivery_id>
docker compose --project-directory python/robojwc exec robojwc robojwc status
docker compose --project-directory python/robojwc exec robojwc robojwc cleanup owner/repo#N
```

HTTP / sqlite / webhook inspection is unaliased — use `curl http://localhost:${ROBJWC_BIND_PORT:-8080}/{healthz,readyz,events,issues}` and `docker compose --project-directory python/robojwc exec robojwc sqlite3 /data/robojwc.sqlite` directly.

Lint + format: TypeScript via Biome (config in `biome.json`), Python via Ruff (config in `pyproject.toml`). Root recipes cover both languages — `bun run lint` / `bun run fix` apply to the whole monorepo including robojwc. `bun run lint:py` / `bun run fix:py` scope to Python only.
...
- **Async style**: FastAPI handlers and `queue.WorkerPool` are async. `worker.run_task` is **synchronous** and runs in a worker thread because `jwc-rpc` is blocking — keep it that way; don't try to async it. CLI commands wrap with `asyncio.run`.
- **Config**: `pydantic-settings` `Settings` in `config.py` with `ROBJWC_*` env prefix (e.g. `ROBJWC_MAX_CONCURRENCY`, `ROBJWC_REPO_ALLOWLIST`). Access only via `get_settings()` (`@cache` singleton). Tests must call `reset_settings_cache()` after mutating env.
...
- **Fixtures** (`tests/conftest.py`):
  - `env` — `monkeypatch`-sets all required `ROBJWC_*` env vars and calls `reset_settings_cache()` before/after.
...
- **Integration**: `tests/test_worker_smoke.py` is gated by `ROBJWC_INTEGRATION=1` (uses `pytestmark.skipif`) and needs `gjc` on `PATH`. Don't enable it in default `bun run test:py`.
