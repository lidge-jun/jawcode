...
from robojwc import persona
from robojwc.config import Settings
from robojwc.db import Database, IssueRow, IssueState, issue_key
...
from robojwc.sandbox import GitTransport, SandboxManager
from robojwc.worker import DirectiveInfo, TaskInputs, ThreadMessage, run_task
...
def _directive_from_payload(payload: Mapping[str, Any]) -> DirectiveInfo | None:
    ...
    raw = payload.get("_robojwc_directive")
...
async def handle_pr_conversation(
    *,
    ...
    # _robojwc_directive; _directive_from_payload rejects it but the key
    # being present tells us a mention happened. Reply cheaply without gjc.
    if directive is None and payload.get("_robojwc_directive") is not None:
        comment = _comment_from_payload(payload)
        log.info(
