...
PAT injection via `--config-env` (see `robojwc.git_ops`); the `ProxyGitTransport`
in `robojwc.proxy_client` forwards the same set of operations over HMAC RPC.
...
from robojwc.natives_cache import CacheHit, NativesCache
...
class GitTransport(Protocol):
    ...
    - `robojwc.proxy_client.ProxyGitTransport`: forwards over HMAC RPC.
    """
