...
Robogjc calls every endpoint with HMAC headers (see `robojwc.proxy_hmac`).
Authenticated requests dispatch to a single `GitHubClient` instance holding
the PAT, or to `robojwc.git_ops` for git transport. The PAT never leaves
this process.

Endpoint payloads are deliberately typed (no generic GitHub passthrough):
each one names exactly one operation robojwc performs.
"""
...
from robojwc.github_client import GitHubClient, GitHubError
from robojwc.proxy_hmac import HEADER_SIGNATURE, HEADER_TIMESTAMP, verify
from robojwc.sandbox import _safe_directory_env, _slot_subprocess_kwargs
from robojwc.sandbox import workspace_key as compute_workspace_key
...
def _resolve_hmac_key(cfg: Settings) -> bytes:
    if cfg.gh_proxy_hmac_key is None:
        raise HTTPException(500, "gh-proxy: ROBJWC_GH_PROXY_HMAC_KEY not configured")
...
def create_proxy_app(settings: Settings) -> FastAPI:
    ...
    app = FastAPI(title="robojwc-gh-proxy", version="0.1.0", lifespan=lifespan)
    ...
    # ---- git transport ----
    #
    # The underlying `robojwc.git_ops` primitives are blocking `subprocess.run`
