...
```text
~/.jwc/agent/sessions/--<cwd-encoded>--/<timestamp>_<sessionId>.jsonl
```
...
```text
~/.jwc/agent/blobs/<sha256>
```

Terminal breadcrumb files are written under:

```text
~/.jwc/agent/terminal-sessions/<terminal-id>
```
...
- `listAll()` -> sessions across all project scopes under `~/.jwc/agent/sessions`
