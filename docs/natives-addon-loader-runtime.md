...
- **Core directories**:
  ...
  - `userDataDir` fallback:
    - Windows: `%LOCALAPPDATA%/jwc` or `%USERPROFILE%/AppData/Local/jwc`.
    - Non-Windows: `~/.local/bin`.
- **Natives cache root** (`getNativesDir()`):
  - if `$XDG_DATA_HOME/jwc` exists, `$XDG_DATA_HOME/jwc/natives`;
  - otherwise `~/.jwc/natives`.
- **Compiled-binary mode** (`detectCompiledBinary`): true if any of:
  - embedded-addon manifest is non-null,
  - `JWC_COMPILED` env var is set,
  - `import.meta.url` contains Bun embedded markers (`$bunfs`, `~BUN`, `%7EBUN`).
- **Variant override**: `JWC_NATIVE_VARIANT` (`modern`/`baseline` only; invalid values ignored).
...
1. `JWC_NATIVE_VARIANT=modern|baseline` wins when valid.
