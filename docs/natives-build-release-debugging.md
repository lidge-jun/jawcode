...
- `JWC_NATIVE_VARIANT`: x64 runtime override; valid values are `modern` and `baseline`.
- `JWC_COMPILED`: legacy compiled-mode signal. A populated embedded-addon manifest is also a compiled-mode signal and is the authoritative signal for Bun standalone builds that do not preserve `process.env.JWC_COMPILED`.
...
In compiled mode (`JWC_COMPILED`, Bun embedded URL markers, or populated embedded manifest):
...
3. Runtime candidate order includes:
   - versioned cache dir,
   - legacy compiled-binary dir (`%LOCALAPPDATA%/jwc` on Windows, `~/.local/bin` elsewhere),
...
| x64 machine loads baseline when modern expected                        | `JWC_NATIVE_VARIANT=baseline`, no AVX2 detected, or modern file unavailable                  | Check env and filenames in `native/`                              | Build modern variant (`TARGET_VARIANT=modern ... build`) and ship it                          |
...
- Root: `/data/cache/pi-natives` (provisioned by `entrypoint.sh` alongside the cargo caches, owned `root:jwc`, mode `02770` setgid so cached files inherit `gid=jwc` and stay readable by every slot user).
...
### Configuration (settings on `robojwc.config.Settings`)

| Env var                                      | Default                  | Effect                                                        |
| -------------------------------------------- | ------------------------ | ------------------------------------------------------------- |
| `ROBJWC_NATIVES_CACHE_ENABLED`               | `true`                   | Master switch. When false the populate/capture hooks no-op and every workspace builds from scratch. |
| `ROBJWC_NATIVES_CACHE_ROOT`                  | `/data/cache/pi-natives` | Cache root directory. Must be `root:jwc 02770` for cross-slot reads.                                  |
| `ROBJWC_NATIVES_CACHE_MAX_ENTRIES_PER_REPO`  | `8`                      | LRU entry-count cap, per repo slug.                                                                  |
| `ROBJWC_NATIVES_CACHE_MAX_BYTES`             | `4294967296` (4 GiB)     | LRU byte cap, per repo slug.                                                                          |
| `ROBJWC_NATIVES_CACHE_GC_INTERVAL_SECONDS`   | `3600`                   | Period of the background GC loop in `WorkerPool`.                                                    |
