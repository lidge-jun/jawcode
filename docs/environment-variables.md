...
> **Legacy aliases**: every `JWC_*` variable below also accepts its `JWC_*` legacy spelling (read-fallback, 062.1). New configurations should use `JWC_*`.
...
Additional rule inside each `.env` file: `JWC_*` keys are mirrored onto their legacy `JWC_*` spellings at load time (062.1 safety net).
...
| Variable                | Used for                                                                                          | Required when                                                                                                          | Notes / precedence                                                                                                                                                                                  |
| ----------------------- | ------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `JWC_AUTH_BROKER_URL`   | Base URL of the remote auth-broker (e.g. `https://broker.tailnet:8765`); selects broker mode      | Resolving credentials through a broker; also required by `jwc auth-gateway serve` (the gateway is itself a broker client) | Wins over `auth.broker.url` in `config.yml`. When set with no resolvable token, `resolveAuthBrokerConfig()` hard-errors instead of falling back to local SQLite.                                    |
...
The gateway has no dedicated env vars — it inherits `JWC_AUTH_BROKER_*`. Its own inbound bearer token lives at `<config-dir>/auth-gateway.token` and is managed via `jwc auth-gateway token`.
...
For OpenRouter traffic, JWC explicitly sends `User-Agent: Jawcode/<package version>` plus OpenRouter attribution headers. For the built-in OpenAI Responses transport and generic OpenAI-compatible Chat Completions transport, JWC passes model/provider headers through the OpenAI JavaScript SDK and does not set a JWC user-agent unless the provider-specific code adds one.
...
### JWC workflow bridge commands

Public workflow use is `jwc interview`, `jwc orchestrate`, `jwc goal`, and `jwc team`. Legacy private bridge command names may still exist internally while the engine cleanup slice is pending; they require `JWC_RUNTIME_BINARY` (or legacy `JWC_LEGACY_RUNTIME_BINARY`) to point at the private runtime executable and must not be documented as the primary user path.

| Variable | Behavior |
| --- | --- |
| `JWC_RUNTIME_BINARY` | Private runtime bridge binary for legacy workflow bridge commands |
| `JWC_LEGACY_RUNTIME_BINARY` | Legacy fallback bridge binary name |

### Team tmux backend, dry-run, and state paths

`jwc team ...` starts tmux worker panes from the current tmux-backed leader session. Start that leader with `jwc --tmux` first; `jwc team` intentionally does not create or attach the leader session itself.

`jwc team ... --dry-run --json` creates the same machine-readable state tree as a team launch without starting tmux panes. By default that state is written under `<cwd>/.jwc/state/team/<team>/`; treat it as ephemeral smoke-test/review state. Do not commit generated `.jwc/state/team` contents. Remove the generated team directory after a dry-run when the harness no longer needs it.
...
| `JWC_TEAM_WORKER_COMMAND` | Worker JWC command override |
| `JWC_TEAM_WORKER_CLI` | Team worker CLI selector; accepted values are `auto` or `jwc` |
| `JWC_TEAM_WORKER_CLI_MAP` | Comma-separated worker CLI selector map; entries must be `auto` or `jwc` |

### Hermes MCP bridge

`jwc mcp-serve coordinator` exposes a JWC-native outward MCP bridge for Hermes-style coordinators. The bridge is read-only by default and fails closed until roots and mutation classes are explicitly configured.

| Variable | Behavior |
| --- | --- |
| `JWC_COORDINATOR_MCP_WORKDIR_ROOTS` | Required allowlist for workdir and artifact paths. `jwc setup hermes` renders absolute normalized paths joined with the platform path delimiter (`:` on POSIX, `;` on Windows). The bridge parser also accepts commas, semicolons, and newlines for legacy manual configs. |
...
| `JWC_COORDINATOR_MCP_SESSION_COMMAND` | Optional JWC-compatible command used by mutating session startup to launch a detached tmux session. When unset, startup records a bridge session without tmux actuation unless a service adapter is injected. `jwc setup hermes` omits this by default and never hard-codes a provider/model; explicit values are preserved as user intent. |
| `JWC_COORDINATOR_MCP_SETUP_MANAGED_BY` | Marker written by `jwc setup hermes` for safe managed config updates. |
| `JWC_COORDINATOR_MCP_SETUP_SCHEMA_VERSION` | Managed setup schema version written by `jwc setup hermes`. |
...
| `JWC_SUBPROCESS_CMD`          | Overrides subagent spawn command (`jwc` / `jwc.cmd` resolution bypass)                             |
