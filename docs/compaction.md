...
- `compaction.thresholdPercent` = `73` and `compaction.thresholdTokens` = `-1`; the default threshold is 73% of the context window, capped so reserved output still fits. Setting `compaction.thresholdPercent` to `-1` keeps the legacy reserve-based threshold: `contextWindow - max(15% of contextWindow, reserveTokens, reserved output tokens)`.
