# Jawcode Onboarding Packet

This packet is a docs-only, public-safe context seed for the `jawcode` repository. It is intentionally a no-new-skill artifact: not a new workflow skill, command, agent, configuration surface, issue template, or runtime behavior.

## Purpose in one paragraph

Jawcode is the `jwc` coding-agent CLI and supporting monorepo. The product centers on a small public workflow loop: clarify with `jaw-interview`, plan and gate with `jwc orchestrate`, execute and verify through `jwc goal`, and use `team` only when parallel tmux workers are useful. The public package is `packages/jwc/`; the main runtime is `packages/coding-agent/`; supporting packages provide LLM/provider access, agent runtime, TUI rendering, native helpers, stats, utilities, benchmarks, and Python RPC/bot integrations.

## Fixed public surface

Keep this invariant front-and-center when onboarding to the repo:

- Default workflow definitions: `jaw-interview`, `plan`, `team`, `goal`.
- Public role agents: `executor`, `executor_ext`, `architect`, `planner`, `critic`.
- Bundled default workflow sources live under `packages/coding-agent/src/defaults/jwc/skills/`.
...
Do not add a fifth default skill, sixth public role agent, new command, new config surface, or feature-intake behavior unless that product decision has already been made and the default-surface gates are updated.
...
| Default workflows | `packages/coding-agent/src/defaults/jwc-defaults.ts` | Embeds and installs the four default workflow definitions plus jaw-interview/goal fragments. |
...
- `packages/jwc/` — public package `jawcode`, bin `jwc`, and `jawcode/sdk` export.
- `packages/coding-agent/` — main JWC runtime, workflows, session runtime, tool registry, discovery, settings, prompts, and tests.
...
- `python/jwc-rpc/` — legacy-named Python client for RPC mode.
- `python/robojwc/` — legacy-named GitHub triage/fix bot that drives RPC mode; this subtree has its own `AGENTS.md`.
...
| `bun test packages/coding-agent/test/default-jwc-definitions.test.ts` | Four-skills/five-callable-roles contract | Required after default workflow/agent surface changes.                      |

Repository rule: do not run `tsc` or `npx tsc`; use the Bun scripts above.

## Danger zones

- **Default surface expansion:** `packages/coding-agent/src/defaults/jwc/skills/`, `packages/coding-agent/src/defaults/jwc-defaults.ts`, `packages/coding-agent/src/prompts/agents/`, and model-assignment tests are contract-heavy. Changes here can accidentally alter the fixed four-workflows/five-callable-roles shape.
...
- **Python bot subtree:** `python/robojwc/` has its own local instructions and trust boundaries.
...
- Which onboarding packet shape will be most useful for future `jwc` context ingestion is still an experiment, not a product contract.
...
- Some packages contain internal or hidden utility prompts/agents beyond the five public role agents. Public-facing docs should keep the four-workflow/five-role contract clear.
...
6. Before changing `packages/coding-agent/src/defaults/jwc/skills/`, `packages/coding-agent/src/prompts/agents/`, `packages/coding-agent/src/commands/`, or config/settings paths, write down whether the change alters public surface area.
...
- Treat the fixed four-workflow/five-callable-role shape as a product constraint.
