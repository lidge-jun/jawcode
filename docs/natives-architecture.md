...
`JWC_NATIVE_VARIANT` can force `modern` or `baseline`; invalid values are ignored.
...
2. Check legacy compiled-binary location:
   - Windows: `%LOCALAPPDATA%/jwc` (fallback `%USERPROFILE%/AppData/Local/jwc`)
   - non-Windows: `~/.local/bin`
3. Fall back to packaged `native/` and executable directory candidates.

`getNativesDir()` uses `$XDG_DATA_HOME/jwc/natives` when `$XDG_DATA_HOME/jwc` exists; otherwise it uses `~/.jwc/natives`.
