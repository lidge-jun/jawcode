...
Status events the prelude emits (e.g. `_emit_status("find", count=…)`) ship inside display bundles under `application/x-jwc-status` so the existing TUI status renderer keeps working.
...
`eval.py` / `eval.js` (both default `true`) plus optional `JWC_PY` override controls eval backend exposure:

- Python backend only (`eval.py=true`, `eval.js=false`)
- JavaScript backend only (`eval.py=false`, `eval.js=true`)
- both backends

`JWC_PY` accepted values:
...
- `application/x-jwc-status` MIME inside `display` → structured status events
...
- `application/x-jwc-status` → status events
...
- **Python backend not available** — Check `eval.py`, `JWC_PY`, and that `python`/`python3` is on PATH. If preflight fails and `eval.js` is enabled, omit `language` or pass `language: "js"` to use JavaScript.
- **No Python on PATH** — Install a system Python 3.8+ or place a venv at `~/.jwc/python-env`. `jwc setup python --check` reports the resolved interpreter.
...
- `JWC_PY` — tool exposure override
- `JWC_PYTHON_SKIP_CHECK=1` — bypass Python preflight/warm checks
- `JWC_PYTHON_INTEGRATION=1` — enable gated integration tests that spawn a real Python
- `JWC_PYTHON_IPC_TRACE=1` — log NDJSON frames exchanged with the runner subprocess
