...
This document maps the main parts of the Jawcode repository. The root README stays intentionally small; this file is the architecture-oriented companion.
...
Main JWC CLI and product runtime.

- `packages/jwc/package.json` exposes package `jawcode`, bin `jwc`, and export `jawcode/sdk`.
- `packages/coding-agent/src/cli.ts` is the executable bootstrap behind the JWC shell. It registers CLI commands such as `setup`, `interview`, `orchestrate`, `goal`, `team`, and the default launch path. Legacy planning/goal engine commands may remain for compatibility but are not the public workflow names.
...
- `packages/coding-agent/src/defaults/jwc-defaults.ts` embeds and installs the default workflow definitions: `jaw-interview`, `plan`, `team`, and `goal`.
- `packages/coding-agent/src/task/agents.ts` embeds bundled task-agent prompts and exposes callable roles. The public contract is `executor`, `executor_ext`, `architect`, `planner`, and `critic`; `executor_ext` reuses executor behavior as the external/fresh model-configurable lane, and other bundled prompts are internal/runtime utilities.
...
- `packages/stats/src/index.ts` exposes the `jwc-stats` CLI entrypoint and exports aggregation/server APIs.
...
Typed Python client for `jwc --mode rpc`.

- `python/jwc-rpc/pyproject.toml` packages `jwc-rpc` for Python 3.11+.
- `python/jwc-rpc/README.md` documents the process-backed stdio client, typed command methods, startup flags, event listeners, todo seeding, host-owned tools, and host-owned URI schemes.

### `python/robogjc/`

Self-hosted GitHub triage/fix bot that drives `jwc --mode rpc`.

- `python/robogjc/AGENTS.md` is the authoritative local contract for this subtree.
- `python/robogjc/pyproject.toml` packages `robogjc` for Python 3.11+ with FastAPI, httpx, pydantic settings, Click, and `jwc-rpc`.
- `python/robogjc/README.md` documents the webhook-to-worktree-to-jwc flow, GitHub sidecar trust boundary, persistent per-issue sessions, and audit trail.
