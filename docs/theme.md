...
1. built-in embedded themes (`abyss-bite.json` and `abyss-bite-light.json` compiled into `defaultThemes`)
...
- overridden by `JWC_CODING_AGENT_DIR` (`$JWC_CODING_AGENT_DIR/themes`)
...
Built-in theme note: `abyss-bite` is the default dark JWC theme, and `abyss-bite-light` is the default light-slot theme. Both use the Abyss Bite design language (dark blue/cyan) with separate semantic error/warning/diff-removal tokens.

Current defaults from settings schema:

- `theme.dark = "abyss-bite"`
- `theme.light = "abyss-bite-light"`
...
Legacy migration exists: old flat `theme: "name"` is migrated to nested `theme.dark` or `theme.light` based on luminance detection; legacy built-in names `dark`/`light` map to `abyss-bite`/`abyss-bite-light` unless matching custom theme files exist.
...
4. Select the theme in Settings (`Display -> Dark theme` or `Display -> Light theme`) depending on which auto slot you want. For bundled themes, choose `abyss-bite` or `abyss-bite-light`.
