...
- Related: [`porting-to-natives.md`](./porting-to-natives.md), [`natives-architecture.md`](./natives-architecture.md), [`natives-binding-contract.md`](./natives-binding-contract.md), [`cpu-hotspot-map.json`](./cpu-hotspot-map.json)
...
The suites already produced concrete decisions that this policy codifies so they are not rediscovered:

- v2 (#530) measured and rejected five remaining Rust port candidates per the FFI cost gates after shipping only `diffLines` (H03) natively. Native overhead did not beat the JS/TS baseline for those candidates on realistic inputs.
...
- Hunt-Szymanski LCS (H05) was implemented as a native/algorithmic replacement, then reverted because it produced byte-different rendered diffs. Byte parity is the gate, not raw speed.
...
These precedents share a root cause: a plausible algorithmic/native win that failed a real gate (cost, byte parity, or end-to-end benefit). The policy makes those gates a precondition rather than a post-hoc discovery.
...
- The profiling corpus threshold/evidence ledger is `packages/coding-agent/bench/perf-threshold.ledger.ts`; native-port proposals must cite concrete corpus artifacts from that ledger once thresholds are promoted.
