# LSP configuration in JWC

This guide explains how to configure language servers for the JWC coding agent.
...
When no LSP config file is present, JWC auto-detects servers by intersecting two conditions:
...
JWC merges LSP config from multiple files, lowest to highest priority:
...
> **Note:** The presence of any LSP config file disables auto-detection. When at least one file is found, JWC skips the binary-scan phase and loads all servers that have matching `rootMarkers`, an available binary, and are not explicitly `disabled`.
...
The `capabilities` object enables optional server-specific features that JWC supports on a per-server basis:
