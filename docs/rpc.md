...
```bash
jwc --mode rpc [regular CLI options]
```
...
```json
{
  ...
  "context": { "method": "confirm", "skill": "plan", "phase": "approval" }
}
```
...
- Automatic session title generation is disabled in RPC mode, and `setTitle` UI
  ...
  meaningful terminal-title surface. Set `JWC_RPC_EMIT_TITLE=1` to opt back in to
  the UI event only.
...
(jaw-interview questions, plan approval, goal execution sign-off)
...
- `stage` is one of `jaw-interview` | `plan` | `goal`. `team` is reserved
  and rejected for v1 (single-agent only). Legacy stage names may still be
  normalized on read, but public hosts should emit and document the JWC names.
...
```json
{ "type": "workflow_gate_response", "gate_id": "wg_4845_plan_000001", "answer": "approve", "idempotency_key": "k1" }
```
...
`python/jwc-rpc` (Python):

```python
from jwc_rpc import RpcClient, WorkflowGate

client = RpcClient(executable="jwc")
