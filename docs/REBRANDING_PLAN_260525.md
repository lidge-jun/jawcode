...
Historical plan for the earlier gajae-code/GJC rebrand and visible UI redesign. It is preserved as lineage context; current public docs should use Jawcode/JWC unless they are explicitly describing legacy compatibility.
...
6. **Visible workflow minimization** — Superseded for Jawcode: the current public workflow surface is `jaw-interview`, `plan`/`jwc orchestrate`, `goal`/`jwc goal`, and `team`.
