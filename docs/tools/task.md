...
- Key collaborators:
  ...
  - `packages/coding-agent/src/task/discovery.ts` — resolve bundled JWC agents.
...
6. It resolves the requested agent with `getAgent(...)`, rejects unknown or disabled agents, and enforces parent spawn policy plus `JWC_BLOCKED_AGENT` self-recursion prevention.
...
- Isolation merge strategy
  ...
  - Branch mode — commit each task onto `jwc/task/<id>` branch, cherry-pick into parent, preserve failed branches for manual resolution.
- Agent source
  - Bundled visible callable task roles — `executor`, `executor_ext`, `planner`, `architect`, `critic`.
  - Hidden support agents — retained for internal workflows from `packages/coding-agent/src/task/agents.ts`.
