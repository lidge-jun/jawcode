...
- The branch push metadata is persisted with `git config` under the repository's shared `.git/config` as:
  ...
  - `branch.pr-<number>.jwcPrHeadRef`
  - `branch.pr-<number>.jwcPrUrl`
  - `branch.pr-<number>.jwcPrIsCrossRepository`
  - `branch.pr-<number>.jwcPrMaintainerCanModify`
...
Push target resolution reads the `branch.<name>.jwcPrHeadRef`, `pushRemote`/`remote`, `jwcPrUrl`, `jwcPrMaintainerCanModify`, and `jwcPrIsCrossRepository` git-config keys written by `pr_checkout`. If the current checked-out branch matches the target branch, the source ref is `HEAD`; otherwise it pushes `refs/heads/<branch>`. The refspec is `HEAD:refs/heads/<headRef>` or `refs/heads/<branch>:refs/heads/<headRef>`.
...
- Local validation errors throw `ToolError`, including:
  ...
  - `pr_push` on a branch without `jwcPrHeadRef` metadata
