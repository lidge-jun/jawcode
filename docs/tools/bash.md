...
| `pty` | `boolean` | No | Request PTY mode. Default `false`. PTY is used only when `pty: true`, `JWC_NO_PTY !== "1"`, and the tool context has a UI. |
...
2. Foreground PTY
   - Requires `pty: true`, UI context, and `JWC_NO_PTY !== "1"`.
...
- PTY mode is ignored in non-UI contexts and when `JWC_NO_PTY=1`; the tool silently falls back to non-PTY execution.
