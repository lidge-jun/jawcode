...
3. `getSSHConfigPath("project")` and `getSSHConfigPath("user")` in `packages/utils/src/dirs.ts` resolve those managed files to `.jwc/ssh.json` in the project and `~/.jwc/agent/ssh.json` in the user config dir. This tool does not read `~/.ssh/config`.
