...
> **스냅샷 (2026-06-14)**: `resolve-heads.ts` + 재생성 스크립트로 worktree/클론 HEAD 자동 반영. **99.03** discovery·stage-context ✅ ([structure/50_status.md](../structure/50_status.md)).
...
|---|---|---|
| **gjc_origin** | [`devlog/_upstream_gjc/`](../devlog/_upstream_gjc/) — Yeachan-Heo/gajae-code 클론 (gitignored), branch `dev` | `269387babcbf02e33613032bc8e41ea395aa10ec` (재생성 시 `git -C devlog/_upstream_gjc rev-parse HEAD`) |
| **jwc_patched** | jawcode worktree (포크 패치 반영; dirty local edits respected) | `d60b78223d5d5f5b3f82b3d0ccfe95620f754eb5` (재생성 시 `git rev-parse HEAD`) |
| **omp_origin** | [`devlog/_upstream_omp/`](../devlog/_upstream_omp/) | `7e8122000` (재생성 시 클론 HEAD) |
