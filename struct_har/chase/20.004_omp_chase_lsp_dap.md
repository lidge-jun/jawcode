...
|------|-----------------|
| Decision A — source behavior classification | **split / pending** — held this pass (hold_lsp) |
| Decision B — JWC naming impact | none |
| Decision C — test impact | manual evidence only (when resumed) |
| Decision D — docs impact | chase only |
| Decision E — rollout impact | no migration — 현재 구현 없음 |
| Decision F — residual risk | **deferred (held)** — LSP/DAP 선택-포팅 후보 검토 미완 |
| Decision G — reviewer needed | architecture (if productized) |
| Decision H — bundle policy | single-card; stays active |

Held (hold_lsp): kept active for a later candidate-selection review; not retired this pass.

## Jawdev chase expansion — 2026-06-26

> Document: `struct_har/chase/20.004_omp_chase_lsp_dap.md`
> Title: 20.004 — omp: LSP · DAP
> Lane: OMP
