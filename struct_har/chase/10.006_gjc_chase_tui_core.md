...
|------|-----------------|
| Decision A — source behavior classification | **defer** — upstream tui 버그픽스는 082/083 분리 후에만; abyss-bite·99.03 HUD 금지 |
| Decision B — JWC naming impact | none |
| Decision C — test impact | focused test (unblock 시) — `diff -qr packages/tui` ≤10줄 후 _fin |
| Decision D — docs impact | chase only |
| Decision E — rollout impact | no migration |
| Decision F — residual risk | **deferred** — 082/083 게이트 이후로 보류 |
| Decision G — reviewer needed | frontend (unblock 시) |
| Decision H — bundle policy | single-card; stays active until 082/083 split |

Deferred (gated): tui bugfix work blocked on 082/083 separation; direction recorded, card kept active. Not retired this pass.

## Jawdev chase expansion — 2026-06-26

> Document: `struct_har/chase/10.006_gjc_chase_tui_core.md`
> Title: 10.006 — gjc chase: packages/tui
> Lane: GJC
...
1. Start from this document, then open the current source lane at `devlog/_gjc_chase/gajae-code` and the matching JWC files under packages/tui/.
