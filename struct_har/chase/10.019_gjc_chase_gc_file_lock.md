...
| harness `gc-adapter` | **⬜** — compare after [10.011](./_fin/10/10.011_gjc_chase_receipt_spool.md) |
...
- Scope: port **lock GC + harness adapter** first; team/tmux GC only if same tmux contract as [10.007](./_fin/10/10.007_gjc_chase_team_profile_self_heal.md).

## 완료 기준

- [ ] `jwc gc` (or documented subset) runs without mutating user sessions unexpectedly.
- [ ] file-lock GC TOCTOU regression covered.
- [ ] Harness gc-adapter wired if receipt spool ([10.011](./_fin/10/10.011_gjc_chase_receipt_spool.md)) landed.
...
**Remaining (deferred, keeps card OPEN):** team-workers / tmux-sessions / harness-leases / registry-entries GC adapters (10.011 의존).
