...
|---|---|---|---|
| `050aa1731551` dev: team `@gjc-profile` self-heal | team/tmux ownership guard로 분리 | [10.007](./_fin/10/10.007_gjc_chase_team_profile_self_heal.md) | 2026-06-13 |
| `050aa1731551` dev: RPC stdio lifecycle + `get_state` payload | headless/RPC gap으로 분리 | [10.008](./_fin/10/10.008_gjc_chase_rpc_lifecycle.md) | 2026-06-13 |
| `050aa1731551` dev: pre-send estimated context maintenance | 기존 session/compaction card 상세화 | [10.004](./_fin/10/10.004_gjc_chase_session_compaction.md) | 2026-06-13 |
...
| `75d103f` dev: harness receipt JSONL spool exporter (#554) | 등재 — owner/storage clean, harness.ts 배관 선별 (구현가치 60); 10.008 묶음 권장 | [10.011](./_fin/10/10.011_gjc_chase_receipt_spool.md) | 2026-06-13 (3차) |
...
| `a791d72a..fa995807` v0.7.4→0.7.7 (55 commits) | 클러스터 split | 10.054–058 ([10.053](./10.053_gjc_chase_local_provider_terminal_bell_windows.md) 우산) | 2026-06-29 |
| `fa995807..20c299eb` v0.7.7→**0.7.8** (21 commits) | 테마별 7카드 + chore/docs 3 no-card | 10.059–065 | 2026-07-01 |

### v0.7.8 델타 (21 commits, `fa995807..20c299eb`) — 카드 귀속

> 리뷰 head: `20c299eb` (`upstream/dev`, v0.7.8). JWC worktree `cfe71cd` (2026-07-01).
> 21커밋 전부 카드 또는 chore/docs no-card 행에 귀속 (미할당 0).

| card | commits | theme |
|---|---|---|
| [10.059](./_fin/10/10.059_gjc_chase_deep_interview_ask_ralplan_gate.md) ✅ _fin 260701 | `20c299eb`, `bb6f0e98`, `eef6ec3a`, `f1343220`, `2bc0e2c5`, `19408acc` | deep-interview wording/spec, ralplan ask gate, ultragoal session-scoped guard, render undefined guard (ADAPT+IMPORT 4 slices; ultragoal-scope + render-middleware DEFER③, noisy-settings already-covered) |
| [10.060](./_fin/10/10.060_gjc_chase_tui_render_resilience_editor_submit.md) ✅ _fin 260701 | `eb346860`, `0455d408`, `0e537348`, `8bf665af` | per-component render isolation + Ctrl/Ctrl+Shift Enter submit imported/adapted; usage display mode adapted; custom-editor UX deferred |
| [10.061](./_fin/10/10.061_gjc_chase_tmux_team_windows_psmux_titles.md) ✅ _fin 260701 | `beec1af0`, `1d65050d` | tmux workspace titles + Windows/psmux team spawn reliability (#1303 follow-up to #1282) |
| [10.062](./10.062_gjc_chase_ai_provider_deepinfra_gemini_ua.md) | `68179e2d`, `b200fc0e` | DeepInfra provider + service-tier; Gemini CLI user-agent alignment + spoofed-version guard |
| [10.063](./10.063_gjc_chase_natives_platform_split_packages.md) | `eb8a76f6` | split `@jawcode-dev/natives` into per-platform packages |
| [10.064](./_fin/10/10.064_gjc_chase_telegram_daemon_entrypoint_notify.md) ✅ _fin 260701 | `d00cbe2c`, `308d4a20` | compiled Telegram daemon entrypoint fix; Windows Terminal bell workaround doc + setting — ADAPT hidden daemon adapter + docs/settings description |
| [10.065](./_fin/10/10.065_gjc_chase_prompt_self_awareness_grounding.md) ✅ _fin 260701 | `aa7322f6` | system prompt self-awareness source grounding — ADAPT, JWC-authored content (identity-safe) |
| — no JWC card (chore/docs) | `af1e9c5d` (version bump 0.7.8), `b948e377` (maintainer contributing guide #1312), `ebacf8d0` (Discord invite refresh #1301) | release/docs hygiene, no behavior gap |

> OMP 축: 클론이 849커밋 앞섰으나 본 사이클 범위 밖 — 카드 미발급, 보류 노트만 ([004_reference_from_omp](./004_reference_from_omp.md) 참조). 추후 별도 사이클에서 triage.

### ① tier 구현 루프 (goal `f8909338-255`, 2026-07-01~)

> ② 결정 인터뷰(theme7)에서 ①로 격하된 15 카드를 **실제 JWC 코드**로 구현→검증→_fin. 슬라이스 맵: `devlog/_plan/260701_chase_tier1_impl/00_INDEX_slice_map.md`.

| card | 구현 상태 | evidence |
|---|---|---|
| [10.054](./_fin/10/10.054_gjc_chase_local_provider_discovery.md) local provider | ✅ _fin 260701 | `8b3b861`/`f8838a4` (CLI/discovery) + `afd7038` (role resolution + unavailable fallback) |
| [10.062](./_fin/10/10.062_gjc_chase_ai_provider_deepinfra_gemini_ua.md) DeepInfra + Gemini UA | ✅ _fin 260701 | `b249348` (provider+service-tier+UA) + `ad6ec8f`/`27311f6` (tokenizer routing review fix) |
| [10.048](./_fin/10/10.048_gjc_chase_dev_ci_release_packaging.md) dev/CI/release packaging | ✅ _fin 260701 | `8b3ab60` affected-path selector false-green guard: `Task.cwd` execution + selector self-test; matrix fanout/release credential changes deferred |
| [10.060](./_fin/10/10.060_gjc_chase_tui_render_resilience_editor_submit.md) TUI render/editor/status-line | ✅ _fin 260701 | render isolation + Ctrl/Ctrl+Shift Enter submit + usage mode tests: 122 focused pass; custom editor UX deferred |
| [10.041](./_fin/10/10.041_gjc_chase_tui_input_render_windows_psmux.md) TUI input/render Windows psmux | ✅ _fin 260701 | `38ac6dcd` adapted: main editor autocomplete provider now reaches HookSelector inline/docked/list-slot custom inputs; 48 focused pass + `check:ts` + verifier PASS; broad tmux/session lifecycle split remains 10.050 |
| [10.057](./_fin/10/10.057_gjc_chase_windows_hardening.md) Windows hardening | ✅ _fin 260701 | npm shim detection + package-manager verification + backup cleanup tolerance; Windows-safe team worker command; worktree/coordinator real error preservation; 106 focused pass + `check:ts`; v0.7.8 psmux/titles closed in 10.061 |
| [10.061](./_fin/10/10.061_gjc_chase_tmux_team_windows_psmux_titles.md) tmux/team Windows psmux titles | ✅ _fin 260701 | ADAPT tmux `JWC-`/`JWC:` titles, native Windows psmux `--tmux` planning, psmux bare targets, prompt normalization, literal `send-keys` dispatch; focused 5+18+9+4 pass + `check:ts` + Backend verifier PASS |
| [10.056](./_fin/10/10.056_gjc_chase_terminal_bell_notifications.md) terminal bell + completion hook | ✅ _fin 260701 | ADAPT opt-in terminal BEL settings, guarded bell utility, global-only `completion.notifyCommand`, `JWC_NOTIFICATION_*` env, foreground hook/background-only desktop toast; focused 5+11 pass + schema check + `check:ts` |
| [10.064](./_fin/10/10.064_gjc_chase_telegram_daemon_entrypoint_notify.md) Telegram daemon entrypoint + Windows bell | ✅ _fin 260701 | ADAPT hidden `jwc notify daemon-internal` smokeable adapter, compile entrypoint wiring, owner PID liveness guard, JWC config token/chat mapping, Windows Terminal bell workaround docs; focused 3+6+6 pass + schema check + `check:ts` |
| [10.052](./_fin/10/10.052_gjc_chase_docs_external_integrations.md) docs external integrations | ✅ _fin 260701 | ADAPT docs-only external integration matrix, bridge/RPC `@jawcode-dev/bridge-client` namespace correction, generated docs index, stale shipped-claim regression guard |
