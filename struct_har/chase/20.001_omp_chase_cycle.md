...
## Changelog 메모 (reference-only)

| upstream 범위 | jaw 판정 | NNN | 날짜 |
|---|---|---|---|
| `cc0c67be..0fc6d136` v16.1.13→16.1.20 (20.015 release/test leak hardening) | release/test leak hardening **track-only reference closure** — no standalone JWC product code remains for this OMP card because release/CI/package implementation ownership closed in `10.048`; existing release guards mapped and deferred buckets documented. → [20.015](./_fin/20/20.015_omp_chase_release_test_leak_hardening.md) ✅ _fin | 20.015 | 2026-07-01 |
| `ca9f2847e..b6c9747d4` v16.2.5→16.2.9 (20.028 web-search cluster) | web-search provider settings **IMPORT** — DDG HTML scrape / explicit `--provider auto` / result cap already aligned; JWC-native 2 slices adopted: native-browser-aligned DDG request headers + blank `b` form param, and standalone CLI no-flag `providers.webSearch` application. Gemini api-key search DEFER③ pending credential/security pass. commits `c874eab` + `5790c88`, 28 pass/1 skip/0 fail, `bun run check:ts` exit 0. → [20.028](./_fin/20/20.028_omp_chase_web_search_provider_settings.md) ✅ _fin | 20.028 | 2026-07-01 |
| `ca9f2847e..b6c9747d4` v16.2.5→16.2.9 (20.025 cluster) | compaction/snapcompact caps + session-scope **IMPORT** — JWC-native 1 slice: `pruneOversizedEditSnapshots`/`capPerFileSnapshots` bound edit-tool oldText/newText (32KB cap, shared per-file aggregate) at 4 persisted apply sites. `9e4e0f669`+`398796949` IMPORT; snapcompact-frame/session-loader-elision/branch-scope (7 commits) DEFER③ (no JWC surface); mid-run key-identity covered by 20.009. commit `314b7a1`, 12 pass/0 fail, tsgo exit 0. → [20.025](./_fin/20/20.025_omp_chase_compaction_snapcompact_session_scope.md) ✅ _fin | 20.025 | 2026-07-01 |
| `ca9f2847e..b6c9747d4` v16.2.5→**16.2.9** (175 commits) | 테마별 8 reference 카드 + 70 no-card (38 merge·12 style·10 chore·8 test·1 revert·1 keep). **1:1 port ❌ — design reference only.** | 20.023–030 | 2026-07-01 |
| `0fc6d136..ca9f2847e` v16.1.20→16.2.5 (split←016 cluster) | v2 streaming integrity **IMPORT** — JWC-native 1 slice: `scrubPartialJson` clears streaming partialJson from finalized proxy messages at catch/done/error terminal paths (consumer mid-stream contract preserved). cfa0cd84b IMPORT; Map/symbol/compaction (357c29224·9c795f886·08e1ea727·102d6d54a) DEFER③. commit `e84dc5d`, 5 pass/0 fail, tsgo exit 0. → [20.021](./_fin/20/20.021_omp_chase_v2_streaming_integrity.md) ✅ _fin | 20.021 | 2026-07-01 |
| `0fc6d136..ca9f2847e` v16.1.20→16.2.5 (split←016 cluster) | session title + LLM idle recap **ADAPT** — JWC-native 1 slice: `reconcileTitleCasing` reconciles small-title model casing against user message (verbatim/distinctive/camel-flatten/preserve). `52b8fb156` ADAPT; title-slot storage (`0ce330ab7`·`222b036a1`·`9c1b55dbe`) + LLM idle recap (`96a1aed19`) + config schema (`a36f12be8`) DEFER③ (no JWC storage/recap surface). commit `89f4105`, 8 pass/0 fail, tsgo exit 0. → [20.020](./_fin/20/20.020_omp_chase_session_title_idle_recap.md) ✅ _fin | 20.020 | 2026-07-01 |
| `cc0c67be..0fc6d136` v16.1.13→16.1.20 (8 commits) | append-only context integrity **IMPORT** — JWC-native 3 slices: stable-prefix preservation+truncate, tool-result/providerPayload digest 확장, malformed empty-name tool-call sanitizer. 2 atomic commits `04421e3`+`b7a6705`, 96 pass/0 fail, tsgo×2 exit 0. → [20.009](./_fin/20/20.009_omp_chase_append_only_context_integrity.md) ✅ _fin | 20.009 | 2026-07-01 |

### v16.2.9 델타 (175 commits, `ca9f2847e..b6c9747d4`) — 카드 귀속

> 리뷰 head: `b6c9747d4` (`origin/main`, v16.2.9). JWC worktree `cfe71cd` (2026-07-01).
> 175커밋 = 105 card-bound (79 fix · 23 feat · 2 refactor · 1 perf) + 70 no-card. 미할당 0. 카드는 클러스터 대표 commit만 인용(전수 나열 아님).

| card | 대표 commits | theme |
|---|---|---|
| [20.023](./_fin/20/20.023_omp_chase_ai_providers_catalog_service_tier.md) ✅ _fin 260701 | `d20e6c082`, `dca8a7afb`, `f1063cdfb`, `bc7e5b31a`, `6fb9bbfc1`, `cd6ba0214` | AI providers/model catalog 정비 + per-family service-tier 해석 (reference-triage: no JWC code, leaked-fence-heal defer→20.018) |
| [20.019](./_fin/20/20.019_omp_chase_codex_ai_config.md) ✅ _fin 260701 | `f46d03093`, `2863a9c4e`, `2708c1edb`, `f018c5ce8`, `f0f7a5ba8` | codex/AI config ADAPT: codex usage base-URL host-boundary parsing 수정 + OpenAI Responses textVerbosity official-endpoint plumbing (2 JWC slices); all_turns inapplicable, default-verbosity/tiny-role defer→③ |
| [20.024](./20.024_omp_chase_mcp_oauth_reauth_flow.md) | `3106a15f7`, `45df90bda`, `4ce9d659c`, `a49c7027b`, `e29792afe` | MCP oauth/reauth 로그인 플로우 + cmd-shim launch |
| [20.025](./_fin/20/20.025_omp_chase_compaction_snapcompact_session_scope.md) ✅ _fin 260701 | `9e4e0f669`, `398796949`, `156dfd846`, `5b026d304`, `cc2cf5c7d` | compaction/snapcompact caps + session-branch scoping (IMPORT: bounded edit snapshots; snapcompact/session-loader DEFER③) |
| [20.026](./20.026_omp_chase_tui_input_loader_mcp_enable.md) | `16ef3c54f`, `5cc84db4c`, `64734021a`, `7871591b4`, `c2d6c8cf1` | TUI loader re-arm·double-Esc + MCP enable sources |
| [20.027](./_fin/20/20.027_omp_chase_prompts_subagent_discovery_rules.md) | `213fdca79`, `51ce5a87f`, `6b7d7e6e7`, `9ccd83a13`, `dcc7a1ce2` | ✅ _fin · system prompt·tester/sonic subagent·discovery rules (identity-sensitive) |
| [20.028](./_fin/20/20.028_omp_chase_web_search_provider_settings.md) ✅ _fin 260701 | `09061ed80`, `3560d108d`, `530113eb7`, `755a61de0`, `ba6b64bf8` | web-search DDG browser-aligned request + CLI provider settings (gemini api-key defer③) |
| [20.029](./20.029_omp_chase_stats_sync_worker_perf.md) | `234c1919f`, `467f46b19`, `7d5544493`, `b48d488f2`, `efb96b123` | stats sync worker hardening + perf index |
| [20.030](./20.030_omp_chase_misc_dictation_binary_font_yield_irc_win.md) | `0ef430a39`, `168bdae5d`, `242705c06`, `2c8b72308`, `4810f47db`, `e8090bb48` | dictation·binary-detect·CJK font·yield·irc·windows paths 횡단 |
| — no JWC card | merge(38)·style(12)·chore(10)·test(8) + revert `d1e412eef` + keep `6f8f76be4` + version bumps `0ba736f5b`(16.2.6)·`38250ce88`(16.2.7)·`5bc68f57c`(16.2.8)·`b6c9747d4`(16.2.9) | release/hygiene, behavior gap 없음 |

> 전부 reference-only: JWC 코드/테스트 무변경, import 언어 금지(설계 참조만). OMP 패키지(`packages/catalog`·`agent`·`snapcompact`) → JWC `packages/coding-agent` 매핑은 [008](./008_gjc_jwc_naming_contract.md).

## Jawdev chase expansion — 2026-06-26

> Document: `struct_har/chase/20.001_omp_chase_cycle.md`
> Title: 20.001 — omp chase 운영 사이클
> Lane: OMP
