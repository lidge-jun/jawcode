...
> Document: `struct_har/chase/_fin/10/10.011_gjc_chase_receipt_spool.md`
> Title: 10.011 — gjc chase: harness receipt JSONL spool exporter
> Lane: GJC
> Status: completed guardrail
> Canonical source: `devlog/_gjc_chase/gajae-code` (dev tracking upstream/dev)
> Primary patch surfaces: packages/coding-agent/, python/jwc-rpc/
...
1. Start from this document, then open the current source lane at `devlog/_gjc_chase/gajae-code` and the matching JWC files under packages/coding-agent/, python/jwc-rpc/.
...
9. If the correct patch surface is outside packages/coding-agent/, python/jwc-rpc/, record why the owner changed in the devlog before widening scope.
...
3. JWC owner files listed before edit: packages/coding-agent/, python/jwc-rpc/.
