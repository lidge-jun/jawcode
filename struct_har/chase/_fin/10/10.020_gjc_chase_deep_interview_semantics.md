...
> **따라갈 순서**: jaw-interview PABCD 별도 — [02 Tier 1 #7](../../../../devlog/_plan/260614_chase_upstream_pull_priority_report/02_debt_priority_stack.md)
...
|------|-----------------|
| Decision A — source behavior classification | **split** — deep-interview SKILL/ralplan 체리픽 거부; scoring/recorder 패턴만 jaw-interview로 translate (별도) |
| Decision B — JWC naming impact | state path — `jaw-interview` slug·`.jwc/specs/`·mutation guard 불변 |
| Decision C — test impact | manual evidence only (this pass) — adopt/reject 목록 |
| Decision D — docs impact | chase only — 패턴 채택은 별도 jaw-interview devlog 소유 |
| Decision E — rollout impact | no migration |
| Decision F — residual risk | monitored — 패턴 이식은 jaw-interview PABCD로 분리 |
| Decision G — reviewer needed | backend/architecture (패턴 이식 시) |
| Decision H — bundle policy | split — reference 확정 now, pattern adapt는 별도 카드 |

Reference-only: reject wholesale deep-interview SKILL/ralplan; scoring/recorder patterns may translate into jaw-interview later via its own devlog. No code this pass. Disposition → `_fin/10/`.

## Jawdev chase expansion — 2026-06-26

> Document: `struct_har/chase/10.020_gjc_chase_deep_interview_semantics.md`
> Title: 10.020 — gjc chase: deep-interview semantics (reference-only)
> Lane: GJC
