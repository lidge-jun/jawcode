...
> Document: `struct_har/chase/_fin/10/10.026_gjc_chase_rpc_issues_audit.md`
> Title: 10.026 — gjc chase: RPC/unattended issues audit pack (design input)
> Lane: GJC
> Status: completed guardrail
> Canonical source: `devlog/_gjc_chase/gajae-code` (dev tracking upstream/dev)
> Primary patch surfaces: packages/tui/, python/jwc-rpc/, packages/coding-agent/
...
1. Start from this document, then open the current source lane at `devlog/_gjc_chase/gajae-code` and the matching JWC files under packages/tui/, python/jwc-rpc/, packages/coding-agent/.
...
9. If the correct patch surface is outside packages/tui/, python/jwc-rpc/, packages/coding-agent/, record why the owner changed in the devlog before widening scope.
...
3. JWC owner files listed before edit: packages/tui/, python/jwc-rpc/, packages/coding-agent/.
