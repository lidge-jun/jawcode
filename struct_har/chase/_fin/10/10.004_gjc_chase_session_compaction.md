...
> Document: `struct_har/chase/_fin/10/10.004_gjc_chase_session_compaction.md`
> Title: 10.004 — gjc chase: 세션 · compaction · harness
> Lane: GJC+OMP
...
> Primary patch surfaces: packages/coding-agent/, packages/agent/, python/jwc-rpc/
...
1. Start from this document, then open the current source lane at `devlog/_gjc_chase/gajae-code + devlog/_omp_chase/oh-my-pi` and the matching JWC files under packages/coding-agent/, packages/agent/, python/jwc-rpc/.
...
9. If the correct patch surface is outside packages/coding-agent/, packages/agent/, python/jwc-rpc/, record why the owner changed in the devlog before widening scope.
...
3. JWC owner files listed before edit: packages/coding-agent/, packages/agent/, python/jwc-rpc/.
