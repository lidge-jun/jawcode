...
> Reviewed source: GJC `269387b` (`5bcf585c` #610) vs JWC `cdee4f7` (live-surface proof).
> **구현가치: 55 (MLB)** — `goal` completion gate / QA matrix와 직결.
> **동레인**: [10.012 goal steering](./10.012_gjc_chase_goal_steering.md) — 2026-06-26 interview decision fixes this as **after 10.012 hardening** (now ✅).
> **Decision A=split (closed 2026-06-27)**: live-surface inline-evidence rejection adapted here; deep byte/structural/replay engine split to [10.027](../../10.027_gjc_chase_goal_live_artifact_engine.md).
...
- [x] Review/red-team path documented in goal SKILL / runtime. — `src/defaults/jwc/skills/goal/SKILL.md:176-200` (review gate + `record-review-blockers`); live-surface artifact-file rule appended (`SKILL.md:193`).
- [x] Tests: shallow QA evidence rejected (parity with upstream intent). — live-surface inline-only evidence now rejected (`requireLiveSurfaceProof`, `goal-engine.ts`; commit `cdee4f7`). `ultragoal-runtime.test.ts` 85/0 incl. **non-vacuous** negative test (source reverted 0/1 → restored 1/1). jwc already had ~15 shallow-gate rejections via the executorQa matrix schema.
- [x] [10.012](./10.012_gjc_chase_goal_steering.md) dispatcher compatible with review transitions. — `review_blocker` kind / `review_blocked` status already steered; this change touches the completion gate only, not dispatcher transitions (steering tests stay green).

> **Split residual** → [10.027](../../10.027_gjc_chase_goal_live_artifact_engine.md): deep content verification (screenshot byte/non-uniform decode, automation transcript, PTY, CLI replay execution [security-sensitive], standalone `review` subcommand, contract-strength grading, computer mandatory cases). jwc verifies live-surface artifact **presence**; content verification is the deferred engine.
...
| Decision B — JWC naming impact | Preserve `goal`/`jwc` runtime naming and map upstream review/red-team terms onto JWC qualityGate/executorQa contracts. |
| Decision C — test impact | Done gate starts after 10.012 hardening and must reject shallow QA evidence with focused goal runtime tests. |
...
| Decision E — rollout impact | Dependent rollout: this card does not precede 10.012 steering hardening. |
| Decision F — residual risk | Intentionally deferred/split until the steering dispatcher and runtime surface are stable. |
| Decision G — reviewer needed | Backend/architecture review because completion gates and red-team review alter goal lifecycle behavior. |
| Decision H — bundle policy | Separate implementation phase after 10.012; do not bundle with first steering patch. |

Current action for the interview pass: record confirmed ordering and keep later contradictions for follow-up interview after chase-doc hardening.

## Jawdev chase expansion — 2026-06-26

> Document: `struct_har/chase/10.021_gjc_chase_goal_redteam_review.md`
> Title: 10.021 — gjc chase: ultragoal red-team verification + review mode
> Lane: GJC
