...
|------|-----------------|
| Decision A — source behavior classification | **split** — geobench YAML 복사 거부; threshold ledger 패턴만 |
| Decision B — JWC naming impact | none |
| Decision C — test impact | manual evidence only — 채택 게이트 결정 |
| Decision D — docs impact | chase only (+ optional 99.02 CI ledger) |
| Decision E — rollout impact | no migration |
| Decision F — residual risk | monitored — ledger 패턴은 99.02 옵션 |
| Decision G — reviewer needed | no (docs); data/devops if ledger productized |
| Decision H — bundle policy | split — reference now, ledger→99.02는 옵션 spinoff |

Reference-only: reject geobench YAML copy; threshold-ledger pattern optional for 99.02 CI. Disposition → `_fin/10/`.

## Jawdev chase expansion — 2026-06-26

> Document: `struct_har/chase/10.025_gjc_chase_perf_corpus_geobench.md`
> Title: 10.025 — gjc chase: perf profiling corpus + geobench (reference)
> Lane: GJC
> Status: active chase card
> Canonical source: `devlog/_gjc_chase/gajae-code` (dev tracking upstream/dev)
> Primary patch surfaces: packages/utils/
...
1. Start from this document, then open the current source lane at `devlog/_gjc_chase/gajae-code` and the matching JWC files under packages/utils/.
...
9. If the correct patch surface is outside packages/utils/, record why the owner changed in the devlog before widening scope.
...
3. JWC owner files listed before edit: packages/utils/.
