...
> **따라갈 순서**: [02 priority #18](../../../../devlog/_fin/260614_chase_upstream_pull_priority_report/02_debt_priority_stack.md)
...
| harness `gc-adapter` | **✅** — wired after [10.011](./10.011_gjc_chase_receipt_spool.md) |
...
- [x] `jwc gc` safe subset runs without mutating live user sessions.
- [x] file-lock GC TOCTOU regression covered.
- [x] Harness gc-adapter wired after receipt spool ([10.011](./10.011_gjc_chase_receipt_spool.md)) landed.
...
| Decision B — JWC naming impact | Public command, docs, and state paths must be `jwc`/`.jwc`; upstream `gjc gc` is a source reference only. |
| Decision C — test impact | Done gate requires TOCTOU file-lock coverage plus runtime/command surface evidence. |
...
| Decision E — rollout impact | Multi-step rollout; each slice must prove it does not mutate user sessions unexpectedly. |
| Decision F — residual risk | Intentionally split until file-lock GC, `jwc gc`, harness adapter, and team/tmux GC can be proven independently. |
| Decision G — reviewer needed | Backend/architecture review for every runtime slice. |
| Decision H — bundle policy | Multiple PABCD cycles expected; do not collapse the whole GC surface into one patch. |

Split phases:

1. File-lock stale-owner GC with TOCTOU regression tests.
2. `jwc gc` command/runtime/render surface.
3. Harness GC adapter after receipt-spool readiness is confirmed.
4. Team/tmux GC only after the 10.007 tmux self-heal contract is stable.

## Jawdev chase expansion — 2026-06-26

> Document: `struct_har/chase/10.019_gjc_chase_gc_file_lock.md`
> Title: 10.019 — gjc chase: `gjc gc` + file-lock GC + harness adapter
> Lane: GJC
> Status: active chase card
> Canonical source: `devlog/_gjc_chase/gajae-code` (dev tracking upstream/dev)
> Primary patch surfaces: packages/coding-agent/, packages/utils/, python/jwc-rpc/
...
1. Start from this document, then open the current source lane at `devlog/_gjc_chase/gajae-code` and the matching JWC files under packages/coding-agent/, packages/utils/, python/jwc-rpc/.
...
9. If the correct patch surface is outside packages/coding-agent/, packages/utils/, python/jwc-rpc/, record why the owner changed in the devlog before widening scope.
...
3. JWC owner files listed before edit: packages/coding-agent/, packages/utils/, python/jwc-rpc/.
...
ADAPT — jwc gc(lock/orphan/TOCTOU GC) 운영 명령 채택. .gjc→.jwc 네이밍.

### Phase 1 landed (2026-07-01) — card stays OPEN

> file-lock GC LIVE vertical 만 구현. Phases 2-4 (team/tmux workers, harness leases, registry entries adapters)는 **10.011 orchestrate receipt/harness 레인 의존**으로 DEFER → 카드 OPEN 유지.

**Implemented (commits `6171283`, `7cd9d54`):**
- `packages/coding-agent/src/config/file-lock.ts` — `readFileLockInfoForGc`, `removeFileLockDirForGc`(TOCTOU owner-token guard, GJC #606), `FileLockOwnerToken`/`FileLockGcRemoval`; `isLockStale`를 GJC #652로 보강(live owner는 elapsed-time으로 절대 reap 안 함, dead 즉시 reclaim, unknown만 시간 fallback).
- `packages/coding-agent/src/config/file-lock-gc.ts` (NEW) — `fileLocksGcAdapter`(walk roots→`.lock` discovery→probe 분류→fail-closed prune).
- `packages/coding-agent/src/jwc-runtime/gc-runtime.ts` (NEW) — orchestrator. `GcStore`는 forward-compat 5-member union 유지하되 runtime-mapped Record는 전부 `ActiveGcStore = typeof GC_STORES[number]`(현재 `file_locks`만)로 type-sound. `defaultGcAdapters()`는 top-level static import한 `fileLocksGcAdapter`만 반환(GJC lazy multi-import 미채택, AGENTS.md no-inline-import).
- `packages/coding-agent/src/jwc-runtime/gc-render.ts` (NEW) — `buildGcReportText`, `jwc gc` 브랜딩.
- `packages/coding-agent/src/commands/gc.ts` (NEW) + `cli.ts` baseCommands 등록 — `jwc gc [--prune|--force] [--json] [--dry-run]`, dry-run 기본.
- 테스트: `test/file-lock-gc-toctou.test.ts`(#606 owner-token + #652 live-owner), `test/jwc-runtime/gc-runtime.test.ts`(probe/collect/prune/exit-code/branding/adapter-wiring) — 22 pass.

**Verification:** `bun test`(22 pass 0 fail), `bun run check:types`(tsgo EXIT 0), `git diff --check` clean.

**Audit/Review:** Meitner(gpt-5.4) plan-audit FAIL→fixed(type-soundness+no-inline-import). Hypatia(gpt-5.4) impl-review checks 1-5 PASS, #652 divergence 지적→GJC #652 fix 채택.

**Historical remainder at that phase:** team-workers / tmux-sessions / harness-leases / registry-entries GC adapters (10.011 dependency). The 2026-07-17 closure below resolves all except the safety-gated tmux residual.
...
| slot | final decision |
|---|---|
| Decision A — source behavior classification | **adapt** — wire the stores whose JWC owners can revalidate liveness safely. |
| Decision B — JWC naming impact | Public command and state paths remain `jwc` / `.jwc`; legacy `GJC_*` compatibility variables remain internal. |
| Decision C — test impact | Added focused harness/registry/team GC tests and repaired the file-lock owner-token happy-path fixture. |
| Decision D — docs impact | Chase card and MOC only. |
| Decision E — rollout impact | No migration; dry-run remains the default and prune is fail-closed. |
| Decision F — residual risk | `tmux_sessions` remains intentionally inactive. Safe prune requires terminal runtime markers plus native session/owner-generation identity that JWC does not yet persist; importing path/branch heuristics would risk deleting live sessions. |

Implemented:

- `harness_leases`, `team_workers`, `file_locks`, and `registry_entries` are now active `GC_STORES` with statically wired adapters.
- Harness lease prune rechecks owner PID and lease identity; team worker prune requires every recorded PID to be dead; registry prune re-reads roots before rewriting/removing.
- `tmux_sessions` is tracked as the bounded residual above, not represented as falsely active.

Verification:

- `bun test packages/coding-agent/test/file-lock-gc-toctou.test.ts packages/coding-agent/test/jwc-runtime/gc-stores.test.ts packages/coding-agent/test/jwc-runtime/gc-runtime.test.ts` — 25 pass, 0 fail.
- Targeted `biome check` on the nine changed TypeScript files — clean.
- `bun run check:types` — no errors in this card's files; blocked by five unrelated pre-existing/parallel errors in Cursor version and session-title work.
