...
> **구현가치: 60 (MLB)** — goal runtime 조향 6종(add_subgoal + split·reorder·revise·annotate·mark_blocked_superseded) + 통합 dispatcher. [10.021](../../10.021_gjc_chase_goal_redteam_review.md) 게이트 해제.
...
- [x] Steering types/constants ported with `Goal*` naming — `NATIVE_STEERING_KINDS`(6) + `GoalSteeringKind` + 11 helpers, `Ultragoal*`→`Goal*`, session-scope 제거(`getGoalPaths(cwd)`).
- [x] All 6 steering commands functional — `addGoalSubgoalToPlan`·`splitGoalSubgoal`·`reorderPendingGoals`·`revisePendingGoalWording`·`annotateGoalLedger`·`markBlockedGoalSuperseded`. last-goal guard로 vacuous completion 차단.
- [x] `dispatchGoalCommand` routes through new dispatcher — `case "steer"` → `executeGoalSteeringCommand(args, cwd)` (6종 전부); `--replacements-json`/`--order-json`를 `FLAGS_WITH_VALUES`에 추가.
- [x] Tests cover each steering kind — `test/jwc-runtime/ultragoal-runtime.test.ts` 84/0 (steering 5 new: split state-transform·reorder/revise/annotate/supersede·last-goal reject·another-remains accept·rejection-audit sweep). **Non-vacuous**: 구버전 hard-reject 소스 0/5 → 포트 5/5. 회귀 `goal-mode-integration`+`jwc-skill-state-hooks` 19/0; tsc 0. JWC `0974e20`.
...
| Decision B — JWC naming impact | Port as `Goal*`/`jwc goal` runtime language, never public `ultragoal`/`gjc` strings. |
| Decision C — test impact | Focused goal-runtime tests must cover all steering kinds plus help/manifest command surface. |
...
| Decision E — rollout impact | No migration decision yet; ledger/schema compatibility must be checked before runtime patching. |
| Decision F — residual risk | Monitored until `add_subgoal` compatibility and additional steering commands are both proven. |
| Decision G — reviewer needed | Backend/architecture review because this is goal runtime steering. |
| Decision H — bundle policy | Single-card implementation for all goal runtime steering unless tests show dispatcher and help/manifest need separate commits. |

Scope is **all goal runtime**: split, reorder, wording revise, ledger annotation, blocked-superseded, and unified dispatcher, with existing `add_subgoal` preserved.

## Jawdev chase expansion — 2026-06-26

> Document: `struct_har/chase/10.012_gjc_chase_goal_steering.md`
> Title: 10.012 — gjc chase: goal steering subsystem
> Lane: GJC
...
9. If the correct patch surface is outside packages/coding-agent/, record why the owner changed before widening scope.
