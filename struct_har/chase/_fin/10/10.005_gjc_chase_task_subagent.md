...
> MOC: [10_gjc_chase_MOC](../../10_gjc_chase_MOC.md) · G1 · ✅ **_fin**

upstream task·OAuth sessionId — jaw `forkContext: allowed`·가드 유지.

## 완료

- [x] diff confirms jwc-adequate (adapt=no-op): OAuth sessionId byte-identical to gjc, forkContext gate intact, jwc canonical extended task-agent surface; existing tests (`default-jwc-definitions`, `task-fork-context`, `task-bundled-agent-surface`) cover it; Decision F resolved (byte-identical OAuth = no new leak) → `_fin/10/`
...
| Decision B — JWC naming impact | Preserve JWC task/subagent naming and the `forkContext: allowed` gate; do not copy upstream strings blindly. |
| Decision C — test impact | Existing JWC tests are the first proof: `default-jwc-definitions.test.ts`, `task-fork-context.test.ts`, `task-bundled-agent-surface.test.ts`, and executor-ext routing tests. |
...
| Decision E — rollout impact | No migration; runtime patch must preserve subagent session contextFiles and OAuth sessionId safety. |
| Decision F — residual risk | Monitored until implementation proves OAuth session-sticky behavior does not leak provider state across subagent boundaries. |
| Decision G — reviewer needed | Backend/architecture review because task execution, context inheritance, and auth session routing cross runtime boundaries. |
| Decision H — bundle policy | Single-card implementation unless OAuth sessionId propagation and task/subagent surface need separate commits. |

Boundary decisions:

- Executor default without `inheritContext` keeps the current JWC self-fork behavior.
- `task.forkContext.enabled` remains strictly gated by bundled agent `forkContext: allowed`.
- AGENTS/contextFiles forwarding into subagent sessions is preserved.
- JWC's extended bundled task-agent surface is canonical.
- `executor_ext` is the external-model fresh lane; normal `executor` remains the fork-agent lane. Treat them as different contracts.
- OAuth `sessionId` behavior is safe adapt only: session-sticky lookup is allowed, provider/session leakage is not.
- Done gate is decision plus focused existing tests, not document length.

## Jawdev chase expansion — 2026-06-26

> Document: `struct_har/chase/10.005_gjc_chase_task_subagent.md`
> Title: 10.005 — gjc chase: task · subagent
> Lane: GJC
