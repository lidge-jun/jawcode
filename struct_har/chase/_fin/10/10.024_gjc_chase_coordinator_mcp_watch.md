...
- [x] Product decision recorded in this chase card (Decision A=split; 2026-06-26 interview).
- [ ] Subset (event-watch/snapshot) deferred to a later focused card per Decision H — not part of this card close.
...
| Decision B — JWC naming impact | Preserve JWC coordinator naming and MCP quarantine rules; GJC/Hermes terms remain source references unless productized. |
| Decision C — test impact | Done gate is card decision plus subset tests for coordinator MCP, harness observe, or RPC `get_state` equivalent hooks. |
...
| Decision E — rollout impact | Product surface is split: public coordinator MCP expansion is not enabled by this card recording. |
| Decision F — residual risk | Monitored until event-watch/snapshot subset is either implemented behind JWC quarantine or explicitly rejected. |
| Decision G — reviewer needed | Backend/architecture review for any coordinator MCP, harness, or RPC observation runtime change. |
| Decision H — bundle policy | Split implementation: decision/card first, event-watch/snapshot subset as a later focused card or PABCD phase. |

Split scope: keep public MCP discovery quarantined by default, document equivalent JWC observation hooks first, and only then adapt a bounded event-watch/snapshot subset if product needs remote/bot observation.

## Jawdev chase expansion — 2026-06-26

> Document: `struct_har/chase/10.024_gjc_chase_coordinator_mcp_watch.md`
> Title: 10.024 — gjc chase: coordinator MCP event watch + Hermes snapshot
> Lane: GJC
