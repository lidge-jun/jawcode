...
> MOC: [10_gjc_chase_MOC](../../10_gjc_chase_MOC.md) · [bands/081_cursor](../../bands/081_cursor.md) · G1 **P1** · ✅ **_fin**
...
jaw: host pin · Cursor IDE disclaimer · [20.004](../../20.004_omp_chase_lsp_dap.md) omp는 참고만. Autocompact fallback is a cross-card/session concern, not part of this cursor patch surface.

## 완료

- [x] **timeout ms→s 버그픽스 채택** (commit `4eeffb7`, `shellTimeoutSeconds` helper byte-identical to reviewed gjc, tsc 0 err, 2 call site); F15 cache 의식적 보류(perf 후속); flatten 의식적 held(jwc `schema/` 파이프라인); host-pin/disclaimer는 jwc 선행 → `_fin/10/`
...
| Decision B — JWC naming impact | Preserve JWC Cursor host-model pin and Cursor IDE disclaimer; autocompact fallback belongs to the session/10.004 lane, not this cursor surface. |
| Decision C — test impact | Focus provider/cursor and coding-agent cursor tests or snapshot anchors that prove the JWC fork delta still holds. |
...
| Decision E — rollout impact | No migration; user-visible behavior note only if provider/cursor prompts or host model routing change. |
| Decision F — residual risk | **Reconciled (2026-06-27)**: timeout bug fixed in `coding-agent/cursor.ts` (`4eeffb7`); `ai/providers/cursor.ts` has no passthrough bug (display only); F15 deferred (perf), flatten held. Runtime-test residual = native catalog drift ([devlog 16](../../../../devlog/_plan/260627_chase_direction_interview/16_pause_audit.md)). |
| Decision G — reviewer needed | Backend review for provider behavior; architecture review if cursor routing crosses package boundaries. |
| Decision H — bundle policy | Single-card implementation covering both cursor surfaces. |

The patch surface is intentionally two-part: `packages/ai/src/providers/cursor.ts` and `packages/coding-agent/src/cursor.ts`.

## Jawdev chase expansion — 2026-06-26

> Document: `struct_har/chase/10.003_gjc_chase_cursor.md`
> Title: 10.003 — gjc chase: cursor (081)
> Lane: GJC
> Status: active chase card
> Canonical source: `devlog/_gjc_chase/gajae-code` (dev tracking upstream/dev)
> Primary patch surfaces: packages/ai/, packages/coding-agent/src/cursor.ts
...
1. Start from this document, then open the current source lane at `devlog/_gjc_chase/gajae-code` and the matching JWC files under packages/ai/ and packages/coding-agent/src/cursor.ts.
...
9. If the correct patch surface is outside packages/ai/ or packages/coding-agent/src/cursor.ts, record why the owner changed before widening scope.
...
3. JWC owner files listed before edit: packages/ai/ and packages/coding-agent/src/cursor.ts.
