...
|------|-----------------|
| Decision A — source behavior classification | **reject** (pi-catalog 비채택 D4); omp workerHost는 M2 100 참조만 |
| Decision B — JWC naming impact | none — JWC M2/Node 명명 유지; workerHost/pi-catalog는 소스 참조어 |
| Decision C — test impact | manual evidence only — 코드 변경 없음 (100 MOC 링크 1줄) |
| Decision D — docs impact | chase only → `_fin/20/` + 20 MOC 행 |
| Decision E — rollout impact | no migration |
| Decision F — residual risk | closed — pi-catalog 의식적 거부, workerHost는 M2 100에서 추적 |
| Decision G — reviewer needed | no |
| Decision H — bundle policy | single-card commit |

Closed reference: pi-catalog rejected (D4), workerHost tracked under M2 100. Disposition → `_fin/20/`.

## Jawdev chase expansion — 2026-06-26

> Document: `struct_har/chase/20.002_omp_chase_worker_catalog.md`
> Title: 20.002 — omp: worker · catalog
> Lane: OMP
