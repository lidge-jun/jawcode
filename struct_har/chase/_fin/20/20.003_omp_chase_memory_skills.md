...
|------|-----------------|
| Decision A — source behavior classification | **reject/reference** — mnemopi는 99.01로 이미 구현; `.omp/skills`는 D5 cli-jaw 경로 |
| Decision B — JWC naming impact | none — `.jwc/skills` + cli-jaw 명명 유지 |
| Decision C — test impact | manual evidence only — 99.01 ✅ (`006 §99.01`) |
| Decision D — docs impact | chase only → `_fin/20/` + MOC |
| Decision E — rollout impact | no migration |
| Decision F — residual risk | closed — 99.01 covers; `.omp/skills`는 D5로 의식적 라우팅 |
| Decision G — reviewer needed | no |
| Decision H — bundle policy | single-card commit |

Closed reference: mnemopi → shipped 99.01; `.omp/skills` → D5 cli-jaw. Disposition → `_fin/20/`.

## Jawdev chase expansion — 2026-06-26

> Document: `struct_har/chase/20.003_omp_chase_memory_skills.md`
> Title: 20.003 — omp: memory · skills
> Lane: OMP
...
1. Start from this document, then open the current source lane at `devlog/_omp_chase/oh-my-pi` and the matching JWC files under packages/coding-agent/, packages/utils/.
