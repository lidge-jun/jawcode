...
> Primary patch surfaces: packages/tui/
...
1. Start from this document, then open the current source lane at `devlog/_omp_chase/oh-my-pi` and the matching JWC files under packages/tui/.
...
9. If the correct patch surface is outside packages/tui/, record why the owner changed in the devlog before widening scope.
...
3. JWC owner files listed before edit: packages/tui/.
