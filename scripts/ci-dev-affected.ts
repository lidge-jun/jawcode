...
function planTasks(paths: readonly string[], packages: readonly WorkspacePackage[]): Task[] {
	...
	const webChanged = paths.some(changedPath => changedPath.startsWith("python/robojwc/web/"));
	...
	if (needsNativeRuntime) {
		add(tasks, "cli-smoke", "JWC CLI smoke test", ["bun", "run", "ci:test:smoke"]);
	}
...
function isPythonPath(changedPath: string): boolean {
	return changedPath.startsWith("python/robojwc/") && !changedPath.startsWith("python/robojwc/web/");
}
