# Jawcode Installer for Windows
# Usage: irm https://raw.githubusercontent.com/lidge-jun/jawcode/main/scripts/install.ps1 | iex
#
# Or with options:
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/lidge-jun/jawcode/main/scripts/install.ps1))) -Source
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/lidge-jun/jawcode/main/scripts/install.ps1))) -Binary
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/lidge-jun/jawcode/main/scripts/install.ps1))) -Source -Ref v3.20.1
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/lidge-jun/jawcode/main/scripts/install.ps1))) -Source -Ref main
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/lidge-jun/jawcode/main/scripts/install.ps1))) -Binary -Ref v3.20.1
...
$Package = "jawcode"
$InstallDir = if ($env:JWC_INSTALL_DIR) { $env:JWC_INSTALL_DIR } elseif ($env:GJC_INSTALL_DIR) { $env:GJC_INSTALL_DIR } else { "$env:LOCALAPPDATA\jwc" }
$BinaryName = "jwc-windows-x64.exe"
...
function Configure-BashShell {
    try {
        ...
        if ($bashPath) {
            ...
            Write-Host "  Jawcode requires a bash shell on Windows. Options:" -ForegroundColor Yellow
...
function Install-ViaBun {
    ...
    if ($Ref) {
        ...
        $tmpRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("jwc-install-" + [System.Guid]::NewGuid().ToString("N"))
        ...
        try {
            ...
            $packagePath = Join-Path $tmpRoot "packages\jwc"
    ...
    Write-Host ""
    Write-Host "✓ Installed jwc via bun" -ForegroundColor Green

    Configure-BashShell

    Write-Host "Run 'jwc' to get started!"
}
...
function Install-Binary {
    ...
    $OutPath = Join-Path $InstallDir "jwc.exe"
    Invoke-WebRequest -Uri $BinaryUrl -OutFile $OutPath

    Write-Host ""
    Write-Host "✓ Installed jwc to $OutPath" -ForegroundColor Green
    ...
    if ($needsRestart) {
        Write-Host "Restart your terminal, then run 'jwc' to get started!"
    } else {
        Write-Host "Run 'jwc' to get started!"
    }
}
