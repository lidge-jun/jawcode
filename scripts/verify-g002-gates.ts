...
const ALLOWED_PUBLIC_PACKAGE_VERSIONS = new Map<string, string>([["jawcode", "1.0.8"]]);
...
async function verifyVisibleDefinitions(): Promise<GateResult> {
	...
	const bundledRoleAgentPromptFiles = readVisibleEntries("packages/coding-agent/src/prompts/agents").filter(entry =>
		EXPECTED_ROLE_AGENT_PROMPT_FILES.includes(entry as (typeof EXPECTED_ROLE_AGENT_PROMPT_FILES)[number]),
	);
	const callableTaskRoleFindings = await assertCallableTaskRoles(EXPECTED_CALLABLE_TASK_ROLES, repoRoot);
	...
	const expectedRoleAgentPromptFiles = [...EXPECTED_ROLE_AGENT_PROMPT_FILES].sort();
	const expectedCallableTaskRoles = [...EXPECTED_CALLABLE_TASK_ROLES].sort();
	const skills = [...bundledSkills].sort();
	const roleAgentPromptFiles = [...bundledRoleAgentPromptFiles].sort();
	...
	details.push(`expected bundled role prompt files: ${expectedRoleAgentPromptFiles.join(", ")}`);
	details.push(`actual bundled role prompt files: ${roleAgentPromptFiles.join(", ") || "<none>"}`);
	details.push(`expected callable task roles: ${expectedCallableTaskRoles.join(", ")}`);
	details.push(`callable task role findings: ${callableTaskRoleFindings.join("; ") || "<none>"}`);
	...
	return {
		name: "bundled workflow skills and source task-role definitions",
		passed:
			arraysEqual(skills, expectedSkills) &&
			arraysEqual(roleAgentPromptFiles, expectedRoleAgentPromptFiles) &&
...
async function verifyPublicDefinitionContent(): Promise<GateResult> {
	...
	for (const agent of EXPECTED_ROLE_AGENT_PROMPT_FILES) {
		const relativePath = `packages/coding-agent/src/prompts/agents/${agent}.md`;
		const text = await readText(relativePath);
		if (new RegExp(String.raw`\b` + "om" + "x" + String.raw`\b`, "iu").test(text)) findings.push(`${relativePath}: contains legacy team runtime naming`);
		for (const pattern of FORBIDDEN_SKILL_PATTERNS) {
			if (pattern.test(text)) findings.push(`${relativePath}: ${pattern.source}`);
		}
	}
