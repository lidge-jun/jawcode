...
import { assertCallableTaskRoles, listCallableTaskRoles } from "./lib/callable-task-roles";
...
const expectedBundledWorkflowSkills = ["jaw-interview", "plan", "team", "goal"] as const;
...
const legacyAllowlist = [
	{
		...
		path: /(^LICENSE$|^README\.md$|(^|\/)(CHANGELOG|NOTICE|AUTHORS)\.md$)/,
...
function listBundledRoleAgents(): VisibleDefinition[] {
	return listDefinitionFiles("packages/coding-agent/src/prompts/agents", "agent", [".md"])
		.filter(def => expectedBundledRoleAgentPromptFiles.includes(def.name as (typeof expectedBundledRoleAgentPromptFiles)[number]))
		.sort((a, b) => a.name.localeCompare(b.name) || a.path.localeCompare(b.path));
}
...
function scanLegacyHits(): LegacyHit[] {
	const roots = [
		...
		"Dockerfile.robojwc.dockerignore",
	];
...
const callableTaskRoles = listCallableTaskRoles();
...
const unexpectedBundledRoleAgents = bundledRoleAgents.filter(def => !expectedBundledRoleAgentPromptFiles.includes(def.name as (typeof expectedBundledRoleAgentPromptFiles)[number]));
...
const missingBundledRoleAgents = expectedBundledRoleAgentPromptFiles.filter(name => !bundledRoleAgents.some(def => def.name === name));
...
const report = {
	allowlists: {
		...
		bundledRoleAgentPromptFiles: expectedBundledRoleAgentPromptFiles,
		callableTaskRoles: expectedCallableTaskRoles,
	...
	inventory: {
		bundledRoleAgentPromptFiles: bundledRoleAgents,
		callableTaskRoles,
	...
	violations: {
		missingBins,
		missingBundledRoleAgentPromptFiles: missingBundledRoleAgents,
		missingBundledWorkflowSkills,
		nonGajaePackages,
		rootMetadataViolations,
		callableTaskRoleFindings,
		unexpectedBundledRoleAgentPromptFiles: unexpectedBundledRoleAgents,
		...
		JSON.stringify(
			{
				...
				bundledRoleAgentPromptFiles: bundledRoleAgents.map(def => `${def.type}:${def.name}`),
				callableTaskRoles,
