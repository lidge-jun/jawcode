...
# Jawcode Installer
# Usage: curl -fsSL https://raw.githubusercontent.com/lidge-jun/jawcode/main/scripts/install.sh | sh
#
# Options:
...
PACKAGE="jawcode"
INSTALL_DIR="${JWC_INSTALL_DIR:-${GJC_INSTALL_DIR:-$HOME/.local/bin}}"
...
install_via_bun() {
    ...
    if [ -n "$REF" ]; then
        ...
        if [ ! -d "$TMP_DIR/packages/jwc" ]; then
            echo "Expected package at ${TMP_DIR}/packages/jwc"
            exit 1
        fi

        bun install -g "$TMP_DIR/packages/jwc" || {
    ...
    echo ""
    echo "✓ Installed jwc via bun"
    echo "Run 'jwc' to get started!"
}
...
install_binary() {
    ...
    curl -fsSL "$BINARY_URL" -o "${INSTALL_DIR}/jwc"
    chmod +x "${INSTALL_DIR}/jwc"
    echo ""
    echo "✓ Installed jwc to ${INSTALL_DIR}/jwc"

    # Check if in PATH
    case ":$PATH:" in
        *":$INSTALL_DIR:"*) echo "Run 'jwc' to get started!" ;;
        *) echo "Add ${INSTALL_DIR} to your PATH, then run 'jwc'" ;;
    esac
}
