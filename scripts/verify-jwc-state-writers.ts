...
const ALLOWED_WRITER_RELATIVE = path.join("packages", "coding-agent", "src", "jwc-runtime", "state-writer.ts");
...
const KNOWN_ALLOWED_SITES = new Set<string>([
	"packages/coding-agent/src/jwc-runtime/team-runtime.ts:1610:fs.mkdir",
	"packages/coding-agent/src/jwc-runtime/team-runtime.ts:1612:fs.mkdir",
	"packages/coding-agent/src/jwc-runtime/team-runtime.ts:1623:fs.mkdir",
...
function locallyReferencesJwc(lines: readonly string[], index: number): boolean {
...
function sameLineReferencesJwc(line: string): boolean {
	return GJC_REFERENCE_PATTERNS.some(re => re.test(line));
}


function isGuardedOutsideProjectJwcFallback(lines: readonly string[], index: number): boolean {
	const start = Math.max(0, index - 8);
	const context = lines.slice(start, index + 1).join("\n");
	return /isUnderProjectJwc\([\s\S]*?\}\s*else\s*\{[\s\S]*$/u.test(context);
}

function isGuardedOutsideProjectJwcTernaryFallback(lines: readonly string[], index: number): boolean {
	const start = Math.max(0, index - 6);
	const context = lines.slice(start, index + 1).join("\n");
	return /isUnderProjectJwc\([\s\S]*?\?[\s\S]*?:\s*[^\n]*$/u.test(context);
}
...
function collectFindings(): Finding[] {
	...
	for (const file of listTsFiles(SCAN_ROOT)) {
		...
		for (let i = 0; i < lines.length; i++) {
			...
			if (!allowed && !sameLineReferencesJwc(line) && !nearbyAssignmentTargetsThisLine(lines, i)) continue;
			if (!allowed && (isGuardedOutsideProjectJwcFallback(lines, i) || isGuardedOutsideProjectJwcTernaryFallback(lines, i))) continue;
...
function main(): void {
	...
	console.log(`jwc .jwc/** writer inventory - scanned ${path.relative(repoRoot, SCAN_ROOT)}`);
