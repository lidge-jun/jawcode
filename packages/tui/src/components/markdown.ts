		...
		switch (token.type) {
			...
			case "code": {
				...
				if (token.lang === "mermaid" && this.#theme.resolveMermaidAscii) {
					...
					if (ascii) {
						const stripAnsi =
							typeof Bun !== "undefined" ? Bun.stripANSI : (s: string) => s.replace(/\x1b\[[0-9;]*m/g, "");
			...
			case "html": {
				...
				if (visibleHtml.trim().length > 0) lines.push(this.#applyDefaultStyle(visibleHtml.trim()));
				break;
			}
	...
	#renderInlineTokens(tokens: Token[], styleContext?: InlineStyleContext): string {
		...
		for (const token of tokens) {
			switch (token.type) {
				...
				case "html": {
					const visibleHtml = "raw" in token && typeof token.raw === "string" ? stripHtmlComments(token.raw) : "";
					if (visibleHtml.trim().length > 0) result += applyTextWithNewlines(visibleHtml);
					break;
				}
...
export function renderInlineMarkdown(text: string, mdTheme: MarkdownTheme, baseColor?: (t: string) => string): string {
	...
	for (const token of tokens) {
		if (token.type === "paragraph" && token.tokens) {
			...
			if (visibleHtml.trim().length > 0) result += applyText(visibleHtml);
