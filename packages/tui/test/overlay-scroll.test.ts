...
class ViewportAwareOverlay implements Component {
	#rows = 1;
	...
	render(_width: number): string[] {
		return Array.from({ length: this.#rows }, (_v, i) => `overlay-${i}`);
	}
}
...
describe("TUI overlays", () => {
	...
	it("passes resolved row budget to full-height overlays", async () => {
		const term = new VirtualTerminal(40, 6);
		const tui = new TUI(term);

		tui.addChild(new MutableContentComponent(buildRows(6)));

		tui.start();
		await flushRender(term);

		tui.showOverlay(new ViewportAwareOverlay(), {
		...
		expect(viewport).toEqual(["overlay-0", "overlay-1", "overlay-2", "overlay-3", "overlay-4", "overlay-5"]);
