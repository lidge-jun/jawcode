...
describe("RenderMetrics red-team coverage", () => {
	it("keeps disabled record paths zeroed under high-volume no-op calls", () => {
		...
		for (let i = 0; i < 100_000; i++) {
			...
			metrics.recordCounter(`source-${i}`, `counter-${i}`, i);
			metrics.recordEvent(`event-source-${i}`, { [`event-counter-${i}`]: i });
		}
		...
		expect(snapshot.eventsBuffered).toBe(0);
		expect(snapshot.eventsDropped).toBe(0);
	});
	...
	it("bounds enabled metric label maps and aggregates overflow under other", () => {
		...
		for (let i = 0; i < MAX_LABEL_MAP_ENTRIES * 4; i++) {
			...
			metrics.recordCounter(`counter-source-${i}`, `counter-${i}`, 1);
		}
		...
		expect(Object.keys(snapshot.counters)).toHaveLength(MAX_LABEL_MAP_ENTRIES);
		expect(snapshot.counters.other).toBe(MAX_LABEL_MAP_ENTRIES * 4 - (MAX_LABEL_MAP_ENTRIES - 1));
	});
	it("bounds retained event rows and records dropped overflow", () => {
		const metrics = new RenderMetrics(true);
		for (let i = 0; i < MAX_PERF_EVENTS + 7; i++) {
			metrics.recordEvent("overflow", { count: 1 });
		}
		const snapshot = metrics.snapshot();
		expect(snapshot.eventsBuffered).toBe(MAX_PERF_EVENTS);
		expect(snapshot.eventsDropped).toBe(7);
	});
	it("bounds emitted event source, counter, and label cardinality", () => {
		const metrics = new RenderMetrics(true);
		for (let i = 0; i < MAX_LABEL_MAP_ENTRIES * 4; i++) {
			metrics.recordEvent(`event-source-${i}`, { [`event-counter-${i}`]: 1 }, { [`event-label-${i}`]: "value" });
		}
		const events = metrics.events();
		expect(events.some(event => event.source === "other")).toBe(true);
		expect(events.some(event => event.counters.other !== undefined)).toBe(true);
		expect(events.some(event => event.labels?.other === "other")).toBe(true);
	});

	it("does not sample memory during event recording", () => {
		const metrics = new RenderMetrics(true);
		const memorySpy = spyOn(process, "memoryUsage");
		try {
			metrics.recordCounter("tui.frame", "frame.count", 1);
			metrics.recordEvent("tui.frame", { "frame.lines": 1 });
	...
	it("flush helpers write compact JSONL without stdout noise and drain on success", async () => {
		const metrics = new RenderMetrics(true);
		metrics.recordCounter("tui.frame", "frame.count", 1);
		const tmpPath = path.join(process.cwd(), `.tmp-${Date.now()}-perf-redteam.jsonl`);
		const stdoutSpy = spyOn(process.stdout, "write");
		const stderrSpy = spyOn(process.stderr, "write");
		try {
			await flushPerfEvents(metrics, tmpPath);
			expect(metrics.snapshot().eventsBuffered).toBe(0);
			...
			metrics.recordCounter("tui.frame", "frame.count", 1);
			flushPerfEventsSync(metrics, tmpPath);
			expect(metrics.snapshot().eventsBuffered).toBe(0);
			const lines = (await Bun.file(tmpPath).text()).trim().split("\n");
	...
	it("flush write errors are swallowed and surfaced as perf.write.error", () => {
		const metrics = new RenderMetrics(true);
		metrics.recordCounter("tui.frame", "frame.count", 1);
		flushPerfEventsSync(metrics, process.cwd());
		const snapshot = metrics.snapshot();
		expect(snapshot.counters["perf.write.error"]).toBe(1);
		expect(snapshot.eventsBuffered).toBeGreaterThanOrEqual(1);
	});
