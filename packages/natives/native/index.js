...
try {
	...
	const msg = `@jawcode-dev/natives: no prebuilt binary for ${platform}. ` +
		`Native features (grep, ast-grep, syntax highlight, PTY) are unavailable. ` +
		`Install a platform-specific build or use WSL on Windows.\n` +
		`  Original error: ${loadError?.message ?? loadError}`;
	if (typeof process !== "undefined" && process.stderr) process.stderr.write(msg + "\n");
	const stub = () => { throw new Error(`@jawcode-dev/natives is not available on ${platform}`); };
	nativeBindings = new Proxy({}, { get: (_, prop) => typeof prop === "string" ? stub : undefined });
}
