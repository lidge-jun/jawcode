...
describe("staleness supersession ordering", () => {
	...
	it("an Add File patch envelope edit supersedes earlier reads of the added path", () => {
		const entries: SessionEntry[] = [];
		const readAdded = pair(entries, "c1", "read", { path: "src/new.ts" });
		const readOther = pair(entries, "c2", "read", { path: "src/other.ts" });
		const envelope = ["*** Begin Patch", "*** Add File: src/new.ts", "@@", "+new", "*** End Patch"].join("\n");
		pair(entries, "c3", "apply_patch", { input: envelope }, 100);
		const ids = prunedIds(entries, EAGER);
		expect(ids).toContain(readAdded.id);
		expect(ids).not.toContain(readOther.id);
	});
