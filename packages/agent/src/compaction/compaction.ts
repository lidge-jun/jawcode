...
interface CompactionProgressEmitterOptions {
	onProgress?: CompactionProgressBoundaryCallback;
}

function emitCompactionProgress(
	options: CompactionProgressEmitterOptions | undefined,
	update: CompactionProgressBoundaryUpdate,
): void {
	const onProgress = options?.onProgress;
	if (!onProgress) return;
	const clamped: CompactionProgressBoundaryUpdate = {
		...update,
		percent: Math.max(0, Math.min(100, update.percent)),
	};
	try {
		const result = onProgress(clamped);
...
export function resolveThresholdTokens(
	...
	// Percentage-based threshold. Missing values use the built-in default; the
	// non-positive sentinel keeps the legacy reserve-based behavior available.
	const configuredThresholdPercent = settings.thresholdPercent ?? DEFAULT_COMPACTION_SETTINGS.thresholdPercent;
	if (
		typeof configuredThresholdPercent !== "number" ||
		!Number.isFinite(configuredThresholdPercent) ||
		configuredThresholdPercent <= 0
	) {
		return contextWindow - effectiveReserveTokens(contextWindow, settings, maxOutputTokens);
	}
	const clampedThresholdPercent = Math.min(99, Math.max(1, configuredThresholdPercent));
	const percentThreshold = Math.floor(contextWindow * (clampedThresholdPercent / 100));
	const outputReserve = Math.max(0, maxOutputTokens);
	const outputReservedThreshold = outputReserve > 0 ? contextWindow - outputReserve : percentThreshold;
	return Math.min(percentThreshold, outputReservedThreshold);
}
 ...
 */
function countCollectedMessageFragments(
	collected: { fragments: string[]; extra: number },
	encoding?: Encoding,
): number {
	return countTokens(collected.fragments, encoding) + collected.extra;
}

export function countMessageTokensNative(message: AgentMessage, encoding?: Encoding): number {
	return countCollectedMessageFragments(collectMessageFragments(message), encoding);
}

export function countMessageTokensNativeO200k(message: AgentMessage): number {
	return countMessageTokensNative(message);
}

export function createEstimateTokens(encoding?: Encoding): (message: AgentMessage) => number {
	return message => countMessageTokensNative(message, encoding);
}
...
const entryTokenCache = new WeakMap<SessionEntry, { fingerprint: string; encoding: string; tokens: number }>();

export function estimateEntryTokens(entry: SessionEntry, encoding?: Encoding): number {
	...
	const encKey = encoding ?? "";
	const cached = entryTokenCache.get(entry);
	if (cached?.fingerprint === fingerprint && cached.encoding === encKey) return cached.tokens;
	const tokens = countCollectedMessageFragments(collected, encoding);
	entryTokenCache.set(entry, { fingerprint, encoding: encKey, tokens });
	return tokens;
}
...
export function estimateEntriesTokens(
	...
	for (let i = startIndex; i < endIndex; i++) {
		total += estimateEntryTokens(entries[i], encoding);
	}
	return total;
}
...
export function findCutPoint(
	...
	for (let i = endIndex - 1; i >= startIndex; i--) {
		...
		const messageTokens = estimateEntryTokens(entry, encoding);
...
export async function generateSummary(
	...
	if (options?.remoteEndpoint) {
		emitCompactionProgress(options, {
			phase: "remote_summarization",
			percent: 15,
			segment: "remote_await",
			message: "Remote summarization…",
			indeterminate: true,
			backend: "generic_remote",
		});
...
export async function generateHandoff(
	...
	emitCompactionProgress(options, {
		...
		percent: 30,
		segment: "local_summary",
		message: "Generating handoff document…",
		indeterminate: true,
		backend: "local_llm",
	});
...
async function generateShortSummary(
	...
	emitCompactionProgress(options, {
		...
		indeterminate: true,
		backend: options?.remoteEndpoint ? "generic_remote" : "local_llm",
	});
	if (options?.remoteEndpoint) {
		const remote = await requestRemoteCompaction(
			options.remoteEndpoint,
...
export function prepareCompaction(
	...
	if (lastUsage) {
		const estimatedTokens = estimateEntriesTokens(pathEntries, boundaryStart, boundaryEnd, encoding);
	...
	const cutPoint = findCutPoint(pathEntries, boundaryStart, boundaryEnd, keepRecentTokens, encoding);
...
export async function compact(
	...
	if (isSplitTurn && turnPrefixMessages.length > 0) {
		emitCompactionProgress(summaryOptions, {
			...
			percent: 30,
			segment: "parallel_local_summaries",
			message: "Summarizing split turn…",
			indeterminate: true,
			backend: "local_llm",
		});
