...
export interface GenerateBranchSummaryOptions {
	...
	/** Tokenizer encoding override for provider-accurate token counting. */
	encoding?: Encoding;
}
...
export async function generateBranchSummary(
	...
	const { model, apiKey, signal, customInstructions, reserveTokens = 16384, metadata, encoding } = options;

	// Token budget = context window minus reserved space for prompt + response
	const contextWindow = model.contextWindow || 128000;
	const tokenBudget = contextWindow - reserveTokens;

	const { messages, fileOps } = prepareBranchEntries(entries, tokenBudget, encoding);
