 ...
 * by callers. Published `jawcode` bundles run from `dist/jwc.bundle.js`, so
 * the package build also emits `dist/sync-worker.js`; use it when the source
 * worker is not present beside the bundle.
 */
function createSyncWorker(): Worker {
	if (isCompiledBinary()) {
		return new Worker("./packages/stats/src/sync-worker.ts", { type: "module" });
	}
	const sourceWorkerUrl = new URL("./sync-worker.ts", import.meta.url);
	if (!fileUrlExists(sourceWorkerUrl)) {
		const bundledWorkerUrl = new URL("./sync-worker.js", import.meta.url);
		if (fileUrlExists(bundledWorkerUrl)) {
			return new Worker(bundledWorkerUrl.href, { type: "module" });
		}
	}
	return new Worker(sourceWorkerUrl.href, { type: "module" });
}
