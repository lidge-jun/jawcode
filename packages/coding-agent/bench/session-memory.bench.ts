 ...
 */
export function measureSessionMemory(entryCount = 4_000, largeBodyChars = 2_048, options: SessionMemoryOptions = {}): SessionMemoryReport {
	const gc = (globalThis as { gc?: () => void }).gc;
	const root = fs.mkdtempSync(path.join(options.rootParent ?? os.tmpdir(), "jwc-session-memory-"));
	try {
		gc?.();
		...
		for (let i = 0; i < entryCount; i++) {
			manager.appendMessage({ role: "user", content: bigText(i, largeBodyChars), timestamp: Date.now() + i });
		}
