...
import { isRpcCommandType, scopeForRpcCommand } from "@jawcode-dev/coding-agent/modes/shared/agent-wire/scopes";
import {
	MemoryGateStore,
	WorkflowGateBroker,
...
describe("RPC workflow_gate contract", () => {
	...
	it("round-trips a gate emission and a schema-validated answer", async () => {
		...
		const gate = broker.openGate({ stage: "plan", kind: "approval", schema, context: { title: "Approve plan?" } });
