...
import {
	...
	readJwcTeamSnapshot,
	readJwcTeamTask,
	requestJwcWorkerIntegrationAttempt,
	resolveJwcTeamWorkerCli,
	resolveJwcTeamWorkerCliPlan,
	resolveJwcWorkerCommand,
	sendJwcTeamMessage,
	shutdownJwcTeam,
	startJwcTeam,
	transitionJwcTeamTask,
	translateJwcWorkerLaunchArgsForCli,
} from "../../src/jwc-runtime/team-runtime";
...
async function readTeamConfig(stateDir: string): Promise<JwcTeamConfig> {
	return Bun.file(path.join(stateDir, "config.json")).json() as Promise<JwcTeamConfig>;
}
		...
		items: [
			{
				...
				command:
					"bun test packages/coding-agent/test/jwc-runtime/team-runtime.test.ts --test-name-pattern completion evidence",
				output: "passed",
			},
		],
		files: ["packages/coding-agent/src/jwc-runtime/team-runtime.ts"],
...
function inspectionCompletionEvidence(summary = "Completed by inspection") {
	return {
		...
		files: ["packages/coding-agent/test/jwc-runtime/team-runtime.test.ts"],
	};
}
...
describe("native gjc team runtime", () => {
	...
	it("separates managed worker lifecycle from worker-reported status", async () => {
		...
		const initialStatus = (await executeJwcTeamApiOperation(
			"read-worker-status",
		...
		const startupAck = (await executeJwcTeamApiOperation(
			"worker-startup-ack",
		...
		let snapshot = await readJwcTeamSnapshot("worker-lifecycle-team", cleanupRoot, { PATH: "" });
		...
		const workingStatus = (await executeJwcTeamApiOperation(
		...
		snapshot = await readJwcTeamSnapshot("worker-lifecycle-team", cleanupRoot, { PATH: "" });
		...
		await executeJwcTeamApiOperation(
			"update-worker-status",
		...
		const forceRequest = (await executeJwcTeamApiOperation(
	...
	it("keeps worker CLI selection limited to GJC teammate sessions", async () => {
		...
		expect(resolveJwcTeamWorkerCli({ GJC_TEAM_WORKER_CLI: "gjc" })).toBe("gjc");
		expect(resolveJwcTeamWorkerCliPlan(3, { GJC_TEAM_WORKER_CLI_MAP: "auto" })).toEqual(["gjc", "gjc", "gjc"]);
		expect(resolveJwcTeamWorkerCliPlan(2, { GJC_TEAM_WORKER_CLI_MAP: "gjc,auto" })).toEqual(["gjc", "gjc"]);
		expect(translateJwcWorkerLaunchArgsForCli("gjc", ["--model", "frontier"])).toEqual(["--model", "frontier"]);
		cleanupRoot = await fs.mkdtemp(path.join(os.tmpdir(), "gjc-team-runtime-"));
		const snapshot = await startJwcTeam({
			workerCount: 2,
			agentType: "executor",
		...
		for (const provider of ["codex", "claude", "gemini"]) {
			expect(() => resolveJwcTeamWorkerCli({ GJC_TEAM_WORKER_CLI: provider })).toThrow(
				/GJC team launches GJC teammate sessions only/,
			);
			expect(() => resolveJwcTeamWorkerCliPlan(1, { GJC_TEAM_WORKER_CLI_MAP: provider })).toThrow(
				/GJC team launches GJC teammate sessions only/,
			);
			expect(() =>
				resolveJwcTeamWorkerCliPlan(1, { GJC_TEAM_WORKER_CLI: provider, GJC_TEAM_WORKER_CLI_MAP: "gjc" }),
	...
	it("distributes explicit markdown lane sections into worker-owned initial tasks", async () => {
		...
		const task1 = await readJwcTeamTask("lane-distribution-team", "task-1", cleanupRoot, { PATH: "" });
		const task2 = await readJwcTeamTask("lane-distribution-team", "task-2", cleanupRoot, { PATH: "" });
	...
	it("removes clean created worker worktrees on normal shutdown", async () => {
		...
		const stopped = await shutdownJwcTeam("cleanup-team", cleanupRoot, { PATH: process.env.PATH ?? "" });
	...
	it("does not kill stale or leader pane ids during shutdown", async () => {
		...
		await shutdownJwcTeam("stale-pane-team", cleanupRoot, {
	...
	it("preserves dirty worker worktrees on normal shutdown", async () => {
		...
		const stopped = await shutdownJwcTeam("dirty-cleanup-team", cleanupRoot, { PATH: process.env.PATH ?? "" });
	...
	it("supports task claim, transition, list, status, and shutdown lifecycle operations", async () => {
		...
		await expect(
			transitionJwcTeamTask("life-team", "task-1", "completed", cleanupRoot, { PATH: "" }),
		).rejects.toThrow("claim_token_required:task-1");

		const claim = await claimJwcTeamTask("life-team", "worker-1", cleanupRoot, { PATH: "" });
		expect(claim.ok).toBe(true);
		await expect(
			transitionJwcTeamTask("life-team", "task-1", "completed", cleanupRoot, { PATH: "" }),
		).rejects.toThrow("claim_token_required:task-1");
		await expect(
			transitionJwcTeamTask("life-team", "task-1", "pending", cleanupRoot, { PATH: "" }, claim.claim_token),
		).rejects.toThrow("invalid_task_transition:task-1:pending_requires_release");
		expect(claim.task?.status).toBe("in_progress");
		const task = await transitionJwcTeamTask(
			"life-team",
			"task-1",
			"completed",
			cleanupRoot,
		...
		const reclaim = await claimJwcTeamTask("life-team", "worker-1", cleanupRoot, { PATH: "" }, "task-1");
		expect(reclaim.ok).toBe(false);
		expect(reclaim.reason).toBe("task_not_pending:task-1");

		const status = await readJwcTeamSnapshot("life-team", cleanupRoot, { PATH: "" });
		expect(status.task_counts.completed).toBe(1);
		expect(await listJwcTeams(cleanupRoot, { PATH: "" })).toHaveLength(1);

		const stopped = await shutdownJwcTeam("life-team", cleanupRoot, { PATH: "" });
	...
	it("chains claim, transition, and release using receipt-only task fields", async () => {
		...
		const claimReceipt = (await executeJwcTeamApiOperation(
			"claim-task",
		...
		const releaseReceipt = (await executeJwcTeamApiOperation(
			"release-task-claim",
			{
		...
		const secondClaimReceipt = (await executeJwcTeamApiOperation(
			"claim-task",
		...
		const transitionReceipt = (await executeJwcTeamApiOperation(
			"transition-task-status",
			{
				team_name: "receipt-team",
	...
	it("writes versioned structured traces linked to legacy events", async () => {
		...
		const claim = await claimJwcTeamTask("trace-team", "worker-1", cleanupRoot, { PATH: "" });
		expect(claim.ok).toBe(true);
		await transitionJwcTeamTask(
			"trace-team",
			"task-1",
			"completed",
			cleanupRoot,
		...
		const traceRead = (await executeJwcTeamApiOperation("read-traces", { team_name: "trace-team" }, cleanupRoot, {
			PATH: "",
		})) as {
	...
	it("sanitizes mailbox bodies from structured traces", async () => {
		...
		const traceRead = (await executeJwcTeamApiOperation(
			"read-traces",
	...
	it("stores structured completion evidence in task listings and honors claim tokens without implicit worker defaults", async () => {
		...
		const workerTwoClaim = await claimJwcTeamTask("evidence-team", "worker-2", cleanupRoot, { PATH: "" }, "task-2");
		...
		const completedTask = await readJwcTeamTask("evidence-team", "task-2", cleanupRoot, { PATH: "" });
		...
		const listed = (await executeJwcTeamApiOperation("list-tasks", { team_name: "evidence-team" }, cleanupRoot, {
			PATH: "",
		...
		const workerOneClaim = await claimJwcTeamTask("evidence-team", "worker-1", cleanupRoot, { PATH: "" }, "task-1");
	...
	it("rejects completed transitions without valid evidence and leaves task state unchanged", async () => {
		...
		const claim = await claimJwcTeamTask("invalid-evidence-team", "worker-1", cleanupRoot, { PATH: "" });
		expect(claim.ok).toBe(true);
		const taskBefore = await readJwcTeamTask("invalid-evidence-team", "task-1", cleanupRoot, { PATH: "" });
		...
		const taskAfter = await readJwcTeamTask("invalid-evidence-team", "task-1", cleanupRoot, { PATH: "" });
	...
	it("allows non-command completion evidence and requires evidence-backed shutdown completion", async () => {
		...
		const firstClaim = await claimJwcTeamTask(
		...
		const first = await transitionJwcTeamTask(
			"review-evidence-team",
			"task-1",
			"completed",
			cleanupRoot,
		...
		const secondClaim = await claimJwcTeamTask(
			"review-evidence-team",
			"worker-2",
			cleanupRoot,
			{ PATH: "" },
		...
		const stopped = await shutdownJwcTeam("review-evidence-team", cleanupRoot, { PATH: "" });
		expect(stopped.phase).toBe("complete");
	});
	...
	it("treats legacy evidence-free completed tasks as failed on shutdown", async () => {
		...
		const task = await readJwcTeamTask("legacy-completed-team", "task-1", cleanupRoot, { PATH: "" });
		await Bun.write(
			path.join(stateDir, "tasks", "task-1.json"),
			`${JSON.stringify({ ...task, status: "completed", completed_at: new Date().toISOString() }, null, 2)}\n`,
		);

		const stopped = await shutdownJwcTeam("legacy-completed-team", cleanupRoot, { PATH: "" });
	...
	it("recovers expired task claims before new claim attempts", async () => {
		...
		const claim = await claimJwcTeamTask("expired-claim-team", "worker-1", cleanupRoot, { PATH: "" });
		expect(claim.ok).toBe(true);
		const claimedTask = await readJwcTeamTask("expired-claim-team", "task-1", cleanupRoot, { PATH: "" });
		...
		const recoveredClaim = await claimJwcTeamTask("expired-claim-team", "worker-1", cleanupRoot, { PATH: "" });
	...
	it("recovers stale heartbeat claims during monitor and blocks stale workers from reclaiming", async () => {
		...
		const claim = await claimJwcTeamTask("stale-heartbeat-team", "worker-1", cleanupRoot, { PATH: "" });
		expect(claim.ok).toBe(true);
		await Bun.write(
		...
		const snapshot = await monitorJwcTeam("stale-heartbeat-team", cleanupRoot, {
			PATH: "",
		...
		const retry = await claimJwcTeamTask("stale-heartbeat-team", "worker-1", cleanupRoot, {
			PATH: "",
			GJC_TEAM_HEARTBEAT_STALE_MS: "1",
		});
	...
	it("keeps read-only status separate from mutating monitor recovery", async () => {
		...
		const claim = await claimJwcTeamTask("status-semantics-team", "worker-1", cleanupRoot, { PATH: "" });
		expect(claim.ok).toBe(true);
		await Bun.write(
			path.join(stateDir, "workers", "worker-1", "heartbeat.json"),
			`${JSON.stringify(
		...
		const statusSnapshot = await readJwcTeamSnapshot("status-semantics-team", cleanupRoot, {
			PATH: "",
			GJC_TEAM_HEARTBEAT_STALE_MS: "1",
		});
		...
		const monitorSnapshot = await monitorJwcTeamSnapshot("status-semantics-team", cleanupRoot, {
			PATH: "",
			GJC_TEAM_HEARTBEAT_STALE_MS: "1",
		});
	...
	it("recovers missing-pane claims and marks the worker lifecycle failed", async () => {
		...
		const stateDir = snapshot.state_dir;
		const claim = await claimJwcTeamTask("missing-pane-team", "worker-1", cleanupRoot, {
			PATH: process.env.PATH ?? "",
			GJC_TEAM_TMUX_COMMAND: fakeTmux,
		});
		expect(claim.ok).toBe(true);
		...
		const recovered = await monitorJwcTeam("missing-pane-team", cleanupRoot, {
			PATH: process.env.PATH ?? "",
			GJC_TEAM_TMUX_COMMAND: fakeTmux,
		});
	...
	it("enforces typed lane claim eligibility before leases are granted", async () => {
		...
		const wrongOwner = await claimJwcTeamTask("lane-team", "worker-1", cleanupRoot, { PATH: "" }, "task-2");
		expect(wrongOwner.ok).toBe(false);
		expect(wrongOwner.reason).toBe("task_owner_mismatch:task-2:worker-2");

		const dependentTask = (await executeJwcTeamApiOperation(
			"create-task",
			{
				team_name: "lane-team",
		...
		const blockedByDependency = await claimJwcTeamTask("lane-team", "worker-1", cleanupRoot, { PATH: "" }, "task-3");
		...
		const wrongRole = await claimJwcTeamTask("lane-team", "worker-1", cleanupRoot, { PATH: "" }, "task-4");
		expect(wrongRole.ok).toBe(false);
		expect(wrongRole.reason).toBe("task_role_mismatch:task-4:architect");

		const implementationClaim = await claimJwcTeamTask("lane-team", "worker-1", cleanupRoot, { PATH: "" }, "task-1");
		...
		const verificationClaim = await claimJwcTeamTask("lane-team", "worker-1", cleanupRoot, { PATH: "" }, "task-3");
	...
	it("allows only one worker to claim a task under concurrent claim attempts", async () => {
		...
		const claims = await Promise.all([
			claimJwcTeamTask("claim-race-team", "worker-1", cleanupRoot, { PATH: "" }, "task-1"),
			claimJwcTeamTask("claim-race-team", "worker-2", cleanupRoot, { PATH: "" }, "task-1"),
		]);
		...
		const status = await readJwcTeamSnapshot("claim-race-team", cleanupRoot, { PATH: "" });
	...
	it("supports GJC team parity behavioral API operations", async () => {
		...
		const created = (await executeJwcTeamApiOperation(
			"create-task",
		...
		const read = (await executeJwcTeamApiOperation(
			"read-task",
		...
		const updated = (await executeJwcTeamApiOperation(
			"update-task",
		...
		const claim = (await executeJwcTeamApiOperation(
			"claim-task",
		...
		const claimed = (await executeJwcTeamApiOperation(
			"claim-task",
			{ team_name: "api-team", task_id: created.task_id, worker: "worker-1" },
			cleanupRoot,
			{ PATH: "" },
		...
		const message = (await executeJwcTeamApiOperation(
			"send-message",
		...
		const mailbox = (await executeJwcTeamApiOperation(
			"mailbox-list",
		...
		const heartbeat = (await executeJwcTeamApiOperation(
		...
		const awaited = (await executeJwcTeamApiOperation("await-event", { team_name: "api-team" }, cleanupRoot, {
			PATH: "",
		})) as { status: string };
		...
		const monitor = (await executeJwcTeamApiOperation(
		...
		const approval = (await executeJwcTeamApiOperation(
			"read-task-approval",
			{ team_name: "api-team", task_id: created.task_id },
			cleanupRoot,
			{ PATH: "" },
	...
	it("stores mailbox messages per recipient and maintains native notification transitions", async () => {
		...
		const first = (await executeJwcTeamApiOperation(
			"send-message",
			{
		...
		const second = (await executeJwcTeamApiOperation(
			"send-message",
			{
				team_name: "notification-team",
				from_worker: "worker-1",
		...
		let notifications = (await executeJwcTeamApiOperation(
			"notification-list",
			{ team_name: "notification-team" },
			cleanupRoot,
			{ PATH: "" },
		...
		notifications = (await executeJwcTeamApiOperation(
			"notification-list",
			{ team_name: "notification-team" },
			cleanupRoot,
			{ PATH: "" },
	...
	it("rejects path-like worker ids and reports lifecycle nudges without automatic worker action", async () => {
		...
		const monitored = await monitorJwcTeam("guard-team", cleanupRoot, {
			PATH: "",
	...
	it("monitor integrates dirty detached worker worktrees and records GJC-scoped hygiene artifacts", async () => {
		...
		const monitored = await monitorJwcTeam("integrate-dirty-team", cleanupRoot, {
			PATH: process.env.PATH ?? "",
			GJC_TEAM_TMUX_COMMAND: fakeTmux,
		});
	...
	it("checkpoint classification excludes GJC runtime paths from worker auto-commits", async () => {
		...
		await monitorJwcTeam("protected-checkpoint-team", cleanupRoot, {
			PATH: process.env.PATH ?? "",
			GJC_TEAM_TMUX_COMMAND: fakeTmux,
		});
	...
	it("worker turn-end integration requests notify the leader once per fingerprint", async () => {
		...
		const first = await requestJwcWorkerIntegrationAttempt(worker.worktree_path, env);
		const second = await requestJwcWorkerIntegrationAttempt(worker.worktree_path, env);
	...
	it("reports awaiting integration when all worker tasks completed after an integration request", async () => {
		...
		const requested = await requestJwcWorkerIntegrationAttempt(worker.worktree_path, requestEnv);
		expect(requested.requested).toBe(true);

		const claim = await claimJwcTeamTask("awaiting-request-team", "worker-1", cleanupRoot, {
			PATH: process.env.PATH ?? "",
		});
		await transitionJwcTeamTask(
		...
		const status = await readJwcTeamSnapshot("awaiting-request-team", cleanupRoot, { PATH: process.env.PATH ?? "" });
		expect(status.task_counts.completed).toBe(1);
		expect(status.phase).toBe("awaiting_integration");
		expect(status.phase).not.toBe("running");

		const stopped = await shutdownJwcTeam("awaiting-request-team", cleanupRoot, { PATH: process.env.PATH ?? "" });
	...
	it("monitor cherry-picks diverged worker commits and stays idempotent on repeated status checks", async () => {
		...
		const first = await monitorJwcTeam("diverged-team", cleanupRoot, {
			PATH: process.env.PATH ?? "",
			GJC_TEAM_TMUX_COMMAND: fakeTmux,
		});
		const leaderAfterFirst = runGit(cleanupRoot, ["rev-parse", "HEAD"]);
		const second = await monitorJwcTeam("diverged-team", cleanupRoot, {
			PATH: process.env.PATH ?? "",
			GJC_TEAM_TMUX_COMMAND: fakeTmux,
		});
	...
	it("monitor reports merge conflicts without falsely advancing last integrated head", async () => {
		...
		const monitored = await monitorJwcTeam("merge-conflict-team", cleanupRoot, {
			PATH: process.env.PATH ?? "",
			GJC_TEAM_TMUX_COMMAND: fakeTmux,
		});
	...
	it("keeps completed conflicting teams in awaiting integration instead of plain running", async () => {
		...
		await commitFile(workerPath, "README.md", "# worker\n", "worker readme");
		await Bun.write(path.join(cleanupRoot, "README.md"), "# leader dirty\n");
		const claim = await claimJwcTeamTask("awaiting-conflict-team", "worker-1", cleanupRoot, {
			PATH: process.env.PATH ?? "",
		});
		await transitionJwcTeamTask(
		...
		const monitored = await monitorJwcTeam("awaiting-conflict-team", cleanupRoot, {
			PATH: process.env.PATH ?? "",
			GJC_TEAM_TMUX_COMMAND: fakeTmux,
		});
		...
		const stopped = await shutdownJwcTeam("awaiting-conflict-team", cleanupRoot, { PATH: process.env.PATH ?? "" });
		expect(stopped.task_counts.completed).toBe(1);
		expect(stopped.phase).toBe("awaiting_integration");
		expect(stopped.phase).not.toBe("complete");
	});
	...
	it("monitor reports cherry-pick conflicts and aborts cleanly", async () => {
		...
		const monitored = await monitorJwcTeam("pick-conflict-team", cleanupRoot, {
			PATH: process.env.PATH ?? "",
			GJC_TEAM_TMUX_COMMAND: fakeTmux,
		});
	...
	it("cross-rebases idle, done, and failed workers while skipping working workers", async () => {
		...
		const monitored = await monitorJwcTeam("cross-rebase-team", cleanupRoot, {
			PATH: process.env.PATH ?? "",
			GJC_TEAM_TMUX_COMMAND: fakeTmux,
		});
	...
	it("pure team reads, status, and list operations stay read-only while monitor and resume can mutate", async () => {
		...
		const listed = await listJwcTeams(cleanupRoot, { PATH: process.env.PATH ?? "" });
		const read = await readJwcTeamSnapshot("pure-read-team", cleanupRoot, { PATH: process.env.PATH ?? "" });
		...
		expect(commandSource).toContain("readJwcTeamSnapshot(teamName)");
		expect(commandSource).toContain('action === "monitor" || action === "resume"');
		expect(commandSource).toContain("monitorJwcTeamSnapshot(teamName)");
		expect(commandSource).toContain("listJwcTeams()");
		...
		const runtimeSource = await Bun.file(path.join(import.meta.dir, "../../src/jwc-runtime/team-runtime.ts")).text();
