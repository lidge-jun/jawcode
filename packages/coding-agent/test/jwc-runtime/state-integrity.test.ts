...
describe("jwc state integrity", () => {
	it("blocks out-of-band edits on the next mode-state write unless forced", async () => {
		await withTempCwd(async cwd => {
			...
			expect(mismatch).toMatchObject({ skill: "plan", category: "state", owner: "jwc-state-cli" });
	...
	it("handoff writes and removes its per-mutation journal on success", async () => {
		await withTempCwd(async cwd => {
			...
			const result = await runNativeStateCommand(["handoff", "--mode", "jaw-interview", "--to", "plan"], cwd);
	...
	it("an injected mid-handoff failure leaves a same-mutation journal while unrelated writes still proceed", async () => {
		await withTempCwd(async cwd => {
			...
			process.env.GJC_STATE_HANDOFF_FAIL_AFTER_CALLER = "jaw-interview:handoff:plan:2026-06-03T00:00:00.000Z";
			try {
				const failed = await runNativeStateCommand(["handoff", "--mode", "jaw-interview", "--to", "plan"], cwd);
			...
			expect(journal).toMatchObject({
				status: "pending",
				mutation_id: "jaw-interview:handoff:plan:2026-06-03T00:00:00.000Z",
			});

			Date.prototype.toISOString = () => "2026-06-03T00:00:00.000Z";
			try {
				const recovered = await runNativeStateCommand(["handoff", "--mode", "jaw-interview", "--to", "plan"], cwd);
			...
			expect(await readJson(path.join(cwd, ".jwc/state/goal-state.json"))).toMatchObject({
				skill: "goal",
			});
		});
	});
