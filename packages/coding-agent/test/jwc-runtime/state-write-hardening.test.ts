...
describe("jwc state write hardening", () => {
	...
	it("rejects a known-bad jump", async () => {
		...
		expect(result.stderr).toContain("invalid plan phase transition from planner to final");
	});
	...
	it("rejects unknown legacy target phases unless forced", async () => {
		...
		expect(rejected.stderr).toContain('unknown plan phase "legacy-custom"');
	...
	it("reads unknown legacy phases fail-open", async () => {
		...
		await fs.writeFile(
			...
			JSON.stringify({ skill: "plan", version: 1, active: true, current_phase: "legacy-custom" }),
		);
	...
	it("corrupt existing state fails open for read and status but write and clear require force", async () => {
		const root = await tempDir();
		await writeRawState(root, "plan", "{broken json");
