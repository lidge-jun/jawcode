...
import { monitorJwcTeam, persistJwcTeamModeStateSummary, startJwcTeam } from "../../src/jwc-runtime/team-runtime";
...
describe("native gjc team mode-state convergence", () => {
	it("keeps gjc state team read aligned with dry-run team start and status snapshots", async () => {
		...
		const started = await startJwcTeam({
			workerCount: 1,
			agentType: "executor",
		...
		await persistJwcTeamModeStateSummary(started, cleanupRoot);
		...
		const status = await monitorJwcTeam(started.team_name, cleanupRoot, { PATH: "" });
		await persistJwcTeamModeStateSummary(status, cleanupRoot);
		...
		expect(statusState.state.receipt.owner).toBe("jwc-runtime");
	});
});
