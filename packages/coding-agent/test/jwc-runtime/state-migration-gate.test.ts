...
describe("G7 gjc state migration gate", () => {
	it("normalizes legacy state purely and persists migration only through the state command", async () => {
		await withTempCwd(async cwd => {
			...
			const persistedPath = path.join(cwd, ".jwc/state/plan-state.json");
	...
	it("migrates tampered state with --force and audits the forced mismatch", async () => {
		await withTempCwd(async cwd => {
			...
			expect(receipt).toMatchObject({ skill: "plan", migrated: true, integrity_mismatch: true });
			...
			expect(entries.at(-1)).toMatchObject({ skill: "plan", category: "state", verb: "migrate" });
		});
	});
});
