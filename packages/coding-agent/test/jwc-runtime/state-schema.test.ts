...
describe("state-schema (A1)", () => {
	...
	it("RequiredOnWriteEnvelopeSchema requires anchored fields incl. stamped receipt", () => {
		const validReceipt = {
			...
			skill: "plan",
			owner: "jwc-state-cli",
			command: "jwc state plan write",
			state_path: "/x/.jwc/state/plan-state.json",
			storage_path: "/x/.jwc/state/plan-state.json",
			mutated_at: "2026-01-01T00:00:00.000Z",
			fresh_until: "2026-01-01T00:30:00.000Z",
			status: "fresh",
			mutation_id: "plan:2026-01-01T00:00:00.000Z",
			content_sha256: {
				algorithm: "sha256",
				value: "abc",
				covered_path: "$",
		...
		expect(RequiredOnWriteEnvelopeSchema.safeParse({ skill: "plan" }).success).toBe(false);
	});
	...
	it("ModeState and SkillActiveState schemas are lenient", () => {
		...
		expect(SkillActiveStateSchema.safeParse({ version: 1, active_skills: [{ skill: "plan" }] }).success).toBe(true);
...
describe("readJwcJson (A2)", () => {
	...
	it("returns ok for valid content and preserves raw", async () => {
		const file = await tempFile("ok.json", JSON.stringify({ skill: "plan", version: 1, extra: "keep" }));
