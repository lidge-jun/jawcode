...
describe("InteractiveMode goal mode integration", () => {
	...
	it("replaces the active goal via direct /goal text", async () => {
		...
		const state = harness.session.getGoalModeState();
		expect(state?.enabled).toBe(true);
		expect(state?.goal.objective).toBe("objective B");
		expect(state?.goal.status).toBe("active");
		expect(state?.goal.id).not.toBe(originalGoal.id);
		expect(submittedText).toBe("objective B");
		expect((await readGoalPlan(harness.tempDir.path()))?.brief).toBe("objective B");
	...
	it("schedules hidden goal continuation while active goal waits for input", async () => {
		vi.useFakeTimers();
		try {
			await harness.mode.handleGoalModeCommand("Ship the release");

			const inputPromise = harness.mode.getUserInput();
			vi.advanceTimersByTime(801);
			const input = await inputPromise;

			expect(input.customType).toBe("goal-continuation");
			expect(input.display).toBe(false);
		} finally {
			vi.useRealTimers();
		}
	});

	it("re-arms hidden goal continuation instead of submitting while the session is busy", async () => {
		vi.useFakeTimers();
		try {
			await harness.mode.handleGoalModeCommand("Ship the release");
			const startSubmission = vi.spyOn(harness.mode, "startPendingSubmission");
			let streaming = true;
			Object.defineProperty(harness.session, "isStreaming", {
				configurable: true,
				get: () => streaming,
			});
			...
			const inputPromise = harness.mode.getUserInput();
			vi.advanceTimersByTime(801);
			expect(startSubmission).not.toHaveBeenCalledWith(expect.objectContaining({ customType: "goal-continuation" }));
	...
	it("re-arms hidden goal continuation instead of submitting while compaction is active", async () => {
		vi.useFakeTimers();
		try {
			await harness.mode.handleGoalModeCommand("Ship the release");
			const startSubmission = vi.spyOn(harness.mode, "startPendingSubmission");
	...
	it("suppresses hidden goal continuation after an explicit user interrupt", async () => {
		vi.useFakeTimers();
		try {
			await harness.mode.handleGoalModeCommand("Ship the release");
			const startSubmission = vi.spyOn(harness.mode, "startPendingSubmission");

			harness.mode.onUserInterrupt();
			const inputPromise = harness.mode.getUserInput();
			vi.advanceTimersByTime(801);

			expect(startSubmission).not.toHaveBeenCalledWith(expect.objectContaining({ customType: "goal-continuation" }));

			harness.mode.onInputCallback?.(harness.mode.startPendingSubmission({ text: "manual follow-up" }));
			const input = await inputPromise;
			expect(input.text).toBe("manual follow-up");
		} finally {
			vi.useRealTimers();
		}
	});
	...
	it("treats /goal plan text as AI-driven planning hint, not a literal objective", async () => {
		let submittedText: string | undefined;
		harness.mode.onInputCallback = input => {
			submittedText = input.text;
		};

		await harness.mode.handleGoalModeCommand("plan ship import cleanup");

		const goal = harness.session.getGoalModeState()?.goal;
		expect(goal?.objective).toContain(GOAL_PLAN_PENDING_BRIEF);
		expect(goal?.objective).toContain("hint: ship import cleanup");
		expect(goal?.objective).not.toBe("plan ship import cleanup");
		expect(submittedText).toContain("AI-driven goal planning is active.");
		expect(submittedText).toContain("Hint: ship import cleanup");
	});

	it("replaces an active goal via /goal plan and submits only the planning prompt", async () => {
		let submittedText: string | undefined;
		harness.mode.onInputCallback = input => {
			submittedText = input.text;
		};
		...
		expect(goal?.id).not.toBe(originalGoal.id);
		expect(submittedText).toContain("AI-driven goal planning is active.");
