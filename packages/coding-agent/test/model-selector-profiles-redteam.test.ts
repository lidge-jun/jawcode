...
} from "@jawcode-dev/coding-agent/modes/components/model-selector";
...
describe("model selector profile red-team", () => {
	...
	test("preset controls render above flat model rows", async () => {
		const selector = createSelector(() => {});
		const rendered = await renderSelector(selector);

		expect(rendered.indexOf("Current preset: none")).toBeGreaterThanOrEqual(0);
		expect(rendered.indexOf("View preset configuration")).toBeGreaterThan(rendered.indexOf("Current preset: none"));
