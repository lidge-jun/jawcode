...
describe("perf corpus schema + runner", () => {
	test("runner emits the schema with separated evidence fields and >=3 required fixture classes", () => {
		...
		for (const fixture of report.fixtures) {
			// the evidence classes are present as SEPARATE named fields
	...
	test("session-memory bench reports finite RSS and removes its temp root", () => {
		const rootParent = fs.mkdtempSync(path.join(os.tmpdir(), "jwc-session-memory-test-parent-"));
		try {
			const report = measureSessionMemory(4, 128, { rootParent });
			expect(report.entryCount).toBe(4);
			expect(report.largeBodyChars).toBe(128);
			expect(report.warmGetEntriesCount).toBe(4);
			expect(Number.isFinite(report.rssMemory.growthBytes)).toBe(true);
			expect(fs.readdirSync(rootParent).filter(name => name.startsWith("jwc-session-memory-")).length).toBe(0);
		} finally {
