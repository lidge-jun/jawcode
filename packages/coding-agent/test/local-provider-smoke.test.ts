...
describe("local provider streaming smoke", () => {
	...
	beforeEach(() => {
		tempDir = path.join(os.tmpdir(), `jwc-local-provider-smoke-${crypto.randomUUID()}`);
	...
	test("reports a clear configuration failure when local provider is not configured", async () => {
	...
	test("classifies streaming smoke auth failures and skips the stream when models auth fails", async () => {
		fs.writeFileSync(
			modelsPath,
			JSON.stringify({
				providers: {
	...
	test("reports local discovery network failures clearly", async () => {
		fs.writeFileSync(
			...
			JSON.stringify({
				providers: {
					local: { baseUrl: "http://127.0.0.1:65535/v1", apiKey: "local-key" },
				},
			}),
		);
		using _hook = hookFetch(() => {
