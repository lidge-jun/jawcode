...
describe("createAgentSession MCP discovery prompt gating", () => {
	...
	it("does not advertise MCP discovery when search_tool_bm25 is not active", async () => {
		const { session } = await createAgentSession({
			...
			modelRegistry,
			sessionManager: SessionManager.inMemory(),
			settings: Settings.isolated({ "mcp.discoveryMode": true, "tools.discoveryMode": "off" }),
			model: getBundledModel("openai", "gpt-4o-mini"),
			disableExtensionDiscovery: true,
			skills: [],
			contextFiles: [],
	...
	it("exposes generic discovery tooling for builtin-only tools.discoveryMode all sessions", async () => {
		...
		expect(session.getActiveToolNames()).not.toContain("job");
	...
	it(
		...
		async () => {
			...
			expect(await session.activateDiscoveredTools(["job"])).toEqual(["job"]);
			expect(session.getSelectedDiscoveredToolNames()).toContain("job");

			await session.setActiveToolsByName(["read", "search_tool_bm25"]);

			expect(session.getActiveToolNames()).not.toContain("job");
			expect(session.getSelectedDiscoveredToolNames()).not.toContain("job");
			expect(await session.activateDiscoveredTools(["job"])).toEqual(["job"]);
			expect(session.getActiveToolNames()).toContain("job");
		},
