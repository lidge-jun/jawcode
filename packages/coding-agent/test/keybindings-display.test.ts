...
describe("KeybindingsManager.getDisplayString", () => {
	...
	it("uses ctrl+t for full transcript and keeps thinking toggle unbound by default", () => {
		const keybindings = KeybindingsManager.inMemory();

		expect(keybindings.getDisplayString("app.transcript.full")).toBe("Ctrl+T");
		expect(keybindings.getDisplayString("app.thinking.toggle")).toBe("");
		expect(keybindings.getDisplayString("app.tools.transcript")).toBe("Alt+T");
	});
});
