...
import type { RpcWorkflowGate } from "@jawcode-dev/coding-agent/modes/rpc/rpc-types";
import {
	FileGateStore,
	type GateAuditEvent,
	MemoryGateStore,
...
describe("WorkflowGateBroker", () => {
	it("emits run-scoped monotonic gate ids per stage", () => {
		...
		const g1 = broker.openGate({ stage: "plan", kind: "approval", schema: { type: "string", enum: ["a", "b"] } });
		const g2 = broker.openGate({ stage: "plan", kind: "approval", schema: { type: "string", enum: ["a", "b"] } });
		expect(g1.gate_id).toBe("wg_04494845_plan_000001");
		expect(g2.gate_id).toBe("wg_04494845_plan_000002");
	...
	it("throws on unknown gate ids", async () => {
		const { broker } = makeBroker();
		await expect(broker.resolve({ gate_id: "wg_x_plan_000099", answer: 1 })).rejects.toThrow(WorkflowGateBrokerError);
	});
	...
	it("recover() advances accepted-but-not-advanced gates exactly once", async () => {
		...
		const gate = b1.openGate({ stage: "plan", kind: "approval", schema: { type: "string", enum: ["go"] } });
