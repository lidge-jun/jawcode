...
import { parseSessionEntries } from "@jawcode-dev/coding-agent";
import { readLines } from "@jawcode-dev/utils";
...
describe("jwc --mode rpc red-team stdio lifecycle", () => {
	...
	it("flushes durable session state on EOF and reloads it in a new RPC process", async () => {
		...
		expect(persistedMessages.length).toBeGreaterThanOrEqual(1);
		expect(
		...
		try {
			expect(await harness.nextFrame()).toEqual({ type: "ready" });
			harness.send({ id: "switch", type: "switch_session", sessionPath: firstState.sessionFile });
			const switchFrame = await harness.nextFrame();
			...
			harness.send({ id: "messages", type: "get_messages" });
			const messagesFrame = await harness.nextFrame();
			expect(messagesFrame).toMatchObject({
				id: "messages",
				type: "response",
				command: "get_messages",
				success: true,
			});
			const messagesData = frameData<{ messages: Array<{ role?: string; output?: string }> }>(messagesFrame);
			expect(
				messagesData.messages.some(
					message =>
						message.role === "bashExecution" &&
						(message.output === marker || JSON.stringify(message).includes(marker)),
				),
			).toBe(true);
			await harness.closeStdin();
			expect(await harness.proc.exited).toBe(0);
	...
	it("runs independent child sessions concurrently without state bleed", async () => {
		...
		const alphaParsed = JSON.parse(alphaOutput) as { lane: string; cwd: string };
		const betaParsed = JSON.parse(betaOutput) as { lane: string; cwd: string };
		expect(alphaParsed.lane).toBe("alpha");
		expect(betaParsed.lane).toBe("beta");
		expect(await realpath(alphaParsed.cwd)).toBe(await realpath(alphaCwd));
		expect(await realpath(betaParsed.cwd)).toBe(await realpath(betaCwd));
	}, 30_000);
});
