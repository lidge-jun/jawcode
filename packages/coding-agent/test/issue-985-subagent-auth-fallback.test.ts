...
import { kNoAuth } from "@jawcode-dev/coding-agent/config/model-registry";
import {
...
const configuredExecutorModel: Model<Api> = {
...
describe("issue #985: subagent dispatch auth fallback", () => {
	...
	test("falls back to parent active model when subagent selector does not resolve", async () => {
		const registry = createMockRegistry({
			models: [parentModel],
	...
	test("falls back to later configured role selector before parent active model", async () => {
		const registry = createMockRegistry({
			models: [parentModel, configuredExecutorModel],
			authedProviders: new Set(["deepseek", "xai"]),
		});

		const result = await resolveModelOverrideWithAuthFallback(
			["openai-codex/gpt-5.4", "xai/grok-composer-2.5-fast"],
			"deepseek/deepseek-v4-pro",
			registry,
		);
		...
		expect(result.model?.id).toBe("grok-composer-2.5-fast");
	});
