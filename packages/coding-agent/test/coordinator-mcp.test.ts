...
async function withTempRoot(run: (root: string) => Promise<void>): Promise<void> {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "jwc-coordinator-mcp-"));
	try {
		await run(root);
	} finally {
...
describe("jwc mcp-serve coordinator", () => {
	...
	it("implements initialize, tools/list, and read-only mutating rejection", async () => {
		const env = { ...process.env, JWC_COORDINATOR_MCP_REPO: "repo-a" };
		...
		expect(initialize).toEqual({
			...
			result: {
				...
				serverInfo: { name: "jwc-coordinator-mcp", version: expect.any(String) },
			},
		});

		const listed = await handleCoordinatorMcpRequest({ jsonrpc: "2.0", id: 2, method: "tools/list" }, { env });
		expect(listed.result.tools.map((tool: { name: string }) => tool.name)).toContain("jwc_coordinator_report_status");
		...
		const called = await handleCoordinatorMcpRequest(
			{
				...
				id: 3,
				method: "tools/call",
				params: { name: "jwc_coordinator_start_session", arguments: { cwd: process.cwd(), allow_mutation: true } },
			},
			{ env },
		);
	...
	it("requires startup mutation class and per-call allow_mutation for mutating tools", async () => {
		await withTempRoot(async root => {
			...
			const env = {
				...
				JWC_COORDINATOR_MCP_ENABLE_MUTATION_CLASSES: "session",
			};
		...
		await withTempRoot(async root => {
			...
			try {
				...
				const policy = await createCoordinatorSafetyPolicy({
					env: { ...process.env, JWC_COORDINATOR_MCP_WORKDIR_ROOTS: root },
				});
	...
	it("bounds artifact reads and denies unsafe roots", async () => {
		await withTempRoot(async root => {
			...
			const env = {
				...process.env,
				JWC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				JWC_COORDINATOR_MCP_ARTIFACT_MAX_BYTES: String(byteCap),
			};
			const server = await createCoordinatorMcpServer({ env });
			const read = await server.callTool("jwc_coordinator_read_artifact", { path: artifact });
			...
			await expect(
				server.callTool("jwc_coordinator_read_artifact", { path: path.join(os.tmpdir(), "missing.txt") }),
			).resolves.toEqual({
				ok: false,
