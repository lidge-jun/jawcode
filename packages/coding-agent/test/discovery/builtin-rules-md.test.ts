 ...
 * from both `~/.jwc/agent/RULES.md` (user) and the nearest `.jwc/RULES.md`
...
import { getCapability } from "@jawcode-dev/coding-agent/capability";
import { clearCache } from "@jawcode-dev/coding-agent/capability/fs";
import { type Rule, ruleCapability } from "@jawcode-dev/coding-agent/capability/rule";
import type { LoadContext } from "@jawcode-dev/coding-agent/capability/types";
// Register all discovery providers as a side effect.
import "@jawcode-dev/coding-agent/discovery";
...
test("user ~/.jwc/agent/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(
		path.join(home, ".jwc", "agent", "RULES.md"),
...
test("project .jwc/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(path.join(project, ".jwc", "RULES.md"), "# Project rule\nAlways say hi.\n");
...
test("project RULES.md is found walking up from a sub-package cwd", async () => {
	...
	writeFile(path.join(project, ".jwc", "RULES.md"), "# Repo-wide sticky rule\n");
	...
	expect(projectRule).toBeDefined();
	expect(projectRule?.alwaysApply).toBe(true);
	expect(projectRule?.path).toBe(path.join(project, ".jwc", "RULES.md"));
});

test("alwaysApply is forced even when frontmatter says false", async () => {
	writeFile(path.join(home, ".jwc", "agent", "RULES.md"), "---\nalwaysApply: false\n---\nStick around anyway.\n");
...
test("absent RULES.md does not produce a rule", async () => {
	// No RULES.md anywhere — only a sibling .jwc/rules/ to make sure the directory exists.
	writeFile(path.join(home, ".jwc", "agent", "rules", "other.md"), "# Unrelated rule\n");
