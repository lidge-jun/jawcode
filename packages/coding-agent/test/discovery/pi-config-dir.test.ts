...
import { getConfigDirs } from "@jawcode-dev/coding-agent/config";
import { getUserPath } from "@jawcode-dev/coding-agent/discovery/helpers";
...
describe("PI_CONFIG_DIR", () => {
	...
	test("getConfigDirs respects PI_CONFIG_DIR for user base", () => {
		...
		expect(result[0]).toEqual({ path: expected, source: ".jwc", level: "user" });
	});
