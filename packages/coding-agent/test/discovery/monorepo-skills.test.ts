...
import { clearCache } from "@jawcode-dev/coding-agent/capability/fs";
import type { Skill } from "@jawcode-dev/coding-agent/capability/skill";
import type { LoadContext, LoadResult } from "@jawcode-dev/coding-agent/capability/types";
import { scanSkillsFromDir } from "@jawcode-dev/coding-agent/discovery/helpers";
...
describe("monorepo skill discovery", () => {
	...
	test("finds skills in ancestor .jwc/skills/ directories", async () => {
		// Root has a skill
		writeSkill(path.join(repoRoot, ".jwc", "skills"), "root-skill", "From repo root");
		// Sub-project has a skill
		writeSkill(path.join(subProject, ".jwc", "skills"), "local-skill", "From sub-project");
		...
		while (true) {
			const result = await scanSkillsFromDir(ctx, {
				dir: path.join(current, ".jwc", "skills"),
				providerId: "native",
				level: "project",
			});
			results.push(result);
	...
	test("closest skill wins when same name exists at multiple levels", async () => {
		...
		writeSkill(path.join(repoRoot, ".jwc", "skills"), "shared-skill", "Root version");
		writeSkill(path.join(subProject, ".jwc", "skills"), "shared-skill", "Local version");
	...
	test("finds skills across multiple ancestor levels", async () => {
		...
		writeSkill(path.join(repoRoot, ".jwc", "skills"), "root-skill", "Root");
		writeSkill(path.join(packagesDir, ".jwc", "skills"), "packages-skill", "Packages");
		writeSkill(path.join(subProject, ".jwc", "skills"), "app-skill", "App");
	...
	test("walk-up stops at repo root and does not find skills above it", async () => {
		...
		writeSkill(path.join(tempDir, ".jwc", "skills"), "above-repo-skill", "Above repo");
		// Skill AT the repo root (should be found)
		writeSkill(path.join(repoRoot, ".jwc", "skills"), "root-skill", "At repo root");
