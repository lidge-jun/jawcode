...
import { ModelsConfigSchema, ProfileModelSelectorSchema } from "@jawcode-dev/coding-agent/config/models-config-schema";
...
describe("model profile red-team schema and catalog cases", () => {
	...
	test("resolveProfileBindings preserves effort suffixes verbatim", () => {
		...
		expect(resolved.agentModelOverrides).toEqual({ executor_ext: "provider/executor:high" });
	});

	test("mergeModelProfiles with undefined returns exactly the 17 builtins", () => {
