...
import type { RpcJsonSchema, RpcWorkflowGate } from "@jawcode-dev/coding-agent/modes/rpc/rpc-types";
import {
	FileGateStore,
	type GateAuditEvent,
	MemoryGateStore,
...
} from "@jawcode-dev/coding-agent/modes/shared/agent-wire/workflow-gate-schema";
...
describe("workflow gate red-team contract", () => {
	it("rejects schemas nested beyond the advertised max depth at construction", () => {
		...
		expect(() => makeBroker().broker.openGate({ stage: "plan", kind: "question", schema: tooDeep })).toThrow(
			WorkflowGateSchemaError,
		);
	});
	...
	it("rejects an oversized answer with a typed validation error and leaves the gate pending", async () => {
		...
		const gate = broker.openGate({ stage: "plan", kind: "question", schema: { type: "string" } });
	...
	it("preserves pending gates and accepted idempotency state across fresh FileGateStore brokers", async () => {
		...
		const gate = b1.openGate({ stage: "plan", kind: "approval", schema: { type: "string", enum: ["yes"] } });
