...
function expectFirstSeedParams(
	seedBuilder: { mock: { calls: readonly unknown[] } },
	expected: { maxMessages: number; maxTokens: number },
): void {
	const firstCall = seedBuilder.mock.calls[0] as
		| readonly [{ maxMessages?: number; maxTokens?: number; signal?: AbortSignal }]
		| undefined;
	expect(firstCall?.[0]).toMatchObject(expected);
}
...
describe("fork context policy surface", () => {
	...
	test("default executor self-fork builds a full seed when inheritContext is absent", async () => {
		mockAgents([createAgent("executor", "allowed")]);
		const seedBuilder = vi.fn(async () => createSeed());
		...
		expectFirstSeedParams(seedBuilder, { maxMessages: 500, maxTokens: 150 });
		expect(getOptions()?.forkContextSeed).toBeDefined();
	});

	test("forwards AGENTS.md context files into subagent sessions", async () => {
		mockAgents([createAgent("executor", "allowed")]);
		const seedBuilder = vi.fn(async () => createSeed());
		const { getOptions } = mockCreateAgentSession();
		const contextFiles: ToolSession["contextFiles"] = [
			{ path: "/repo/AGENTS.md", content: "Root repository contract" },
			{ path: "/repo/packages/coding-agent/AGENTS.md", content: "Package repository contract" },
			{ path: "/repo/README.md", content: "Readme context" },
		];
		const tool = await TaskTool.create(
			createSession({ "task.forkContext.enabled": true }, seedBuilder, contextFiles),
		);

		await executeDetached(tool, {
			agent: "executor",
			tasks: [{ id: "ContextGate", description: "seed", assignment: "Respect repo instructions." }],
		});

		expect(getOptions()?.contextFiles?.map(file => file.path)).toEqual([
			"/repo/AGENTS.md",
	...
	test("passes a sanitized fork seed and cache identity without sharing provider state", async () => {
		...
		expectFirstSeedParams(seedBuilder, { maxMessages: 50, maxTokens: 250 });
	...
	test("uses configured maxMessages to cap bounded fork-context seeds", async () => {
		...
		expectFirstSeedParams(seedBuilder, { maxMessages: 3, maxTokens: 250 });
	});
	...
	test("uses configured maxMessages to cap full fork-context seeds", async () => {
		...
		expectFirstSeedParams(seedBuilder, { maxMessages: 7, maxTokens: 150 });
	});

	test("freezes inherited context seeds before detached job execution for non-executor agents", async () => {
		mockAgents([createAgent("architect", "allowed")]);
	...
	test("bundled executor uses default self-fork and still routes explicit inheritContext through the per-agent gate", async () => {
