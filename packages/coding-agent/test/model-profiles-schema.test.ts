...
import { resolveProfileBindings } from "@jawcode-dev/coding-agent/config/model-profiles";
import { ModelsConfigSchema } from "@jawcode-dev/coding-agent/config/models-config-schema";
...
describe("model profile schema", () => {
	...
	test("resolveProfileBindings preserves single selectors verbatim", () => {
		...
		expect(resolved.agentModelOverrides.executor_ext).toBe("provider-a/executor:low");
	});
