...
function makeCtx(
	sessionManager?: Pick<
		SessionManager,
		"buildSessionContext" | "buildVisibleTranscriptContext" | "getEntries" | "getCwd"
	>,
): {
	...
	const buildDisplaySessionContextSpy = vi.fn(() => makeEmptyContext());
	const renderSessionContextSpy = vi.fn();

	const sm = sessionManager ?? {
		buildSessionContext: buildSessionContextSpy,
		buildVisibleTranscriptContext: vi.fn(() => makeEmptyContext()),
		getEntries: vi.fn(() => []),
		getCwd: vi.fn(() => "/tmp"),
	};
	...
	const ctx = {
		...
		sessionManager: sm,
		session: { buildDisplaySessionContext: buildDisplaySessionContextSpy },
...
describe("UiHelpers.renderInitialMessages — isolated", () => {
	it("calls session.buildDisplaySessionContext when no prebuilt context is given", () => {
		const { ctx } = makeCtx();
		new UiHelpers(ctx).renderInitialMessages();
		expect(ctx.session.buildDisplaySessionContext).toHaveBeenCalledTimes(1);
	});
	...
	it("uses the fallback context from session display context when no prebuilt is provided", () => {
		const fallback = makeEmptyContext();
		const { ctx, renderSessionContextSpy } = makeCtx();
		(ctx.session.buildDisplaySessionContext as Mock<() => SessionContext>).mockReturnValue(fallback);
		new UiHelpers(ctx).renderInitialMessages();
