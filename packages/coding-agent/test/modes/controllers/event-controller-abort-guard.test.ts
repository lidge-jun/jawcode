...
describe("EventController.sendCompletionNotification — abort guard", () => {
	...
	it("runs the user-level completion notify command with payload environment", () => {
		...
		const [cmd, options] = spawnSpy.mock.calls[0] as unknown as [
			string[],
			{ cwd?: string; detached?: boolean; env?: Record<string, string>; windowsHide?: boolean },
		];
		expect(cmd).toContain("notify-test");
		expect(options.cwd).toBe(process.cwd());
		expect(options.detached).toBe(true);
		expect(options.windowsHide).toBe(true);
		expect(options.env?.JWC_NOTIFICATION_TYPE).toBe("agent-turn-complete");
		expect(options.env?.JWC_NOTIFICATION_TITLE).toBe("test-session: Complete");
		expect(options.env?.JWC_NOTIFICATION_BODY).toBe("hello");
		expect(options.env?.JWC_NOTIFICATION_SESSION_ID).toBe("session-test");
		expect(options.env?.GJC_NOTIFICATION_TYPE).toBeUndefined();
	});
	...
	it("does not run completion notify commands from runtime overrides", () => {
		const terminalSpy = vi.spyOn(TERMINAL, "sendNotification").mockImplementation(() => {});
		const spawnSpy = vi.spyOn(Bun, "spawn").mockImplementation(
			() =>
				({
