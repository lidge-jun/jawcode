...
describe("built-in model profile catalog", () => {
	test("contains exactly 17 builtins", () => {
		expect(BUILTIN_MODEL_PROFILES).toHaveLength(17);
		expect(new Set(BUILTIN_MODEL_PROFILES.map(profile => profile.name)).size).toBe(17);
	});

	test("required_providers are correct per family", () => {
		for (const profile of BUILTIN_MODEL_PROFILES) {
			if (profile.name.startsWith("custom-")) {
				expect(profile.requiredProviders).toEqual([]);
				expect(profile.modelMapping).toEqual({});
			} else if (profile.name.startsWith("opencode-go-codex-")) {
				...
				profile.name === "minimax-standard" ||
				profile.name === "minimax-cn-standard" ||
				profile.name === "kimi-standard" ||
				profile.name === "glm-standard"
			) {
				expect(profile.requiredProviders.length).toBeGreaterThan(0);
			} else {
