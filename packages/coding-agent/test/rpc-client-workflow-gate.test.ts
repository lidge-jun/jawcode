...
import type { RpcExtensionUIRequest, RpcWorkflowGate } from "@jawcode-dev/coding-agent/modes/rpc/rpc-types";

const gate: RpcWorkflowGate = {
	type: "workflow_gate",
	gate_id: "wg_test_plan_000001",
	stage: "plan",
	kind: "approval",
	schema: { type: "object" },
	schema_hash: "hash-1",
...
async function withFakeServer(source: string, run: (client: RpcClient) => Promise<void>): Promise<void> {
	const scriptPath = path.join(os.tmpdir(), `jwc-rpc-workflow-gate-${Date.now()}-${Math.random()}.js`);
