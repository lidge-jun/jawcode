...
describe("GJC skill-active state", () => {
	...
	it("ignores inactive and blank entries while deduping by skill and session", () => {
		const active = listActiveSkills({
			active_skills: [
				...
				{ skill: "plan", phase: "draft", session_id: "sess-a" },
				{ skill: "plan", phase: "review", session_id: "sess-a" },
				{ skill: "plan", phase: "root" },
			],
		});
		expect(active).toEqual([
			expect.objectContaining({ skill: "plan", phase: "review", session_id: "sess-a" }),
			expect.objectContaining({ skill: "plan", phase: "root" }),
		]);
	});
	...
	it("strict session read ignores root sessionless rows for a fresh session", async () => {
		await withTempCwd(async cwd => {
			await syncSkillActiveState({ cwd, skill: "jaw-interview", active: true, phase: "interviewing" });

			const strict = await readSessionStrictSkillActiveState(cwd, "fresh-session");
			expect(strict).toBeNull();

			const legacyVisible = await readVisibleSkillActiveState(cwd, "fresh-session");
			expect(legacyVisible?.active_skills?.map(entry => entry.skill)).toEqual(["jaw-interview"]);
		});
	});

	it("strict session read returns only exact session rows", async () => {
		await withTempCwd(async cwd => {
			await syncSkillActiveState({ cwd, skill: "team", active: true, phase: "running", sessionId: "sess-a" });
			await syncSkillActiveState({
				cwd,
			...
			const strict = await readSessionStrictSkillActiveState(cwd, "sess-b");
			expect(strict?.active_skills?.map(entry => entry.skill)).toEqual(["jaw-interview"]);
			expect(strict?.session_id).toBe("sess-b");
		});
	});
	...
	it("shows only the callee when a skill is seeded session-less then handed off under a session", async () => {
		await withTempCwd(async cwd => {
			...
			expect(visible?.active_skills?.map(entry => entry.skill)).toEqual(["plan"]);
		});
	});

	it("does not demote a same-skill row owned by a different session during handoff", async () => {
		await withTempCwd(async cwd => {
			await syncSkillActiveState({ cwd, skill: "plan", phase: "planner", active: true, sessionId: "sessB" });
			await syncSkillActiveState({
				cwd,
				skill: "jaw-interview",
				phase: "interviewing",
			...
			expect(sessionA?.active_skills?.map(entry => entry.skill)).toEqual(["plan"]);
			expect(sessionB?.active_skills?.map(entry => entry.skill)).toEqual(["plan"]);
		});
	});
	...
	it("keeps every active pipeline skill at the read layer (HUD pipeline collapse is render-only)", async () => {
		await withTempCwd(async cwd => {
			// `jwc plan` then `jwc goal` each activate their own row without
			...
			expect(visible?.active_skills?.map(entry => entry.skill).sort()).toEqual(["goal", "plan"]);
		});
	});
	...
	it("keeps the canonical GJC workflow skill set intentionally small", () => {
		expect(CANONICAL_JWC_WORKFLOW_SKILLS).toEqual(["jaw-interview", "plan", "goal", "team"]);
	});
});
