...
describe("shouldCompact", () => {
	it("should return true when context exceeds threshold", () => {
		...
		// default mode uses the built-in 73% threshold.
		expect(shouldCompact(74_000, 100_000, settings)).toBe(true);
		expect(shouldCompact(73_000, 100_000, settings)).toBe(false);
	});
	...
	it("should use legacy reserve behavior when threshold is set to reserve sentinel", () => {
		const settings: CompactionSettings = {
			enabled: true,
			thresholdPercent: -1,
	...
	it("does not let a configured threshold exceed the reserved output budget", () => {
		const settings: CompactionSettings = {
			enabled: true,
			thresholdPercent: 73,
			reserveTokens: 16384,
		...
		expect(resolveThresholdTokens(400_000, settings, 128_000)).toBe(272_000);
		expect(resolveThresholdTokens(272_000, settings, 32_000)).toBe(198_560);
	});
