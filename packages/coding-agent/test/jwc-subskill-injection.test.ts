...
describe("GJC sub-skill prompt injection", () => {
	...
	test("session-scoped injection ignores root-only active subskill rows", async () => {
		const cwd = await tempProject();
		const activation = await activationFromFixture(cwd);
		await syncSkillActiveState({
			cwd,
		...
		const rootOnly = await buildSkillPromptMessage(ralplanSkill, "", { cwd, sessionId: "fresh-session" });
		expect(rootOnly.message).not.toContain("<gjc-subskill");
		expect(rootOnly.details.subskillActivation).toBeUndefined();
		...
		const scoped = await buildSkillPromptMessage(ralplanSkill, "", { cwd, sessionId: "fresh-session" });
		expect(scoped.message).toContain("<gjc-subskill");
		expect(scoped.details.subskillActivation?.subskillName).toBe(activation.subskillName);
	});
});
