 ...
 *
 * Note: helpers.ts imports @jawcode-dev/natives (Rust addon via glob).
...
describe("resolveActiveProjectRegistryPath", () => {
	...
	it("does not treat ~/.git as a project root (pass-2 home-dir guard)", async () => {
		...
		try {
			...
			const homeJwcPath = path.join(homeDir, ".jwc", "plugins", "installed_plugins.json");
			expect(result).not.toBe(homeJwcPath);
		} finally {
