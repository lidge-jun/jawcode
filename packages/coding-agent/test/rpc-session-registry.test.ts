...
describe("rpc session registry (issue 10)", () => {
	...
	test("lists socket transport endpoint metadata", async () => {
		await registerRpcSession(rec({ sessionId: "uds", transport: "socket", endpoint: "/tmp/jwc-rpc.sock" }), agentDir);

		expect(await listRpcSessions(agentDir)).toEqual([
			expect.objectContaining({ sessionId: "uds", transport: "socket", endpoint: "/tmp/jwc-rpc.sock" }),
		]);
	});
