...
//    or `../`-relative entry pointing somewhere else under `~/.jwc/`), plus
//    the legacy non-XDG `~/.jwc/plugins` tree on hosts without XDG dirs.
// 2. `getAgentDir()` (`~/.jwc/agent` or `$PI_CODING_AGENT_DIR`)
...
//    Handles installed plugin extensions that live outside `~/.jwc` when
...
// symlinks `~/.jwc/agent/extensions/foo.ts` to a dotfiles repo would see the
