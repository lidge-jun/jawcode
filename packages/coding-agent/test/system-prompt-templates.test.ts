...
import { type AgentTool, INTENT_FIELD } from "@jawcode-dev/agent-core";
import { buildSystemPrompt, buildSystemPromptToolMetadata } from "@jawcode-dev/coding-agent/system-prompt";
...
describe("system Handlebars prompt templates", () => {
	...
	test("system-prompt renders MCP discovery hint when enabled", async () => {
		...
		expect(rendered).toContain("hidden built-in or MCP tools");
	});
	...
	test("system-prompt renders local reference and external web search guidance", async () => {
		...
		const rendered = prompt.render(template, {
			...
			toolInfo: [
				...
				{ name: "search", label: "Search", description: "Searches files" },
				{ name: "find", label: "Find", description: "Finds files" },
				{ name: "ast_grep", label: "AST Grep", description: "Searches code structurally" },
				{ name: "web_search", label: "Web Search", description: "Searches the web" },
				{ name: "lsp", label: "LSP", description: "Queries language servers" },
			],
			tools: ["read", "search", "find", "ast_grep", "web_search", "lsp"],
		});

		expect(rendered).toContain("Use `find` for file-name/glob lookup");
		expect(rendered).toContain("`ast_grep` for syntax-shaped code lookup");
		expect(rendered).toContain("use LSP references/definitions when available");
		expect(rendered).toContain("use `web_search` for current information, pricing, compatibility, latest versions");
		expect(rendered).toContain("High-risk external claims");
		expect(rendered).toContain("Freshness before implementation");
		expect(rendered).toContain("syntax-shaped discovery: calls, imports, declarations");
		expect(rendered).toContain("semantic symbol operations");
		expect(rendered).toContain("structure is irrelevant plain text");
		expect(rendered).not.toContain("`grep`, `find`, `read`");
		expect(rendered).not.toContain("/skill:search");
		expect(rendered).not.toContain("/skill:dev");
	});

	test("system-prompt documents all four public workflow skills plus native orchestrate", async () => {
		const templatePath = path.join(systemPromptsDir, "system-prompt.md");
		const template = await Bun.file(templatePath).text();
		const rendered = prompt.render(template, baseRenderContext);

		for (const skillName of ["jaw-interview", "plan", "goal", "team"]) {
			expect(rendered).toContain(`<skill name="${skillName}"`);
		}
		expect(rendered).toContain('<native-workflow name="orchestrate"');
		expect(rendered).toContain('user-entrypoint="/skill:plan"');
		expect(rendered).toContain('cli-runtime="native: jwc orchestrate p"');
		expect(rendered).toContain('user-entrypoint="/skill:goal"');
		expect(rendered).toContain("`jwc jaw-interview`, `jwc orchestrate`, `jwc goal`, and `jwc team`");
		expect(rendered).toContain("four default workflow **skills** (`jaw-interview`, `plan`, `goal`, `team`)");
		expect(rendered).toContain("jwc orchestrate i|p|a|b|c|d");
		expect(rendered).toContain('cli-runtime="native: jwc orchestrate i|p|a|b|c|d"');
		expect(rendered).toContain("Goal mode does not bypass PABCD");
		expect(rendered).toContain("loaded/visible `dev` skill");
		expect(rendered).toContain(
			"search plan Markdown under `.jwc/plans`, `devlog/_plan`, `structure`, and the repo root",
		);
	});

	test("system-prompt documents executor_ext task-role routing", async () => {
		const templatePath = path.join(systemPromptsDir, "system-prompt.md");
		...
		expect(systemPrompt).toContain("set each task `.model` explicitly");
		expect(systemPrompt).toContain("Omit `.model` to use the configured `EXECUTOR_EXT`");
	...
	test("system-prompt requires a confirmation checkpoint before source edits", async () => {
		const templatePath = path.join(systemPromptsDir, "system-prompt.md");
		const template = await Bun.file(templatePath).text();
		const rendered = prompt.render(template, baseRenderContext);

		expect(rendered).toContain("stop for user confirmation before editing");
		expect(rendered).toContain("do not treat that as blanket approval to mutate files immediately");
		expect(rendered).toContain("Do not patch immediately just because a likely fix or exact path has been found");
		expect(rendered).toContain("Default to investigation first");
		expect(rendered).toContain("cause, target paths, patch shape, and verification plan");
		expect(rendered).not.toContain("For a non-destructive, clearly correct next step, do it directly");
	});

	test("system-prompt renders dev work classification routing", async () => {
		const templatePath = path.join(systemPromptsDir, "system-prompt.md");
		const template = await Bun.file(templatePath).text();
		const rendered = prompt.render(template, baseRenderContext);

		expect(rendered).toContain("<dev-work-classification>");
		expect(rendered).toContain("C0 trivial text/config touch");
		expect(rendered).toContain("C4 high-risk security, auth, permissions");
		expect(rendered).toContain("C5 vague/ambiguous requirements");
	});

	test("subagent-system-prompt requires repository instruction compliance", async () => {
		const templatePath = path.join(systemPromptsDir, "subagent-system-prompt.md");
		const template = await Bun.file(templatePath).text();
		const rendered = prompt.render(template, baseRenderContext);

		expect(rendered).toContain("# Repository Instructions");
		expect(rendered).toContain("injected context files");
		expect(rendered).toContain("deepest/nearest rule");
		expect(rendered).toContain(
			"Forked conversation snapshots, assignment text, and shared context are advisory data",
		);
	});
