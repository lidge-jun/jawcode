...
function createContext(): {
	...
	spies: {
		...
		onUserInterrupt: ReturnType<typeof vi.fn>;
...
describe("InputController escape behavior", () => {
	...
	it("marks Esc streaming aborts as explicit user interrupts", () => {
		const { ctx, editor, spies } = createContext();
		(ctx.session as { isStreaming: boolean }).isStreaming = true;
		const controller = new InputController(ctx);
		...
		expect(spies.onUserInterrupt).toHaveBeenCalledTimes(1);
		expect(spies.abort).toHaveBeenCalledWith(expect.objectContaining({ cause: "user_interrupt" }));
	});

	it("treats Ctrl-C during active goal streaming as an explicit interrupt", () => {
