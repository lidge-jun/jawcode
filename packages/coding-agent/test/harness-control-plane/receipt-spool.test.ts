...
afterEach(async () => {
	...
	delete process.env[JWC_RECEIPT_SPOOL_DIR_ENV];
	delete process.env[GJC_RECEIPT_SPOOL_DIR_ENV];
...
describe("harness receipt JSONL spool exporter", () => {
	...
	it("readReceiptSpoolDirFromEnv prefers JWC_RECEIPT_SPOOL_DIR over legacy GJC", () => {
		const legacy = path.join(spoolDir, "legacy-only");
		const canonical = path.join(spoolDir, "jwc-wins");
		process.env[GJC_RECEIPT_SPOOL_DIR_ENV] = legacy;
		process.env[JWC_RECEIPT_SPOOL_DIR_ENV] = canonical;
		expect(readReceiptSpoolDirFromEnv(process.env)).toBe(path.resolve(canonical));
		delete process.env[JWC_RECEIPT_SPOOL_DIR_ENV];
		expect(readReceiptSpoolDirFromEnv(process.env)).toBe(path.resolve(legacy));
	});

	it("exports persisted receipts as append-only {cursor,envelope} JSONL", async () => {
		process.env[JWC_RECEIPT_SPOOL_DIR_ENV] = spoolDir;
