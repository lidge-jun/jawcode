...
`,
	...
	describe("bash tool", () => {
		...
		it("should register background as discoverable but not essential", async () => {
			const autoBackgroundSession = createTestToolSession(
				testDir,
				Settings.isolated({ "bash.autoBackground.enabled": true }),
			);

			const backgroundTool = BackgroundTool.createIf(autoBackgroundSession);
			expect(backgroundTool).not.toBeNull();
			expect(backgroundTool?.loadMode).toBe("discoverable");
			expect((await BUILTIN_TOOLS.background(autoBackgroundSession))?.name).toBe("background");
			expect(DEFAULT_ESSENTIAL_TOOL_NAMES).not.toContain("background");

			const tools = await createTools(autoBackgroundSession, ["background"]);
			expect(tools.some(tool => tool.name === "background")).toBe(true);
		});

		it("should keep background availability consistent with job support gating", () => {
			const disabledSession = createTestToolSession(testDir, Settings.isolated({ "async.enabled": false }));

			expect(Boolean(BackgroundTool.createIf(disabledSession))).toBe(Boolean(JobTool.createIf(disabledSession)));
		});
	});
