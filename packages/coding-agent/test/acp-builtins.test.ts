...
import { GOAL_PLAN_PENDING_BRIEF } from "../src/goals/goal-planning-start";
import type { GoalModeState } from "../src/goals/state";
import { readGoalPlan } from "../src/jwc-runtime/goal-engine";
...
interface FakeAcpBuiltinSession {
	...
	_goalModeState: GoalModeState | undefined;
...
function createRuntime() {
	...
	const session: FakeAcpBuiltinSession = {
		...
		getGoalModeState() {
			return this._goalModeState;
		},
		setGoalModeState(state: GoalModeState | undefined) {
			this._goalModeState = state;
		},
		goalRuntime: {
			async createGoal(input: { objective: string }) {
				...
				session._goalModeState = state;
				return state;
			},
			async replaceGoal(input: { objective: string }) {
				const current = session._goalModeState;
			...
			async pauseGoal() {
				...
				const state: GoalModeState = {
					...current,
					enabled: false,
					goal: { ...current.goal, status: "paused", updatedAt: Date.now() },
				};
				session._goalModeState = state;
				return state;
			},
			async resumeGoal() {
				const current = session._goalModeState;
				const now = Date.now();
				const state: GoalModeState = current
					? {
							...current,
							enabled: true,
			...
			async dropGoal() {
				session._goalModeState = undefined;
			},
		},
...
describe("ACP builtin slash commands", () => {
	...
	it("advertises goal commands in ACP/text mode", () => {
		const names = ACP_BUILTIN_SLASH_COMMANDS.map(command => command.name);

		expect(names).toContain("goal");
		expect(names).toContain("goalplan");
	});

	it("routes /goal, /goal plan, /goalplan, and /goal-plan with planning semantics", async () => {
		const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "acp-goal-plan-"));
		try {
			const { runtime, session, fakeSessionManager } = createRuntime();
			runtime.cwd = cwd;
			fakeSessionManager._cwd = cwd;

			const directResult = await executeAcpBuiltinSlashCommand("/goal ship import cleanup", runtime);
			expect(directResult).toEqual({ prompt: "ship import cleanup" });
			expect(session._goalModeState?.goal.objective).toBe("ship import cleanup");

			session._goalModeState = undefined;
			session._activeToolNames = [];
			const planResult = await executeAcpBuiltinSlashCommand("/goal plan ship import cleanup", runtime);
			expect(planResult).toEqual(
				expect.objectContaining({ prompt: expect.stringContaining("AI-driven goal planning") }),
			);
			const planState = session.getGoalModeState();
			expect(planState?.goal.objective).toContain(GOAL_PLAN_PENDING_BRIEF);
			expect(planState?.goal.objective).toContain("hint: ship import cleanup");
			expect(planState?.goal.objective).not.toBe("plan ship import cleanup");

			session._goalModeState = undefined;
			session._activeToolNames = [];
			const goalplanResult = await executeAcpBuiltinSlashCommand("/goalplan ship import cleanup", runtime);
			expect(goalplanResult).toEqual(
				expect.objectContaining({ prompt: expect.stringContaining("AI-driven goal planning") }),
			);
			const goalplanState = session.getGoalModeState();
			expect(goalplanState?.goal.objective).toContain(GOAL_PLAN_PENDING_BRIEF);
			expect(goalplanState?.goal.objective).toContain("hint: ship import cleanup");

			session._goalModeState = undefined;
			session._activeToolNames = [];
			const dashedResult = await executeAcpBuiltinSlashCommand("/goal-plan ship import cleanup", runtime);
			expect(dashedResult).toEqual(
				expect.objectContaining({ prompt: expect.stringContaining("AI-driven goal planning") }),
			);
			const dashedState = session.getGoalModeState();
			expect(dashedState?.goal.objective).toContain(GOAL_PLAN_PENDING_BRIEF);
			expect(dashedState?.goal.objective).toContain("hint: ship import cleanup");
		} finally {
			await fs.rm(cwd, { recursive: true, force: true });
		}
	});

	it("replacement goal commands replace active mode and rewrite durable planning files", async () => {
		const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "acp-goal-plan-replace-"));
		try {
			const { output, runtime, session, fakeSessionManager } = createRuntime();
			runtime.cwd = cwd;
			fakeSessionManager._cwd = cwd;
			await executeAcpBuiltinSlashCommand("/goal already active", runtime);
			const originalId = session.getGoalModeState()?.goal.id;

			const directResult = await executeAcpBuiltinSlashCommand("/goal replacement objective", runtime);

			expect(directResult).toEqual({ prompt: "replacement objective" });
			expect(output).not.toContain(
				"Goal mode is already active. Use /goal to manage it, or /goal drop to start over.",
			);
			expect(session.getGoalModeState()?.goal.objective).toBe("replacement objective");
			expect(session.getGoalModeState()?.goal.id).not.toBe(originalId);
			expect((await readGoalPlan(cwd))?.brief).toBe("replacement objective");

			const goalplanResult = await executeAcpBuiltinSlashCommand("/goal-plan should replace hint", runtime);
			...
			expect(session.getGoalModeState()?.goal.objective).toContain("hint: should replace hint");
			expect((await readGoalPlan(cwd))?.brief).toContain("hint: should replace hint");

			const compactAliasResult = await executeAcpBuiltinSlashCommand("/goalplan compact alias hint", runtime);

			expect(compactAliasResult).toEqual(
				expect.objectContaining({ prompt: expect.stringContaining("AI-driven goal planning") }),
			);
			expect(output).not.toContain(
				"Goal mode is already active. Use /goal to manage it, or /goal drop to start over.",
			);
			expect(session.getGoalModeState()?.goal.objective).toContain("hint: compact alias hint");
			expect((await readGoalPlan(cwd))?.brief).toContain("hint: compact alias hint");
		} finally {
			await fs.rm(cwd, { recursive: true, force: true });
		}
	});
