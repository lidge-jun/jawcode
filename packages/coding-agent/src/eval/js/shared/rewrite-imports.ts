...
// `__jwc_import__` helper, which resolves the specifier against the active session cwd. A real
...
function buildJwcImportCall(sourceLiteral: string, optionsLiteral: string | undefined): string {
	// Route every static import through the worker-injected `__jwc_import__` helper so the
	// specifier resolves against the session cwd (and `with`-attribute imports keep working).
	return optionsLiteral ? `__jwc_import__(${sourceLiteral}, ${optionsLiteral})` : `__jwc_import__(${sourceLiteral})`;
}
...
function buildOptionsLiteral(node: BabelImportDeclaration): string | undefined {
	...
	// Native dynamic import takes options as `{ with: { ... } }`. `__jwc_import__` forwards the
...
export function rewriteImports(code: string): string {
	...
	// Top-level static `import` declarations become `await __jwc_import__(...)` calls.
	...
	// Dynamic `import(...)` expressions (anywhere) get their callee swapped for `__jwc_import__`
	...
	walkNodes(ast, node => {
		...
		edits.push({ start: callee.start, end: callee.end, text: "__jwc_import__" });
	});
