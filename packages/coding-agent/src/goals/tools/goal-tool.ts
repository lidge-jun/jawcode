...
import { formatNumber, prompt } from "@jawcode-dev/utils";
...
const goalSchema = z.object({
	op: z
		...
		.describe(
			"op: get | create | complete | drop | resume | pause — drop clears the active goal without exiting goal mode (tool stays callable for the next create); pause suspends the goal and stops continuation steers",
		),
