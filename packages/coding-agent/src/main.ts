...
export async function submitInteractiveInput(
	...
	try {
		...
		if (input.customType) {
			...
			mode.prepareRealUserAgentPromptSubmission();
...
interface RunRootCommandDependencies {
	...
	runPrintMode?: typeof runPrintMode;
	runBridgeMode?: typeof runBridgeMode;
	postmortemQuit?: typeof postmortem.quit;
...
export async function runRootCommand(
	...
	const buildSessionOptionsImpl = deps.buildSessionOptions ?? buildSessionOptions;
	const { options: sessionOptions } = await logger.time(
		"buildSessionOptions",
		buildSessionOptionsImpl,
		parsedArgs,
		scopedModels,
		sessionManager,
		modelRegistry,
	...
	// children running without a session to dispose them.
	...
	const applyStartupModelProfilesImpl = deps.applyStartupModelProfiles ?? applyStartupModelProfiles;
	const runPrintModeImpl = deps.runPrintMode ?? runPrintMode;
	const runBridgeModeImpl = deps.runBridgeMode ?? runBridgeMode;
	const postmortemQuit = deps.postmortemQuit ?? postmortem.quit;
	...
	if (mode === "acp") {
		...
		try {
			...
			sessionCreated = true;
			const { setToolUIContext, modelFallbackMessage, lspServers, mcpManager, eventBus } = result;
			...
			try {
				await applyStartupModelProfilesImpl({
					session,
					settings: settingsInstance,
					modelRegistry,
					parsedArgs,
			...
			if (mode === "rpc" || mode === "rpc-ui") {
				...
				await runBridgeModeImpl(session, setToolUIContext);
				...
				await runPrintModeImpl(session, {
					mode,
