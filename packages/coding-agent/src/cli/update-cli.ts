 ...
 *
 * Handles `jwc update` to check for and install updates.
 * Uses bun if available, otherwise downloads binary from GitHub releases.
 */
import * as fs from "node:fs";
import * as path from "node:path";
import { pipeline } from "node:stream/promises";
import { $which, APP_NAME, isEnoent, VERSION } from "@jawcode-dev/utils";
import { $ } from "bun";
import chalk from "chalk";
import { theme } from "../modes/theme/theme";

const RELEASE_REPO = "lidge-jun/jawcode";
const PACKAGE = "jawcode";
...
function isPathInDirectory(filePath: string, directoryPath: string): boolean {
	...
	// PATH-resolved jwc path can refer to the same directory through different
...
/**
 * Resolve the path that `jwc` maps to in the user's PATH.
 */
...
/**
 * Run the resolved jwc binary and check if it reports the expected version.
 */
...
export async function runUpdateCommand(opts: { force: boolean; check: boolean }): Promise<void> {
	...
	// Choose update method based on the prioritized jwc binary in PATH
	try {
		...
		// Propagate updated bundled defaults (skills, keybindings, themes)
		try {
			console.log(chalk.dim("Propagating updated defaults..."));
			await $`${APP_NAME} setup defaults`.quiet();
		} catch {
			console.log(
				chalk.yellow(`Note: run \`${APP_NAME} setup defaults\` manually to update bundled workflow skills.`),
			);
		}
	} catch (err) {
