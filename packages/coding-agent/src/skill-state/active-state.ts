...
export function listActiveSkills(raw: unknown): SkillActiveEntry[] {
	...
	const topLevelSkill = normalizeWorkflowSkillSlug(safeString(state.skill).trim());
...
export function normalizeSkillActiveState(raw: unknown): SkillActiveState | null {
	...
	const rawSkill = normalizeWorkflowSkillSlug(safeString(state.skill).trim());
	const primary = activeSkills.find(entry => entry.skill === rawSkill) ?? activeSkills[0];
	const skill = rawSkill || primary?.skill || "";
...
function rawActiveEntries(state: SkillActiveState | null): SkillActiveEntry[] {
	...
	if (out.length === 0 && state.active === true) {
		const skill = normalizeWorkflowSkillSlug(safeString(state.skill).trim());
		if (skill) {
			out.push({
				skill,
				phase: safeString(state.phase).trim() || undefined,
 ...
 * plan → goal`. Each stage is activated through its own command path
 * (`jwc jaw-interview`, `jwc orchestrate p`, `jwc goal`), and those activations
 * do not demote the previous stage's row — only the explicit `handoff` verb
 * does. Without this collapse, activating goal while plan is still
 ...
 * excluded — it runs alongside goal — and every non-pipeline skill is left
 * untouched.
 *
 ...
 */
const PLANNING_PIPELINE_SKILLS = new Set<string>(["jaw-interview", "plan", "goal"]);
...
export async function readVisibleSkillActiveState(cwd: string, sessionId?: string): Promise<SkillActiveState | null> {
	...
	return buildVisibleSkillActiveState({ ...(rootState ?? {}), ...(sessionState ?? {}) }, activeSkills, sessionId);
}

export async function readSessionStrictSkillActiveState(
	cwd: string,
	sessionId?: string,
