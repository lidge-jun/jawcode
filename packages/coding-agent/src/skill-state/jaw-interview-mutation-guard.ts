...
export const JAW_INTERVIEW_MUTATION_BLOCK_MESSAGE =
	"Jaw-interview phase boundary: continue gathering context/questions/risks and emit a handoff/spec before code edits. Non-interview-document mutation tools and patch execution are blocked while jaw-interview is active; `.md` files and mockup `.html` files may be created or updated when governed by the active interview prompt.";
...
const HTML_MOCKUP_PATH_RE = /(?:^|[/_.-])(?:mockups?|wireframes?|prototypes?)(?:[/_.-]|$)/i;
...
function extractHashlineTargets(args: unknown, targets: ExtractedTargets): boolean {
	const record = getRecord(args);
	const input = record?.input;
	if (typeof input !== "string") return false;
	...
	for (const line of input.split(/\r?\n/)) {
		...
		const rawPath = line.slice(1).trim();
		if (!rawPath) continue;
		sawSection = true;
		addPath(targets, rawPath);
	}
...
function extractEditTargets(args: unknown, tool: ToolWithEditMode): ExtractedTargets {
	...
	const hasInput = typeof record?.input === "string";
	if (isApplyPatchMode) {
		extractApplyPatchTargets(args, targets);
		if (targets.paths.length === 0) targets.unknown = true;
		return targets;
	}
	if (hasInput && extractHashlineTargets(args, targets)) {
		return targets;
	}
...
function isInterviewDocumentPath(cwd: string, rawPath: string): boolean {
	const { absolutePath, unknown } = resolveRawPath(cwd, rawPath);
	if (unknown || !absolutePath) return false;
	const extension = path.extname(absolutePath).toLowerCase();
	if (extension === ".md") return true;
	if (extension !== ".html") return false;
	const normalizedRawPath = normalizePosix(rawPath);
	if (relativeJwcSegments(cwd, rawPath)?.[0] === ".jwc") return false;
	return HTML_MOCKUP_PATH_RE.test(normalizedRawPath);
}
function isBlockedJwcPath(cwd: string, rawPath: string): boolean {
	const segments = relativeJwcSegments(cwd, rawPath);
	if (segments?.[0] !== ".jwc") return false;
	if (segments[1] === "state") return true;
	return !isInterviewDocumentPath(cwd, rawPath);
}
...
function allTargetsInterviewDocuments(cwd: string, targets: ExtractedTargets): boolean {
	return (
		!targets.unknown &&
		targets.paths.length > 0 &&
		targets.paths.every(rawPath => isInterviewDocumentPath(cwd, rawPath))
	);
}
export async function assertJawInterviewMutationRawPathsAllowed(input: {
	...
	if (targets.unknown || !allTargetsInterviewDocuments(input.cwd, targets)) {
...
export async function getJawInterviewMutationDecision(
	...
	if (allTargetsInterviewDocuments(input.cwd, targets)) {
