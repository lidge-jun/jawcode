...
function removeKey(value: KeyId | KeyId[] | undefined, keyToRemove: KeyId): KeyId | KeyId[] | undefined {
	if (typeof value === "string") {
		return value.toLowerCase() === keyToRemove ? undefined : value;
	}
	if (Array.isArray(value)) {
		const filtered = value.filter(key => key.toLowerCase() !== keyToRemove);
...
function sanitizeTranscriptKeyConflicts(config: KeybindingsConfig): {
	config: KeybindingsConfig;
	sanitized: boolean;
} {
	const thinkingKeys = config["app.thinking.toggle"];
	const sanitizedThinkingKeys = removeKey(thinkingKeys, "ctrl+t");
	if (sanitizedThinkingKeys === thinkingKeys) {
		return { config, sanitized: false };
	}
	const sanitized = { ...config };
	if (sanitizedThinkingKeys === undefined) {
		delete sanitized["app.thinking.toggle"];
	} else {
		sanitized["app.thinking.toggle"] = sanitizedThinkingKeys;
	}
	return { config: sanitized, sanitized: true };
}
...
function loadKeybindingsConfig(filePath: string, writeBack: boolean): KeybindingsConfig {
	...
	const { config: sanitizedConfig, sanitized } = sanitizeTranscriptKeyConflicts(migratedConfig);
	if (writeBack && (migrated || sanitized)) {
		const ordered = orderKeybindingsConfig(sanitizedConfig);
		try {
...
export class KeybindingsManager extends TuiKeybindingsManager {
	...
	constructor(userBindings: KeybindingsConfig = {}, configPath?: string) {
		super(KEYBINDINGS, sanitizeTranscriptKeyConflicts(userBindings).config);
	...
	static inMemory(userBindings: KeybindingsConfig = {}): KeybindingsManager {
		return new KeybindingsManager(sanitizeTranscriptKeyConflicts(userBindings).config);
	}
