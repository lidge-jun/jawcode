 ...
 * Adapted from GJC config/file-lock-gc.ts for the JWC flat layout + imports.
 */

import type { Stats } from "node:fs";
import * as fs from "node:fs/promises";
import * as path from "node:path";
import { getAgentDir, getConfigRootDir, isEnoent } from "@jawcode-dev/utils";
...
import { gcPidStatusLabel } from "../jwc-runtime/gc-runtime";
...
// Global, env-aware JWC lock roots. Per the approved scope this covers the
// user config root, the agent dir, and the configured receipt-spool dir —
// NOT the invocation cwd's project config dir.
...
export const fileLocksGcAdapter: GcStoreAdapter = {
	...
	async prune(record: GcRecord, ctx: GcContext): Promise<GcPruneOutcome> {
		...
		// Fail-closed owner-token guard (GJC #606): we observed `info` (pid+timestamp)
