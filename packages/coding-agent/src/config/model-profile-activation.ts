...
import { isAuthenticated, type ModelRegistry } from "./model-registry";
...
export async function prepareModelProfileActivation(
	...
	for (const [role, selector] of Object.entries(bindings.agentModelOverrides) as [string, string][]) {
...
export async function applyPreparedModelProfileActivation(
	...
	try {
		...
		if (prepared.agentModelOverrides.executor_ext !== undefined) delete nextAgentModelOverrides.executor;
