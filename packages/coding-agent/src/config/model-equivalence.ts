import { type Api, getBundledModels, getBundledProviders, type Model } from "@jawcode-dev/ai";
...
function getClaudeFamilyAliasOfficial(candidate: string, officialIds: Set<string>): string | undefined {
	...
	for (const officialId of officialIds) {
		...
		// Skip bracket-suffixed variants like [1m] — they are separate selector ids,
		// not candidates for the family alias resolution.
		if (officialId.includes("[")) {
			continue;
		}
