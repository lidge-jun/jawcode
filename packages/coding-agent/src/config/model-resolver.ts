...
const SESSION_INHERIT_MODEL_ROLE = "self";
...
function isSessionInheritedAgentPattern(value: string): boolean {
	return (
		value === SESSION_INHERIT_MODEL_ROLE ||
		value === DEFAULT_MODEL_ROLE ||
		value === `${PREFIX_MODEL_ROLE}${DEFAULT_MODEL_ROLE}`
	);
}
...
function resolveConfiguredRolePattern(value: string, settings?: Settings): string[] | undefined {
	...
	if (normalized === SESSION_INHERIT_MODEL_ROLE) return undefined;
...
export function resolveModelRoleValue(
	...
	if (!normalized || normalized === SESSION_INHERIT_MODEL_ROLE || normalized === DEFAULT_MODEL_ROLE) {
		return { model: undefined, thinkingLevel: undefined, explicitThinkingLevel: false, warning: undefined };
	}
...
/**
 * Resolve a list of override patterns to the first matching authenticated model,
 * with an auth-aware fallback to the parent session's active model.
 *
 * Later override patterns are fallback candidates for earlier unresolved or
 * unauthenticated selectors. Task dispatch can therefore try a one-off model
 * first, then the configured role target, then the parent active model. This
 * prevents subagent dispatch from silently routing to a provider/model the user
 * can't actually call (e.g. `modelRoles.task` pointing at an unqualified id
 * whose only available provider variant has no configured credentials — see
 * #985) and prevents unresolved task-role selectors from falling through to the
 * generic session default.
 *
 ...
 * provider (see #1008).
 *
 * If neither the subagent nor the parent has working auth, returns the first
 * resolved primary model unchanged so the existing error path still surfaces a
 * meaningful failure downstream.
 */
export async function resolveModelOverrideWithAuthFallback(
	...
	let primary: { model?: Model<Api>; thinkingLevel?: ThinkingLevel; explicitThinkingLevel: boolean } | undefined;
	const availableModels = modelRegistry.getAvailable();
	const matchPreferences = { usageOrder: settings?.getStorage()?.getModelUsageOrder() };
	for (const pattern of modelPatterns) {
	...
	primary ??= { explicitThinkingLevel: false };
...
export async function resolveModelScope(
	...
	for (const pattern of patterns) {
		// Check if pattern contains glob characters. Bracket-only patterns like
		// "claude-opus-4-6[1m]" are model ids, not globs — require * or ? for glob path.
