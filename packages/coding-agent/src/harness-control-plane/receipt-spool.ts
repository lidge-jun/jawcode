...
/** Canonical jwc harness env (008 D4 — user-facing docs prefer JWC_*). */
export const JWC_RECEIPT_SPOOL_DIR_ENV = "JWC_RECEIPT_SPOOL_DIR";
/** Legacy gjc harness env; still read and written for upstream/tooling compatibility. */
export const GJC_RECEIPT_SPOOL_DIR_ENV = "GJC_RECEIPT_SPOOL_DIR";
/** Owner RPC input field name (upstream wire shape). */
export const RECEIPT_SPOOL_DIR_ENV = GJC_RECEIPT_SPOOL_DIR_ENV;
...
export function assignReceiptSpoolDirEnv(env: NodeJS.ProcessEnv, spoolDir: string): void {
	const trimmed = spoolDir.trim();
	if (!trimmed) throw new Error("receipt_spool_dir_empty");
	const resolved = path.resolve(trimmed);
	env[JWC_RECEIPT_SPOOL_DIR_ENV] = resolved;
	env[GJC_RECEIPT_SPOOL_DIR_ENV] = resolved;
}

export function readReceiptSpoolDirFromEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
	const raw = env[JWC_RECEIPT_SPOOL_DIR_ENV]?.trim() ?? env[GJC_RECEIPT_SPOOL_DIR_ENV]?.trim();
...
export function resolveReceiptSpoolDir(env: NodeJS.ProcessEnv = process.env): string | undefined {
	...
	return readReceiptSpoolDirFromEnv(env);
}
