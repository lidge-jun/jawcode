...
async function readJsonFile(filePath: string): Promise<unknown> {
	try {
		...
		if (error instanceof SyntaxError) {
			throw new JwcPluginLoadError("invalid_manifest", `Invalid GJC plugin manifest JSON at ${filePath}`, {
				cause: error,
			});
		}
		throw new JwcPluginLoadError("missing_file", `Missing GJC plugin manifest at ${filePath}`, {
...
async function readRequiredText(filePath: string, kind: "sub-skill" | "tool"): Promise<string> {
	try {
		...
		throw new JwcPluginLoadError("missing_file", `Missing GJC plugin ${kind} file at ${filePath}`, {
			cause: error instanceof Error ? error : undefined,
		});
	}
}
...
export async function loadJwcPlugin(root: string): Promise<LoadedJwcPlugin> {
	...
	for (const rel of manifest.subskills) {
		...
		try {
			...
			throw new JwcPluginLoadError("invalid_frontmatter", `Invalid GJC sub-skill frontmatter at ${filePath}`, {
				cause: error instanceof Error ? error : undefined,
			});
		}
...
export async function loadJwcPlugins(roots: readonly string[]): Promise<LoadedJwcPlugin[]> {
	const plugins: LoadedJwcPlugin[] = [];
	for (const root of roots) {
		plugins.push(await loadJwcPlugin(root));
	}
