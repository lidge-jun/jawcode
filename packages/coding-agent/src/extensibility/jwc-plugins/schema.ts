import { GJC_PLUGIN_KIND, JwcPluginLoadError, type JwcPluginManifest, type SubskillFrontmatter } from "./types";
...
export function parseManifest(raw: unknown, manifestPath: string): JwcPluginManifest {
	if (!isRecord(raw)) {
		throw new JwcPluginLoadError(
			"invalid_manifest",
	...
	for (const key of FORBIDDEN_MANIFEST_KEYS) {
		if (Object.hasOwn(raw, key)) {
			throw new JwcPluginLoadError("forbidden_surface", `Forbidden GJC plugin surface in ${manifestPath}: ${key}`);
		}
	}
