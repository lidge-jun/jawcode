...
export async function resolveSubskillActivationForSkillInvocation(input: {
	...
	const roots = await discoverJwcPluginRoots({ cwd: input.cwd });
	let plugins: LoadedJwcPlugin[];
	try {
		plugins = await loadJwcPlugins(roots);
	} catch (error) {
		if (error instanceof JwcPluginLoadError) throw error;
