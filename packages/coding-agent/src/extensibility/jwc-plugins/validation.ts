import { isKnownWorkflowState } from "../../jwc-runtime/workflow-manifest";
...
import {
	...
	JwcPluginLoadError,
	type JwcSubskillParentAgent,
	type LoadedSubskillBinding,
	type SubskillFrontmatter,
} from "./types";

function isParentSkill(value: string): value is CanonicalJwcWorkflowSkill {
	return (GJC_SUBSKILL_PARENT_SKILLS as readonly string[]).includes(value);
}

function isParentAgent(value: string): value is JwcSubskillParentAgent {
...
export function validateBinding(fm: SubskillFrontmatter): void {
	...
	if (isParentSkill(parent)) {
		if (!isKnownWorkflowState(parent, fm.phase)) {
			throw new JwcPluginLoadError("invalid_phase", `Invalid GJC sub-skill phase for ${parent}: ${fm.phase}`);
		}
		return;
	}
	...
	throw new JwcPluginLoadError("invalid_parent", `Invalid GJC sub-skill parent: ${parent}`);
}
