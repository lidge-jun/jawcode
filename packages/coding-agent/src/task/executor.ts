...
/** Clamp a frozen parent seed to the child model's actual input budget. */
export function trimForkContextSeedForModel(seed: ForkContextSeed, model: Model | undefined): ForkContextSeed {
	...
	const reservedOutputTokens = Math.max(
		...
		Number.isFinite(model.maxTokens) ? Math.max(0, Math.trunc(model.maxTokens)) : 0,
	);
	...
	for (let index = seed.messages.length - 1; index >= 0; index--) {
		const message = seed.messages[index]!;
		const tokens = estimateMessageTokensHeuristic(message as AgentMessage);
...
export interface ExecutorOptions {
	...
	cacheAffinity?: SingleResult["cacheAffinity"];
}
...
export async function runSubprocess(options: ExecutorOptions): Promise<SingleResult> {
	...
	const runSubagent = async (): Promise<{
		...
		try {
			...
			if (agent.name === "executor_ext" && modelPatterns.length > 0 && !model) {
				const requested = modelPatterns.join(", ");
				const parent = options.parentActiveModelPattern ?? "(none)";
				throw new Error(
					`executor_ext model selector did not resolve to an authenticated model: ${requested}; parent active model: ${parent}. Refusing to fall back to the generic session default.`,
				);
			}
			...
			const forkContextNotice = forkContextSeed
