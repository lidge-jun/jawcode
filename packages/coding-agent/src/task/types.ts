...
import type { Usage } from "@jawcode-dev/ai";
...
export interface CacheAffinityMetadata {
	affine: boolean;
	reason: "self_fork" | "actor_resume" | "executor_ext" | "fork_fallback" | "prewarm_failed" | "not_applicable";
	prewarm?: "refresh" | "cold" | "skipped" | "failed";
}
...
const createTaskItemSchema = (_contextEnabled: boolean) =>
	z.object({
		...
		model: z
			...
			.optional()
			.describe(
				"model hint for this task: prefer explicit provider/modelId[:effort] for specific external models; executor_ext with omitted/empty model uses task.agentModelOverrides.executor_ext; self uses the parent model; cheap:<provider> or best:<provider> are preset lookups; bare/canonical ids may resolve but are less explicit",
			),
	});
...
export interface SingleResult {
	...
	contextWindow?: number;
	modelOverride?: string | string[];
	cacheAffinity?: CacheAffinityMetadata;
	error?: string;
	aborted?: boolean;
	abortReason?: string;
	paused?: boolean;
 ...
 * benchmark (`@jawcode-dev/orchestration-token-benchmark`) consumes to measure
