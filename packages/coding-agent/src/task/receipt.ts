...
function summarizeError(raw: SingleResult): string | undefined {
	if (!raw.error) return undefined;
	return raw.error === "context_unavailable" ? "context_unavailable" : "Error recorded.";
}
...
function buildSafeSynopsis(raw: SingleResult, outputRef: TaskResultReceipt["outputRef"]): string {
	...
	if (raw.error) {
		return `Task ${status}; ${summarizeError(raw)}.`;
	}
	if (outputRef) {
...
export function buildTaskReceipt(raw: SingleResult): TaskResultReceipt {
	...
	return {
		...
		retryFailure: raw.retryFailure
			...
			: undefined,
		errorSummary: summarizeError(raw),
		...
		cacheAffinity: raw.cacheAffinity,
