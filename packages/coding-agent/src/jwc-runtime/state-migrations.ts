...
export async function migrateAndPersistLegacyState(
	...
	if (!changed) return { migrated: false, path: args.persistPath ?? args.statePath };

	const writePath = args.persistPath ?? args.statePath;
	const persistedPath = await writeWorkflowEnvelopeAtomic(writePath, state, {
		cwd: args.cwd,
		receipt: {
			cwd: args.cwd,
