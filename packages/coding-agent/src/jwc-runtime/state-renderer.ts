...
export function renderGoalStatusMarkdown(summary: {
	...
	if (!summary.exists)
		return `# goal status\n\n- status: missing\n- No goal plan found at ${summary.paths.goalsPath}. Run \`jwc goal create-goals --brief "..."\` first.\n`;
	const counts = Object.entries(summary.counts)
		.map(([key, value]) => `${key}=${value}`)
		.join(" ");
	const lines = ["# goal status", "", `- status: ${summary.status}`, `- goals: ${summary.goals.length} (${counts})`];
