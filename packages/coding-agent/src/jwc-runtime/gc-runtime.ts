/**
 * `jwc gc` runtime — a global, liveness-only, dry-run-by-default garbage
 * collector for stale JWC session/PID records.
 *
 * Design (adapted from GJC gjc-runtime/gc-runtime.ts):
...
export interface GcStoreCounts {
	discovered: number;
	stale: number;
	would_remove: number;
	removed: number;
...
export interface GcCounts {
	...
	failed: number;
	errors: number;
	by_store: Record<ActiveGcStore, GcStoreCounts>;
}

export interface GcReport {
	dry_run: boolean;
	stores: Record<ActiveGcStore, GcRecord[]>;
	counts: GcCounts;
	errors: GcError[];
}
...
function emptyByStore(): Record<ActiveGcStore, GcStoreCounts> {
	const by = {} as Record<ActiveGcStore, GcStoreCounts>;
...
function emptyStores(): Record<ActiveGcStore, GcRecord[]> {
	const stores = {} as Record<ActiveGcStore, GcRecord[]>;
	for (const store of GC_STORES) stores[store] = [];
	return stores;
}

function computeCounts(stores: Record<ActiveGcStore, GcRecord[]>, errors: GcError[]): GcCounts {
...
export async function runJwcGcCommand(
	argv: string[],
	...
	try {
		...
		return { stdout: "", stderr: `jwc gc: ${message}\n`, status: 2 };
	}
	...
	const resolvedAdapters = adapters ?? defaultGcAdapters();
...
export function gcHelpText(): string {
	return [
		"jwc gc - garbage-collect stale JWC session/PID records",
		"",
		"USAGE",
		"  $ jwc gc [--prune|--force] [--json]",
		"",
		"FLAGS",
