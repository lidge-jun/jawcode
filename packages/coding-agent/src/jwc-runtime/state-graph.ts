...
export type StateGraphSkill = CanonicalJwcWorkflowSkill | "all";
...
function skillsFor(skill: StateGraphSkill): CanonicalJwcWorkflowSkill[] {
	return skill === "all" ? [...CANONICAL_JWC_WORKFLOW_SKILLS] : [skill];
}
...
function dotId(skill: CanonicalJwcWorkflowSkill, state: string): string {
	return `"${skill}:${state}"`;
}
