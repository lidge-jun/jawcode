...
export interface StateWriterAuditContext {
	...
	verb: string;
	owner: WorkflowStateMutationOwner;
	skill?: CanonicalJwcWorkflowSkill | string;
	mutationId?: string;
	fromPhase?: string;
	toPhase?: string;
	forced?: boolean;
...
export interface WorkflowTransactionJournal {
	...
	created_at: string;
	updated_at: string;
	caller?: CanonicalJwcWorkflowSkill;
	callee?: CanonicalJwcWorkflowSkill;
	paths: string[];
	steps: string[];
}
...
function resolveJwcTarget(targetPath: string, cwd = process.cwd()): string {
...
export async function writeArtifact(
	...
	return writeTextAtomic(targetPath, content, {
		...options,
		audit: options?.audit ?? { category: "artifact", verb: "write", owner: "jwc-runtime" },
	});
}
...
export async function writeReport(targetPath: string, content: string, options?: StateWriterOptions): Promise<string> {
	return writeTextAtomic(targetPath, content, {
		...options,
		audit: options?.audit ?? { category: "report", verb: "write", owner: "jwc-runtime" },
	});
}
...
export async function writeLogJsonl(targetPath: string, entry: unknown, options?: StateWriterOptions): Promise<string> {
	return appendJsonl(targetPath, entry, {
		...options,
		audit: options?.audit ?? { category: "log", verb: "append", owner: "jwc-runtime" },
	});
}
...
export async function softDelete(
	...
	return updateJsonAtomic<Record<string, unknown>>(
		...
		{
			...options,
			audit: options?.audit ?? { category: "prune", verb: "soft-delete", owner: "jwc-runtime" },
		},
	);
}
...
export async function hardPrune(
	...
	for (const target of targets) {
		const filePath = resolveJwcTarget(target.path, cwd);
...
export async function forceOverwrite(
	...
	const auditOptions = {
		...options,
		audit: options?.audit ?? { category: "force", verb: "force-overwrite", owner: "jwc-state-cli", forced: true },
	};
	if (options?.raw === true) {
		const filePath = resolveJwcTarget(targetPath, cwdForOptions(options));
...
export async function appendAuditEntry(cwd: string, entry: AuditEntry): Promise<string> {
	const filePath = resolveJwcTarget(path.join(".jwc", "state", "audit.jsonl"), cwd);
	await fs.mkdir(path.dirname(filePath), { recursive: true });
	await fs.appendFile(filePath, `${JSON.stringify(entry)}\n`, "utf-8");
	return filePath;
}
