...
import { buildPlanphaseHudSummary } from "../skill-state/workflow-hud";
...
/**
 * Native implementation of plan-phase writer (jwc planphase).
 *
 * Two invocation shapes are handled natively:
 *
 * 1. **Consensus handoff**: `jwc planphase [--interactive] [--deliberate] [--architect <kind>]
 *    [--critic <kind>] [--session-id <id>] "<task>"` validates the documented flag surface,
 *    seeds `.jwc/state/planphase-state.json`, and updates the shared HUD rail via
 *    `syncSkillActiveState`. The CLI never *runs* the Planner / Architect / Critic loop itself —
 *    that lives in the bundled `/skill:plan` skill — but it accepts every documented flag so
 *    scripted users see a useful response and the active run is visible to the TUI.
 *
 * 2. **Artifact write**: `jwc planphase --write --stage <type> --stage_n <N> --artifact
 *    <path-or-string> [--run-id <id>] [--session-id <id>] [--json]` persists Planner / Architect
 *    / Critic / revision / ADR / final markdown under `.jwc/plans/planphase/<run-id>/`, maintains
...
export interface PlanphaseCommandResult {
	status: number;
	stdout?: string;
	stderr?: string;
}
...
type PlanphaseStage = (typeof KNOWN_STAGES)[number];
...
export function isPlanphaseArtifactWriteInvocation(args: readonly string[]): boolean {
...
function assertKnownStage(stage: string): asserts stage is PlanphaseStage {
...
async function persistActiveRunId(cwd: string, sessionId: string | undefined, runId: string): Promise<void> {
	...
	if (typeof existing.skill !== "string") existing.skill = "plan";
	...
	existing = migrateWorkflowState(existing, "plan").state;
	existing.updated_at = new Date().toISOString();
	await writeWorkflowEnvelopeAtomic(statePath, existing, {
		cwd,
		receipt: { cwd, skill: "plan", owner: "jwc-runtime", command: "jwc planphase persist-run-id", sessionId },
		audit: { category: "state", verb: "write", owner: "jwc-runtime", skill: "plan" },
	});
}
...
/**
 * Merge persisted-Planner metadata into the planphase run-state JSON. Same-session
...
async function applyPlannerStateUpdate(
	...
	await writeWorkflowEnvelopeAtomic(statePath, existing, {
		cwd,
		receipt: { cwd, skill: "plan", owner: "jwc-runtime", command: "jwc planphase planner-state", sessionId },
		audit: { category: "state", verb: "write", owner: "jwc-runtime", skill: "plan" },
	});
}
...
async function resolveArtifactArgs(args: readonly string[], cwd: string): Promise<ResolvedArtifactArgs> {
	...
	//   2. existing run_id field in .jwc/state[/sessions/<id>]/planphase-state.json
	...
	return { stage: stage as PlanphaseStage, stageN, runId, artifact, sessionId, json: hasFlag(args, "--json") };
}
...
async function persistArtifact(resolved: ResolvedArtifactArgs, cwd: string): Promise<PersistedArtifact> {
	const runDir = path.join(cwd, ".jwc", "plans", "planphase", resolved.runId);
	...
	await writeArtifact(filePath, content, {
		cwd,
		audit: { category: "artifact", verb: "write", owner: "jwc-runtime", skill: "plan" },
	});
	...
	await appendJsonl(path.join(runDir, "index.jsonl"), indexEntry, {
		cwd,
		audit: { category: "ledger", verb: "append", owner: "jwc-runtime", skill: "plan" },
	});
...
async function syncPlanphaseHud(options: {
	cwd: string;
	sessionId?: string;
	stage: string;
	...
	try {
		await syncSkillActiveState({
			...
			source: "jwc-planphase-native",
			hud: buildPlanphaseHudSummary({
				stage: options.stage,
...
async function handleArtifactWrite(args: readonly string[], cwd: string): Promise<PlanphaseCommandResult> {
...
async function seedPlanphaseState(
	cwd: string,
	...
	const statePath = path.join(stateDir, "planphase-state.json");
	// Reuse an existing run id when present so a re-invocation of `jwc planphase --write ...` doesn't
...
async function handleConsensusHandoff(args: readonly string[], cwd: string): Promise<PlanphaseCommandResult> {
	const resolved = resolveConsensusArgs(args);
	if (!resolved.task) {
		throw new PlanWriterCommandError(
			2,
			'planphase writer requires a task description, e.g. `jwc planphase "<task>"`.',
		);
	}
	const { statePath, runId } = await seedPlanphaseState(cwd, resolved);
...
export async function runNativePlanWriterCommand(args: string[], cwd = process.cwd()): Promise<PlanphaseCommandResult> {
	try {
		if (isPlanphaseArtifactWriteInvocation(args)) return await handleArtifactWrite(args, cwd);
...
export const runNativeRalplanCommand = runNativePlanWriterCommand; // deprecated compatibility alias
