...
If the work is unfinished, leave the goal active. Remaining independent features are the NEXT work-phases of this same run — re-enter planning (`jwc orchestrate p`) and chain them; "each needs its own cycle" never justifies completing. Context pressure is not budget exhaustion: checkpoint durable state (devlog, goal updates with evidence) and continue after compaction. If a stated usage limit is about to end the run, report the honest terminal state — best-so-far evidence plus the remaining gap — and never round a budget/time stop up to completion.
