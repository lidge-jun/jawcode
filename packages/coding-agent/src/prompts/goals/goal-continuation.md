...
- **Optimization objectives** (score/win-rate/benchmark maximization — the verifier defines *better*, never *done*): instrument before iterating — if the verifier only reports pass/fail or win/lose, make it emit measurements first. Start each cycle with an analysis deliverable (what did the opponent/environment actually do; what capability is the artifact missing), generate diverse candidates instead of repairing the incumbent, keep a best-so-far ledger, and record at every checkpoint what did NOT improve and which hypothesis died. Budget/time exhaustion is reported as budget-exhausted with best-so-far evidence — never as completion.
