...
- Manage resulting background rows with the `background` tool for list/detail/follow/cancel. The TUI footer rows (`bg … · ctrl+j`) and `background` tool rows describe the same background work.
- Keep commands in the foreground when they are interactive, require immediate output, or need a user decision.
{{/if}}
</instruction>
