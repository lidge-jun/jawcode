...
2. Spawn the auditors IN PARALLEL (dual) or Architect only (solo). Use the normal task tool call; the runtime may resume compatible A-stage audit actors (`a:planner-auditor`, `a:architect-auditor`) within the same PABCD namespace and retires them on stage transition:
   - ⚠️ Each lens MUST set the task tool's top-level `agent` field to its role agent — `agent: "planner"` for the Planner lens, `agent: "architect"` for the Architect lens. NEVER omit `agent` (it silently defaults to the generic `task` agent, which inherits the session model and bypasses the per-role model bindings in `task.agentModelOverrides`). The schema allows one `agent` per call, so dual mode is TWO task tool calls issued in the same message — one per lens — never one call with a mixed task array.
