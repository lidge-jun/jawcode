...
import {
	assignReceiptSpoolDirEnv,
	GJC_RECEIPT_SPOOL_DIR_ENV,
	JWC_RECEIPT_SPOOL_DIR_ENV,
	RECEIPT_SPOOL_DIR_ENV,
	readReceiptSpoolDirFromEnv,
} from "../harness-control-plane/receipt-spool";
...
export default class Harness extends Command {
	...
	static flags = {
		...
		"receipt-spool-dir": Flags.string({
			description:
				"Append persisted ReceiptEnvelope records to spool.jsonl under this directory (jwc harness; env: JWC_RECEIPT_SPOOL_DIR or legacy GJC_RECEIPT_SPOOL_DIR)",
		}),
	};
	...
	async run(): Promise<void> {
		...
		try {
			...
			if (receiptSpoolDir !== undefined) {
				...
				assignReceiptSpoolDirEnv(process.env, receiptSpoolDir.trim());
			}
	...
	#startTmuxResidentOwner(
		...
		const configuredSpoolDir = readReceiptSpoolDirFromEnv(process.env);
		if (configuredSpoolDir) {
			envAssignments.push(`${JWC_RECEIPT_SPOOL_DIR_ENV}=${shellQuote(configuredSpoolDir)}`);
			envAssignments.push(`${GJC_RECEIPT_SPOOL_DIR_ENV}=${shellQuote(configuredSpoolDir)}`);
		}
	...
	async #spawnDetachedOwner(root: string, sessionId: string, cwd: string): Promise<OwnerSpawnResult> {
		...
		const child = Bun.spawn(cmd, {
			...
			env: {
				...
				...(configuredSpoolDir
					? {
							[JWC_RECEIPT_SPOOL_DIR_ENV]: configuredSpoolDir,
							[GJC_RECEIPT_SPOOL_DIR_ENV]: configuredSpoolDir,
						}
					: {}),
	...
	async #tryOwnerRoute(
		...
		try {
			const configuredSpoolDir = readReceiptSpoolDirFromEnv(process.env);
			if (configuredSpoolDir) input[RECEIPT_SPOOL_DIR_ENV] = configuredSpoolDir;
