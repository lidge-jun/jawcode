...
function toolSchema(name: CoordinatorToolName): {
	...
	if (name === "jwc_coordinator_read_turn") {
		return {
			name,
	...
	if (name === "jwc_coordinator_await_turn") {
		return {
			name,
	...
	if (name === "jwc_coordinator_submit_question_answer") {
		return {
			name,
	...
	if (name === "jwc_coordinator_report_status") {
		return {
			name,
	...
	if (name === "jwc_coordinator_read_artifact") {
		return {
			name,
	...
	if (name === "jwc_coordinator_read_status") {
		return {
			name,
	...
	if (name === "jwc_coordinator_read_tail") {
		return {
			name,
	...
	if (name === "jwc_coordinator_list_questions") {
		return {
			name,
...
export function createCoordinatorMcpServer(options: CoordinatorMcpServerOptions = {}) {
	...
	async function callTool(name: string, args: Record<string, unknown> = {}): Promise<Record<string, unknown>> {
		try {
			if (name === "jwc_coordinator_list_sessions") return { ok: true, sessions: await listSessions() };
			if (name === "jwc_coordinator_read_status") {
			...
			if (name === "jwc_coordinator_list_artifacts") return { ok: true, roots: config.allowedRoots };
			if (name === "jwc_coordinator_read_artifact")
