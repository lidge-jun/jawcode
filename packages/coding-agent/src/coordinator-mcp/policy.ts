...
function envFallback(env: NodeJS.ProcessEnv, jwcKey: string, gjcKey: string): string | undefined {
	return env[jwcKey] ?? env[gjcKey];
}

export function buildCoordinatorMcpConfig(env: NodeJS.ProcessEnv = process.env): CoordinatorMcpConfig {
	const stateRoot =
		envFallback(env, "JWC_COORDINATOR_MCP_STATE_ROOT", "GJC_COORDINATOR_MCP_STATE_ROOT")?.trim() ||
		path.join(process.cwd(), ".jwc", "state", "coordinator-mcp");
	return {
		allowedRoots: parseRootList(
			envFallback(env, "JWC_COORDINATOR_MCP_WORKDIR_ROOTS", "GJC_COORDINATOR_MCP_WORKDIR_ROOTS"),
		).map(root => path.resolve(root)),
		mutationClasses: parseMutationClasses(
			envFallback(env, "JWC_COORDINATOR_MCP_MUTATIONS", "GJC_COORDINATOR_MCP_MUTATIONS") ??
				envFallback(
					env,
					"JWC_COORDINATOR_MCP_ENABLE_MUTATION_CLASSES",
					"GJC_COORDINATOR_MCP_ENABLE_MUTATION_CLASSES",
				),
		),
		artifactByteCap: parseByteCap(
			envFallback(env, "JWC_COORDINATOR_MCP_ARTIFACT_BYTE_CAP", "GJC_COORDINATOR_MCP_ARTIFACT_BYTE_CAP") ??
				envFallback(env, "JWC_COORDINATOR_MCP_ARTIFACT_MAX_BYTES", "GJC_COORDINATOR_MCP_ARTIFACT_MAX_BYTES"),
		),
		namespace: {
			profile: cleanScope(envFallback(env, "JWC_COORDINATOR_MCP_PROFILE", "GJC_COORDINATOR_MCP_PROFILE")),
			repo: cleanScope(envFallback(env, "JWC_COORDINATOR_MCP_REPO", "GJC_COORDINATOR_MCP_REPO")),
		},
		stateRoot: path.resolve(stateRoot),
		sessionCommand:
			envFallback(env, "JWC_COORDINATOR_MCP_SESSION_COMMAND", "GJC_COORDINATOR_MCP_SESSION_COMMAND")?.trim() || null,
	};
}
