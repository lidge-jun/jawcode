...
import { isMCPBridgeTool } from "./runtime-mcp/discoverable-tool-metadata";
import {
	collectEnvSecrets,
...
import { type CustomMessage, convertToLlm, replaceLlmImagesWithText } from "./session/messages";
...
export async function createAgentSession(options: CreateAgentSessionOptions = {}): Promise<CreateAgentSessionResult> {
	...
	try {
		...
		const toolCleanups = new Map<string, () => Promise<void> | void>();
		const toolSession: ToolSession = {
			...
			registerSessionCleanup: (name, cleanup) => {
				toolCleanups.set(name, cleanup);
			},
		...
		const effectiveDiscoveryMode: "off" | "mcp-only" | "all" =
			toolsDiscoveryModeSetting === "all" ? "all" : settings.get("mcp.discoveryMode") ? "mcp-only" : "off";
		...
		for (const tool of toolRegistry.values()) {
			...
			const serverName = (tool as unknown as { mcpServerName?: string }).mcpServerName;
			discoverableMCPToolNames.add(tool.name);
			if (requestedToolNameSet.has(tool.name)) {
				explicitlyRequestedMCPToolNames.push(tool.name);
			}
			if (typeof serverName === "string" && settings.get("mcp.discoveryDefaultServers").includes(serverName)) {
		...
		if (mcpDiscoveryEnabled) {
			...
			defaultSelectedMCPToolNames =
				effectiveDiscoveryMode === "mcp-only"
					? [...discoverableMCPToolNames]
					: [...new Set([...discoveryDefaultServerToolNames, ...explicitlyRequestedMCPToolNames])];
			const shouldUsePersistedMCPSelection =
				existingSession.hasPersistedMCPToolSelection && explicitlyRequestedMCPToolNames.length === 0;
			initialSelectedMCPToolNames = shouldUsePersistedMCPSelection
		...
		// Keep images out of provider replay when disabled or unsupported by the active model.
		const convertToLlmWithBlockImages = (messages: AgentMessage[]): Message[] => {
			...
			return activeModel && !activeModel.input.includes("image")
