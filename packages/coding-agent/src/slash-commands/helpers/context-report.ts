	...
	try {
		...
		const activeContext =
			breakdown.usedTokens === null
				? `unknown / ${breakdown.contextWindow.toLocaleString()} tokens (${contextUsageSourceLabel(breakdown.source)})`
				: `${breakdown.usedTokens.toLocaleString()} / ${breakdown.contextWindow.toLocaleString()} tokens (${(
						(breakdown.usedTokens / breakdown.contextWindow) * 100
					).toFixed(1)}% used; ${contextUsageSourceLabel(breakdown.source)})`;
		const lines = [
			"Context usage",
