...
import { $env, getGpuCachePath, getProjectDir, hasFsCode, isEnoent, logger, prompt } from "@jawcode-dev/utils";
import { $ } from "bun";
...
export async function buildSystemPrompt(options: BuildSystemPromptOptions = {}): Promise<BuildSystemPromptResult> {
	...
	// Collect cli-jaw dev skills for the <dev-skill-routing> template block.
	// These are skills discovered from ~/.cli-jaw/skills/ — present only when
	// the jaw brand is active and the cli-jaw global skills directory exists.
	const cliJawDevSkills = skills
		.filter(skill => skill.source === "cli-jaw:user" && skill.name.startsWith("dev"))
