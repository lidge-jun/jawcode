...
import {
	...
	type CompactionProgressBoundaryCallback,
...
import { GOAL_PLAN_PENDING_BRIEF } from "../goals/goal-planning-start";
...
export interface AgentSessionConfig {
	...
	/** Tool-owned cleanup callbacks registered by session-scoped tools. */
	toolCleanups?: Map<string, () => Promise<void> | void>;
...
export class AgentSession {
	...
	readonly #toolCleanups: Map<string, () => Promise<void> | void> | undefined;
	...
	constructor(config: AgentSessionConfig) {
		...
		this.#toolCleanups = config.toolCleanups;
	...
	async buildForkContextSeed(options: ForkContextSeedOptions): Promise<ForkContextSeed> {
		...
		for (let i = providerMessages.length - 1; i >= 0; i--) {
			...
			const enc = this.model ? resolveModelEncoding(this.model) : undefined;
			const messageTokens = enc ? countMessageTokensNative(sanitized, enc) : estimateTokens(sanitized);
	...
	async #emitSessionEvent(event: AgentSessionEvent): Promise<void> {
		...
		const { promise: ticket, resolve: releaseTicket } = Promise.withResolvers<void>();
		this.#subscriberEmitGate = ticket;
		try {
			await persistCoordinatorRuntimeStateFromEvent(event, {
				sessionId: this.sessionId,
				cwd: this.sessionManager.getCwd(),
	...
	#createCompactionProgressSink(
		...
		update: Omit<CompactionProgressUpdate, "trigger" | "mode"> & { mode?: CompactionProgressUpdate["mode"] },
	) => void {
		let maxPercent = 0;
		return update => {
			const boundaryPercent = Math.max(0, Math.min(100, update.percent));
			const resetsProgress = update.phase === "cancelled" || update.phase === "failed";
			maxPercent = resetsProgress ? boundaryPercent : Math.max(maxPercent, boundaryPercent);
			const enriched: CompactionProgressUpdate = {
				...update,
				percent: maxPercent,
				trigger,
				mode: update.mode ?? defaultMode,
			};
			try {
				const result = options?.onProgress?.(enriched);
				void Promise.resolve(result).catch((error: unknown) => {
					logger.warn("Compaction progress callback failed", {
			...
			void this.#emitSessionEvent({ type: "compaction_progress", update: enriched }).catch(error => {
				logger.warn("Compaction progress event failed", {
					error: error instanceof Error ? error.message : String(error),
				});
			});
		};
	...
	async #emitExtensionEvent(event: AgentSessionEvent): Promise<void> {
		...
		} else if (event.type === "compaction_progress") {
			// High-frequency/ephemeral progress is exposed to direct session subscribers
			// and wire observers only. It is intentionally not an extension hook.
			return;
	...
	async dispose(): Promise<void> {
		...
		if (this.#toolCleanups && this.#toolCleanups.size > 0) {
			const entries = Array.from(this.#toolCleanups.entries());
			this.#toolCleanups.clear();
			const results = await Promise.allSettled(entries.map(([, cleanup]) => cleanup()));
			for (let index = 0; index < results.length; index++) {
				const result = results[index];
				if (result?.status === "rejected") {
					logger.warn("Tool cleanup failed during dispose", {
	...
	async refreshJwcSubskillTools(): Promise<void> {
		const activeState = await readSessionStrictSkillActiveState(
			this.sessionManager.getCwd(),
	...
	buildModelSessionContext(): SessionContext {
	...
	async #activatePendingJwcGoalModeRequest(): Promise<boolean> {
		...
		if (currentState?.goal && currentState.goal.status !== "complete" && currentState.goal.status !== "dropped") {
			if (!currentState.goal.objective.startsWith(GOAL_PLAN_PENDING_BRIEF)) {
				return false;
			}
	...
	async #syncSkillPromptActiveState(
		...
		const sessionId = this.sessionManager.getSessionId();
		// Canonical JWC workflow skills (jaw-interview, plan, goal, team)
	...
	setFastMode(enabled: boolean): void {
		...
		this.setServiceTier(enabled ? "priority" : "default");
		...
		// the saved preference. Fast-off persists `"default"` (not `"none"`)
		// so OpenAI/Codex requests explicitly override any stale fast tier.
		this.settings.set("serviceTier", enabled ? "priority" : "default");
	}
	...
	async compact(customInstructions?: string, options?: CompactOptions): Promise<CompactionResult> {
		...
		const progress = this.#createCompactionProgressSink("manual", "context-full", options);
		...
		try {
			...
			const encoding = resolveModelEncoding(this.model);
			const preparation = prepareCompaction(pathEntries, compactionSettings, encoding);
			if (!preparation) {
			...
			progress({
				phase: "awaiting_hooks",
				percent: 5,
				segment: "setup_hooks",
				message: "Running compaction hooks…",
			});
			...
			progress({
				phase: "customizing_prompt",
				percent: 15,
				segment: "setup_hooks",
				message: "Applying compaction context…",
			});
			...
			progress({
				phase: error instanceof CompactionCancelledError ? "cancelled" : "failed",
				percent: 0,
				segment: "terminal",
				message: error instanceof CompactionCancelledError ? "Compaction cancelled" : "Compaction failed",
			});
			options?.onError?.(err);
			throw error;
		} finally {
	...
	async #checkCompaction(assistantMessage: AssistantMessage, skipAbortedCheck = true): Promise<void> {
		...
		// Thresholds reserve a future output budget, so compare against the prompt/input
		// side of the last request. Using total usage here double-counts generated
		// output and can compact immediately after a long answer even when the next
		// prompt would still fit inside the reserved-output threshold.
		//
		...
		// line already trusts and use whichever is larger.
		const contextTokensBeforeMaintenance = Math.max(promptTokens, this.#estimateMessagesTokens());
		const maxOutputTokens = this.model ? effectiveMaxOutputTokens(this.model) : 0;
	...
	async #runAutoCompaction(
		...
		const trigger: CompactionProgressTrigger =
			reason === "overflow" ? "auto_overflow" : reason === "idle" ? "auto_idle" : "auto_threshold";
		let progress = this.#createCompactionProgressSink(trigger, action);
		progress({
			phase: "preparing",
			percent: 0,
			segment: "setup_prepare",
			message: action === "handoff" ? "Preparing handoff…" : "Preparing compaction…",
			mode: action,
		});
				...
				if (!handoffResult) {
					...
					if (aborted) {
						progress({
							...
							percent: 0,
							segment: "terminal",
							message: "Auto-handoff cancelled",
							mode: "handoff",
						});
						await this.#emitSessionEvent({
							type: "auto_compaction_end",
					...
					action = "context-full";
					progress = this.#createCompactionProgressSink(trigger, "context-full");
					progress({
						phase: "preparing",
						percent: 0,
						segment: "setup_prepare",
						message: "Falling back to context-full maintenance…",
						mode: "context-full",
					});
				}
				if (handoffResult) {
	...
	async switchSession(sessionPath: string): Promise<boolean> {
		...
		try {
			...
			const displaySessionContext = this.buildDisplaySessionContext();
			const modelSessionContext = this.buildModelSessionContext();
			const didReloadConversationChange =
				!switchingToDifferentSession &&
				this.#didSessionMessagesChange(previousSessionContext.messages, displaySessionContext.messages);
			const fallbackSelectedMCPToolNames = this.#getSessionDefaultSelectedMCPToolNames(sessionPath);
			await this.#restoreMCPSelectionsForSessionContext(modelSessionContext, { fallbackSelectedMCPToolNames });
			...
			this.agent.replaceMessages(modelSessionContext.messages);
			this.#syncTodoPhasesFromBranch();
			...
			const defaultModelStr = modelSessionContext.models.default;
			if (defaultModelStr) {
			...
			const nextThinkingLevel = resolveThinkingLevelForModel(
				this.model,
				hasThinkingEntry ? (modelSessionContext.thinkingLevel as ThinkingLevel | undefined) : defaultThinkingLevel,
			);
			...
			this.agent.serviceTier = hasServiceTierEntry
				? modelSessionContext.serviceTier
	...
	async branch(entryId: string): Promise<{
		...
		await this.#restoreMCPSelectionsForSessionContext(modelSessionContext);
	...
	async navigateTree(
		...
		// Update agent state with compacted model context, but return visible
		// transcript context to UI callers.
		const modelContext = this.buildModelSessionContext();
		const displayContext = this.buildDisplaySessionContext();
		await this.#restoreMCPSelectionsForSessionContext(modelContext);
		this.agent.replaceMessages(modelContext.messages);
		this.#syncTodoPhasesFromBranch();
		this.#closeCodexProviderSessionsForHistoryRewrite();
		...
		if (this.#extensionRunner?.hasHandlers("session_tree")) {
			...
			const rawContext = this.buildDisplaySessionContext();
	...
	#estimateMessageNativeContextTokens(msg: AgentMessage): number {
		const encoding = this.model ? resolveModelEncoding(this.model) : undefined;
		return encoding ? countMessageTokensNative(msg, encoding) : estimateTokens(msg);
	}
