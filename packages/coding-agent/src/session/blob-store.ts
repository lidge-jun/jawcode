...
/** Synchronously resolve a resident provider image data URL, throwing when the resident blob is missing. */
export function resolveResidentImageDataUrlSync(
	...
	const buffer = blobStore.getSync(hash);
	if (!buffer) {
		throw new ResidentBlobMissingError(hash, context?.kind ?? "imageUrl", context?.sessionId, context?.sessionFile);
	}
	return buffer.toString("utf8");
}

/** Synchronously resolve resident image base64 data, throwing when the resident blob is missing. */
export function resolveResidentImageDataSync(
	...
	const buffer = blobStore.getSync(hash);
	if (!buffer) {
		throw new ResidentBlobMissingError(hash, context?.kind ?? "imageData", context?.sessionId, context?.sessionFile);
	}
	return buffer.toString("base64");
}
