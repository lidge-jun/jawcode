...
import { $which, isRecord, logger, pathIsWithin } from "@jawcode-dev/utils";
...
function getConfigSources(cwd: string): ConfigSource[] {
	...
	// Project config directories (.jwc/, .gemini/)
	...
	// User config directories (~/.jwc/agent/, ~/.gemini/)
 ...
 * 2. Project config dirs: .jwc/lsp.*, .gemini/lsp.* (+ hidden variants)
 * 3. User config dirs: ~/.jwc/agent/lsp.*, ~/.gemini/lsp.* (+ hidden variants)
