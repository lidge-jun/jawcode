...
export class CommandController {
	...
	async executeCompaction(
		...
		const progressPresenter = new CompactionProgressPresenter({
			setMessage: message => compactingLoader.setMessage(message),
			getWidth: () => this.ctx.ui.terminal.columns,
			prefix: isAuto ? "Auto-compacting context" : "Compacting context",
		...
		try {
			...
			await this.ctx.session.compact(instructions, {
				...options,
				onProgress: update => {
					progressPresenter.update(update);
					return options?.onProgress?.(update);
				},
			});
