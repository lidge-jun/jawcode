...
export class EventController {
	...
	constructor(private ctx: InteractiveModeContext) {
		this.#handlers = {
			...
			compaction_progress: e => this.#handleCompactionProgress(e),
	...
	#getReadGroup(): ReadToolGroupComponent {
		if (!this.#lastReadGroup) {
			...
			markLiveToggleEligible(group, true);
		...
		const prefix =
			event.reason === "overflow"
				...
				: event.action === "handoff"
					? "Preparing auto-handoff"
					: event.reason === "idle"
						? "Running idle context maintenance"
						: "Running context maintenance";
		this.ctx.autoCompactionProgressPresenter = new CompactionProgressPresenter({
			setMessage: message => this.ctx.autoCompactionLoader?.setMessage(message),
			getWidth: () => this.ctx.ui.terminal.columns,
			prefix,
			cancelHint: "(esc to cancel)",
		});
	}

	async #handleCompactionProgress(event: Extract<AgentSessionEvent, { type: "compaction_progress" }>): Promise<void> {
		if (event.update.trigger === "manual") return;
		this.ctx.autoCompactionProgressPresenter?.update(event.update);
	}
	...
	async #handleAutoCompactionEnd(event: Extract<AgentSessionEvent, { type: "auto_compaction_end" }>): Promise<void> {
		...
		if (this.ctx.autoCompactionProgressPresenter) {
			this.ctx.autoCompactionProgressPresenter.stop();
			this.ctx.autoCompactionProgressPresenter = undefined;
		}
	...
	sendCompletionNotification(settledMessages?: AgentEndMessages): void {
		...
		const terminal = settledMessages !== undefined ? settledMessages.at(-1) : undefined;
		if (settledMessages !== undefined && terminal?.role !== "assistant") return;
		const last = terminal?.role === "assistant" ? terminal : this.ctx.session.getLastAssistantMessage?.();
