...
import { $pickenv, readLines, Snowflake } from "@jawcode-dev/utils";
import type {
	ExtensionUIContext,
...
export function shouldEmitRpcTitlesForTest(): boolean {
	const raw = $pickenv("JWC_RPC_EMIT_TITLE", "GJC_RPC_EMIT_TITLE", "PI_RPC_EMIT_TITLE");
	if (!raw) return false;
...
export function assertRpcListenSupported(platform: NodeJS.Platform = process.platform): void {
	if (platform === "win32") {
		throw new Error("RPC --listen requires Unix-domain socket support");
	}
}

/**
 * Probe whether a unix-domain socket path has a live server accepting
 * connections. Returns `true` only when a connection succeeds, so `--listen`
 * startup can refuse to unlink a live endpoint instead of stealing it. Missing
 * paths and stale socket files with no listener return `false`.
 */
...
export async function runRpcMode(
	...
	if (options?.listen) {
		...
		type WritableSocket = { write: (line: string) => void };
		let currentSocket: WritableSocket | undefined;
		let buffer = "";
		try {
			Bun.listen({
				unix: socketPath,
				socket: {
					...
					data(socket, data) {
						...
						buffer += inputDecoder.decode(data);
						while (true) {
			...
			throw new Error(`failed to bind rpc listen socket ${socketPath}: ${decodeError(err)}`);
		}
