...
export function estimateSkillsTokens(skills: readonly Skill[], encoding?: Encoding): number {
...
export function estimateToolSchemaTokens(tools: ReadonlyArray<Tool>, encoding?: Encoding): number {
	const fragments: string[] = [];
 ...
 */
export function computeNonMessageTokens(session: AgentSession, encoding?: Encoding): number {
	const parts = computeNonMessageBreakdown(session, encoding);
	return (
...
function computeNonMessageBreakdown(
	...
	const skillsTokens = estimateSkillsTokens(session.skills ?? [], encoding);
	const toolsTokens = estimateToolSchemaTokens(session.agent?.state?.tools ?? [], encoding);
	const systemPromptParts = session.systemPrompt ?? [];
	const rulesTokens = estimateRulesTokens(systemPromptParts, encoding);
	const systemContextTokens = countTokens(systemPromptParts.slice(1), encoding);
	const systemPromptTokens = Math.max(
		0,
		countTokens(systemPromptParts[0] ?? "", encoding) - skillsTokens - rulesTokens,
	);
	return { rulesTokens, skillsTokens, toolsTokens, systemContextTokens, systemPromptTokens };
}

function estimateRulesTokens(systemPromptParts: readonly string[], encoding?: Encoding): number {
	const fragments: string[] = [];
...
function splitLastUserTurn(
	...
	for (let i = 0; i < messages.length; i++) {
		const tokens = encoding ? countMessageTokensNative(messages[i], encoding) : estimateTokens(messages[i]);
		if (i === lastUserIndex) {
...
export function computeContextBreakdown(
	session: AgentSession,
	options: { messages?: readonly AgentMessage[]; encoding?: Encoding } = {},
): ContextBreakdown {
	...
	const { regularMessagesTokens, lastUserTurnTokens } = splitLastUserTurn(convo, encoding);

	const { rulesTokens, skillsTokens, toolsTokens, systemContextTokens, systemPromptTokens } =
		computeNonMessageBreakdown(session, encoding);
