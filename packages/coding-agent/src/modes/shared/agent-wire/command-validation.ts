...
function getStateInclude(value: unknown): boolean {
	const allowed = new Set(["tools", "dumpTools", "systemPrompt"]);
	const record = isRecord(value) ? value : {};
	if (record.include === undefined) return true;
	return Array.isArray(record.include) && record.include.every(item => typeof item === "string" && allowed.has(item));
}
