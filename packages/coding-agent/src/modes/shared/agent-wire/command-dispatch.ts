...
export async function dispatchRpcCommand(
	...
	switch (command.type) {
		...
		case "get_state": {
			...
			if (include.has("systemPrompt")) state.systemPrompt = session.systemPrompt;
		...
		case "login": {
			const resolvedProviderId = resolveOAuthProviderId(command.providerId);
			...
			try {
				...
				await session.modelRegistry.refreshProvider(resolvedProviderId, "online");
