...
export class ToolExecutionComponent extends Container {
	...
	renderFullTranscript(width: number): string[] {
		...
		const previousFullTranscript = this.#fullTranscript;
		this.#fullTranscript = true;
		if (this.#expanded) {
			this.#updateDisplay();
		} else {
		...
		try {
			...
			this.#fullTranscript = previousFullTranscript;
			if (this.#expanded === previousExpanded) {
				this.#updateDisplay();
			} else {
				this.setExpanded(previousExpanded);
			}
		}
	}
	...
	#updateDisplay(): void {
		...
		this.#renderState.fullTranscript = this.#fullTranscript;
		this.#renderState.spinnerFrame = this.#spinnerFrame;
	...
	#formatToolExecution(): string {
		...
		if (textContent.startsWith("{") || textContent.startsWith("[")) {
			try {
				...
				const maxDepth = this.#fullTranscript
					? FULL_TRANSCRIPT_JSON_LIMIT
					: this.#expanded
						? JSON_TREE_MAX_DEPTH_EXPANDED
						: JSON_TREE_MAX_DEPTH_COLLAPSED;
				const maxLines = this.#fullTranscript
					? FULL_TRANSCRIPT_JSON_LIMIT
					: this.#expanded
						? JSON_TREE_MAX_LINES_EXPANDED
						: JSON_TREE_MAX_LINES_COLLAPSED;
				const maxScalarLen = this.#fullTranscript
					? FULL_TRANSCRIPT_JSON_LIMIT
					: this.#expanded
						? JSON_TREE_SCALAR_LEN_EXPANDED
						: JSON_TREE_SCALAR_LEN_COLLAPSED;
		...
		const maxOutputLines = this.#fullTranscript ? Number.POSITIVE_INFINITY : this.#expanded ? 12 : 4;
