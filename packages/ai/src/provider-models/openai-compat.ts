				...
				return (
					fetchOpenAICompatibleModels({
						...
						mapModel: (
							...
							// When a [1m] variant exists for this model, the base model
							// must keep its bundled contextWindow — the API may return
							// 1M for Max-plan users, but the user selects [1m] explicitly.
							const has1mVariant = references.has(`${defaults.id}[1m]`);
							const clampedContextWindow = has1mVariant ? reference.contextWindow : undefined;
							return {
								...reference,
								id: defaults.id,
								name: discoveredName,
								...
								provider: "anthropic",
								baseUrl,
								...(clampedContextWindow !== undefined ? { contextWindow: clampedContextWindow } : {}),
								...(unlisted ? { unlisted: true } : {}),
							};
						},
					}) ?? null
