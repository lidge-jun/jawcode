...
- Shell commands are KILLED at their timeout (default 120s) and killed output is PARTIAL — if a result says the command was killed or timed out, the work did NOT finish; never treat a fallback message (e.g. \`|| echo done\`) as completion. Retry with a NARROWER scope, never with a slower variant of the same scan.
- For repository-wide searches use rg (fast, ignore-aware). With find, prune heavy directories (\`-path ./node_modules -prune -o\`) — \`! -path\` only filters output and still walks everything — and use \`-exec cmd {} +\` or xargs, never \`-exec cmd \\;\` per file. Bulk text replacement is one command: \`rg -l PATTERN | xargs perl -pi -e 's/OLD/NEW/g'\`.`;
