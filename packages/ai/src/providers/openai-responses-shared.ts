import { structuredCloneJSON } from "@jawcode-dev/utils";
...
import { normalizeResponsesToolCallId, serializeToolArguments } from "../utils";
...
export function applyCommonResponsesSamplingParams<P extends CommonResponsesParams>(
	...
	if (shouldSendServiceTier(options?.serviceTier, provider)) {
		...
		if (resolved === "default" || resolved === "flex" || resolved === "scale" || resolved === "priority") {
...
export function applyResponsesReasoningParams<P extends OpenAI.Responses.ResponseCreateParamsStreaming>(
	...
	// we must include the full content). See: https://github.com/lidge-jun/jawcode/issues/41
