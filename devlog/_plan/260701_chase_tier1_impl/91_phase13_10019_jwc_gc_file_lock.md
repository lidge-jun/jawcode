...
Port the orchestrator types + probe + collect/prune + arg-parse + exit-code + `runJwcGcCommand` + help. **Naming: `jwc gc`** in help text (not `gjc gc`); `GcStore` union limited to what's wired now: keep the full union type for forward-compat but `GC_STORES` runtime list + `defaultGcAdapters()` returns ONLY `[fileLocksGcAdapter]` this cycle. Help text JWC-branded.
...
Port `buildGcReportText`; `STORE_HEADINGS` for the file_locks store (+ forward-compat entries). `jwc gc` branding.
