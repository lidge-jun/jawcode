...
| 2 | DDG native-browser-aligned requests | `09061ed80` | ALREADY SHIPPED — rotated desktop UAs + Accept/Accept-Language headers | CONFIRM (no-op) |
...
## Design (JWC-authored ADAPT of OMP `3560d108d`)

### Slice A — apply configured provider settings in standalone search CLI
...
### Slice B — focused test
`packages/coding-agent/test/web/search/cli-provider-settings.test.ts` (NEW), JWC-adapted from OMP `cli-provider-settings.test.ts`:
