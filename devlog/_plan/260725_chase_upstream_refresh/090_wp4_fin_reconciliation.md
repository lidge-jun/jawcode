...
- D8: phase gate is **every card closed in wp4 passes bidirectional closure-integrity checks and wp4 introduces no new integrity violation**. Mechanically, every cited commit must touch at least one explicit narrow owner path and every declared owner path must be touched by at least one cited commit; existence, classification, and residual checks also remain required. This proves commit/owner relation only, not that the prose closure claim is semantically true, so card-level human/reviewer judgment remains mandatory. The script still exits 1 on the legacy-evidence backlog; full exit 0 requires a separate, non-autonomous audit that substantiates each old closure or returns it active.
