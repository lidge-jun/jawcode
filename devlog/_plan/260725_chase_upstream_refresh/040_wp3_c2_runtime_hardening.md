...
|---|---|---|
| `6d7457663` + `09d02c641` | per-command bash approval rules (`bash.patterns` settings) + close deny/prompt precedence and raw shell-control bypasses | `packages/coding-agent/src/tools/bash.ts`, `tools/approval.ts`, `config/settings-schema.ts` (+tests) |
...
| `d1f279971` | opt-in Markdown read previews — **JWC adapt decision: add `read.renderMarkdown` setting but DEFAULT PRESERVES current JWC behavior (render on); setting allows opting out.** Upstream flipped the default to off; silently changing JWC's visible default is a product decision (C), so the port keeps behavior and adds the knob. A-audit must challenge this. | `tools/read.ts`, `config/settings-schema.ts` (+tests) |
...
| commit | change | JWC owner |
|---|---|---|
| `6b6e39ff6` | bundle mupdf in compiled CLI (drop `--external mupdf`) | `packages/coding-agent/scripts/build-binary.ts` |
| `fcf2274b2` | cancel pending saves on discarded settings instances | `config/settings.ts` (+test) |
| `03489d1eb` | coalesce python kernel replacement (shared replacement promise/generation) | `eval/py/executor.ts` (+test) |
| `5a1f227a6` | synchronized memory backend lifecycle (transition/startup cancellation) | `memories/index.ts`, `session/agent-session.ts`, `memory-backend/` (+tests) |
...
- A3: mupdf slice validated by `bun run build:native`-adjacent check or compile-arg diff review (full binary build optional — note evidence either way).
